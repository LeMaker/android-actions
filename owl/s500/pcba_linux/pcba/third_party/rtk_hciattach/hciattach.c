...
struct uart_t {
	...
	int  flags;
	char *bdaddr;
	int  (*init) (int fd, struct uart_t *u, struct termios *ti, enum bt_module);
...
static int realtek_init(int fd, struct uart_t *u, struct termios *ti, enum bt_module device_m)
{
	...
	return rtk_init(fd, u->proto, u->speed, ti, device_m);
} 
...
//Realtek_add_start
	...
	{ "rtk_h5",     0x0000, 0x0000, HCI_UART_3WIRE, 115200,115200, 0,        NULL, realtek_init, realtek_post},
//Realtek_add_end
	{ NULL, 0 }
};
...
static int init_uart(char *dev, struct uart_t *u, int send_break, int raw, enum bt_module device_m)
{
	struct termios ti;
	int fd, i;
	...
	//RS_DBG ("init_uart--------------open");
	fd = open(dev, O_RDWR | O_NOCTTY);
	if (fd < 0) {
		perror("Can't open serial port");
		RS_DBG ("Can't open serial port");
		return -1;
	}
	...
	//	RS_DBG ("set_speed 1-----------%d",u->init_speed);
	/* Set initial baudrate */
	if (set_speed(fd, &ti, u->init_speed) < 0) {
		perror("Can't set initial baud rate");
		RS_DBG ("Can't set initial baud rate");
		return -1;
	}
	...
	if (u->init && u->init(fd, u, &ti, device_m) < 0)
		return -1;

	tcflush(fd, TCIOFLUSH);

	RS_DBG ("set_speed 2-----------%d",u->speed);
	/* Set actual baudrate */
	if (set_speed(fd, &ti, u->speed) < 0) {
		perror("Can't set baud rate");
		return -1;
...
int main(int argc, char *argv[])
{
	...
	sigset_t sigs;
	char dev[PATH_MAX];
	enum bt_module device_m = rtl8723as;
	...
	while ((opt=getopt(argc, argv, "d:bnpt:s:lr")) != EOF) {
		...
		case 'd':
			printf("optarg = %s\n", optarg);
			if(!strcmp(optarg, "rtl8723as"))
				device_m = rtl8723as;
			if(!strcmp(optarg, "rtl8723bs"))
				device_m = rtl8723bs;
			if(!strcmp(optarg, "rtl8723vq0"))
				device_m = rtl8723vq0;
			if(!strcmp(optarg, "rtl8761"))
				device_m = rtl8761;
			printf("device_m = %d\n", device_m);
			break;
		case 'b':
	...
	n = init_uart(dev, u, send_break, raw, device_m);
	if (n < 0) {
		perror("Can't initialize device"); 
		RS_DBG("Can't initialize device");
		exit(1);
	}
