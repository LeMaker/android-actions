...
static char firmware_path[50];
static char firmware_name[50];
...
#else //for Linux 
#define FIRMWARE_DIRECTORY "/system/etc/rtl8723bs/"
#define BT_CONFIG_DIRECTORY "/system/etc/rtl8723bs/"
#endif
...
static const char *get_firmware_name()
{
	...
	ret = sprintf(firmware_file_name, "%s%s", firmware_path, firmware_name);
...
RT_U32 rtk_parse_config_file(RT_U8* config_buf, size_t filelen, char bt_addr[6])
{
	...
	RT_U16 config_len = config->data_len, temp = 0;
...
//--------------------------
GET_CONFIG:
	// ret = sprintf(bt_config_file_name, BT_CONFIG_DIRECTORY"rtk8723_bt_config"); 
	ret = sprintf(bt_config_file_name, "%s%s", firmware_path, config_name);
...
int rtk_vendor_change_speed_h5(int fd, RT_U32 baudrate)
{
	...
	if(DBG_ON)
	{
		...
		printf("change speed command ready baudrate=%d n",baudrate);
	}
...
*/
int rtk_init(int fd, int proto, int speed, struct termios *ti, enum bt_module device_m)
{
	struct sigaction sa;
	int retlen;
	...
	{
		case rtl8723as:
			strcpy(firmware_path, "/lib/firmware/");
			strcpy(firmware_name, "rtk8723a");
			strcpy(config_name, "rtk8723_bt_config");
			break;
		case rtl8723bs:
			strcpy(firmware_path, "/system/etc/rtl8723bs/");
			strcpy(firmware_name, "rtl8723b_fw");
			strcpy(config_name, "rtk8723_bt_config");
			break;
		case rtl8723vq0:
			strcpy(firmware_path, "/system/etc/rtl8723vq0/");
			strcpy(firmware_name, "rtl8723b_fw");
			strcpy(config_name, "rtl8723b_VQ0_config");
			break;
		case rtl8761:
			strcpy(firmware_path, "/system/etc/rtl8761/");
			strcpy(firmware_name, "rtl8761a_fw");
			strcpy(config_name, "rtl8761a_config");
			break;
		default:
			RS_ERR("device type error\n");
			break;
	}
