...
int ReadJpegSections (FILE * infile, ReadMode_t ReadMode)
{
  ...
  /**
   *    ACTIONS_CODE(author:lishiyuan, change_code:improve compatibility 
   *                  for jpeg sections head have too many padding bytes)
   */
        #define COUNT  256				 
        for (a=0;a<=COUNT;a++){
            marker = fgetc(infile);
...
/**
 *    ACTIONS_CODE(author:lishiyuan, bugfix:Add mutex jhead_mutex to protect
 *                 the global variable SectionsRead from being accessed by 
 * 			       muti_thread accessed --->modified by muti_thread at the same
 *				   time )
 */

static pthread_mutex_t jhead_mutex = PTHREAD_MUTEX_INITIALIZER; 
...
int ReadJpegFile(const char * FileName, ReadMode_t ReadMode)
{
    ...
    pthread_mutex_lock(&jhead_mutex);
    ...
    if (infile == NULL) {
        ...
        pthread_mutex_unlock(&jhead_mutex);
        return FALSE;
    }
