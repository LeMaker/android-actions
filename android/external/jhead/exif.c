...
// Actions Add for FocalLengthTag
int IsFocalLengthTag(unsigned short tag)
{
    return ((tag == TAG_FOCALLENGTH)? TRUE: FALSE);
}
...
// Actions Add for FocalLengthTag
//--------------------------------------------------------------------------
static void create_EXIF_internal(ExifElement_t* elements, int exifTagCount, int gpsTagCount, int hasDateTimeTag, int hasFocalLengthTag, char* Buffer)
{
...

    {
        ...
        if (hasFocalLengthTag) {
            //TAG_FOCALLENGTH may not write in Image Info, Otherwise, it can not be read by OS!
            --NumEntries;
        }
...
#endif
                if (elements[i].Tag == TAG_FOCALLENGTH) {
                    //TAG_FOCALLENGTH may not write in Image Info, Otherwise, it can not be read by OS!
                    FocalDataLength =  entry->DataLength;
                    FocalValue = (long)elements[i].Value;
                }
                else {
...

    {
        ...
        NumEntries = 2;
        if (hasFocalLengthTag) {
            //TAG_FOCALLENGTH may write in Camera Info!
            ++NumEntries;
        }
        DataWriteIndex += 2 + NumEntries*12 + 4;
        ...
        if (hasFocalLengthTag) {
            // Link to exif dir entry -- Write TAG_FOCALLENGTH tag here!!!!
            writeExifTagAndData(TAG_FOCALLENGTH,
                                FMT_URATIONAL,
                                FocalDataLength,
                                FocalValue,
                                TRUE,
...
// Actions Add for FocalLengthTag
void create_EXIF(ExifElement_t* elements, int exifTagCount, int gpsTagCount, int hasDateTimeTag, int hasFocalLengthTag)
{
    ...
    if (Buffer != NULL) {
        create_EXIF_internal(elements, exifTagCount, gpsTagCount, hasDateTimeTag, hasFocalLengthTag, Buffer);
        free(Buffer);
    } else {
        ErrFatal("Could not allocate memory");
    }
