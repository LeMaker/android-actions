 ...
 */
static int progressive_jpeg_accelarate = -1;

GLOBAL(boolean)
jpeg_start_decompress (j_decompress_ptr cinfo)
{
  
  if (progressive_jpeg_accelarate == -1)
  {
      boolean native_standard = FALSE;
      char value[PROPERTY_VALUE_MAX];

	  property_get("ro.skia.img.decode.standard",value,"NATIVE");
	
      native_standard = !strcmp(value,"NATIVE");

      if(native_standard == TRUE)
      {
	     progressive_jpeg_accelarate = 0;
	     printf("progressive jpeg unaccelarate  in %s",__FUNCTION__); 
      }else{
	     progressive_jpeg_accelarate = 1;
		 printf("progressive jpeg accelarate  in %s",__FUNCTION__); 
	  }
  }
  ...
  if (cinfo->global_state == DSTATE_PRELOAD) {
    ...
    /**
        * ACTIONS_CODE(author:lishiyuan, optimaze: if cts version, we have to decode 
        *            all scan layers of progressive jpeg  for higher precision, but 
        *            for not cts version, only decode layers less than 7 to accelarate 
        *	         decoding)
        */  
	if (retcode == JPEG_REACHED_EOI \
	    || (cinfo->input_scan_number >= 7 && progressive_jpeg_accelarate == 1))
	  break;
