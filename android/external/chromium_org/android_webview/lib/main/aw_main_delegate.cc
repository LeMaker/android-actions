...
/**
  *
  ************************************
  * ActionsCode(author:hexibin, change_code)
  */
bool AwMainDelegate::BasicStartupComplete(int* exit_code) {
  ...
  //ActionsCode(author:hexibin, optimize)
  cl->AppendSwitch(switches::kDisable2dCanvasAntialiasing);
  cl->AppendSwitch(cc::switches::kDisableCompositedAntialiasing);
  cl->AppendSwitch(switches::kEnableThreadedCompositing);

  //cl->AppendSwitch(switches::kDisableGpuVsync);    //this switch will cause white screen and crash
  
  cl->AppendSwitch(switches::kEnableSmoothScrolling);
  cl->AppendSwitch(switches::kEnablePreparsedJsCaching);
  
  cl->AppendSwitch(switches::kDisableLowResTiling);
  //cl->AppendSwitch(switches::kForceGpuRasterization);

  /**
   * ActionsCode(author:cheasonxie, change_code)
   *   disable overlay fullscreen video, fix display problem in fullscreen
   */   
  cl->AppendSwitch(switches::kDisableOverlayFullscreenVideoSubtitle);

  // debug switch below:
  //cl->AppendSwitch(cc::switches::kShowCompositedLayerBorders);
  //cl->AppendSwitch(cc::switches::kUIShowCompositedLayerBorders);

  //cl->AppendSwitch(cc::switches::kShowFPSCounter);
  //cl->AppendSwitch(cc::switches::kUIShowFPSCounter);
