...
SynchronousInputEventFilter::SynchronousInputEventFilter() {
  ...
  */
SynchronousInputEventFilter::~SynchronousInputEventFilter() {
  //ActionsCode(hexibin, bugfix BUG00233631)
  for(std::set<IdInputMap*>::const_iterator it=id_input_map_.begin(); it!=id_input_map_.end(); ++it){
    delete *it;
  }
}
...
InputEventAckState SynchronousInputEventFilter::HandleInputEvent(
  ...
  for(std::set<IdInputMap*>::const_iterator it=id_input_map_.begin(); it!=id_input_map_.end(); ++it){
    if((*it)->first == routing_id){
      ...
      delete *it;
      id_input_map_.erase(*it);
      DidAddInputHandler(routing_id, input_handler);
      break;
    }
  }
...
void SynchronousInputEventFilter::DidAddInputHandler(
  ...
  if (compositor){
    ...
    //ActionsCode(hexibin, bugfix BUG00233631)
    id_input_map_.insert(new IdInputMap(routing_id, input_handler));
  }
}
