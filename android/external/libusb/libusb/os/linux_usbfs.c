...
#include "linux_usbfs.h"
  ...
  * temporary hack ....
  *   
  *ActionsCode(author:ywwang, type:change_code)
  */
...
static inline int usbdev_isdigit(int ch)
{
	return (ch >= '0') && (ch <= '9');
}
 ...
 /**
  *support 3g dongle
  *     change from idgigit to usbdev_isdigit 
  *ActionsCode(author:ywwang, type:change_code)
  */
	...
	struct discovered_devs **_discdevs, int *usbfs_fallback)
{
	...
	while ((entry = readdir(devices))) {
		...
		if ((!usbdev_isdigit(entry->d_name[0]) && strncmp(entry->d_name, "usb", 3))
				|| strchr(entry->d_name, ':'))
			continue;
