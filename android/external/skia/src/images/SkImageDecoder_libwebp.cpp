...
SkImageDecoder::Result SkWEBPImageDecoder::onDecode(SkStream* stream, SkBitmap* decodedBitmap,
    ...
    /**
     * ACTIONS_CODE(optimize: unsupported to decode  webp picture  that was
     *      too large(>32x1024x1024) for avoiding spending too much memory.)
     */

	if(  (sampler.scaledWidth()*sampler.scaledHeight() * 2) > 32* 1024 *1024) 
	{
			SkDebugf(" Need large memory, Not supported   %d     %d     %d     %d",\
			     __LINE__,sampler.scaledWidth(), sampler.scaledHeight(), sampleSize);
	  		return kFailure;
	}
