...
bool SkPNGImageDecoder::onBuildTileIndex(SkStreamRewindable* sk_stream, int *width, int *height) {
    ...
    fImageIndex->build_index = false;
    fImageIndex->bitmap = NULL;
    fImageIndex->reserve_bitmap = NULL;
    ...
    *height = origHeight;
    ...
    if((origWidth * origHeight <= SOFTDEC_MAX_SIZE)&&(native_standard == true))
    {
    ...
    SkDebugf("==png=====sdram : %d=======doEnhance : %d ============%d   %d \n",sdram_cap,doEnhance,origWidth,origHeight);
    
    if(sdram_cap >= SDRAM_SUPPORT && doEnhance == 1 && native_standard == false)    
    {
        /*decode png into bitmap*/
        config = kRGB_565_SkColorType;
        fImageIndex->fColorType = config;
        if(config == kN32_SkColorType)
            shift = 2;
            
        SkBitmap *bitmap = new SkBitmap;
        fImageIndex->bitmap = bitmap;  
        
         //SkDebugf("sampleSize : %d",sampleSize);
         //SkDebugf("sampleSize_sam : %d",(origWidth/sampleSize)*(origHeight/sampleSize));
              
        while((origWidth/sampleSize)*(origHeight/sampleSize) > MAX_DECODESIZE)
        {
            sampleSize<<=1;
        }
       ...
       // SkDebugf("sampleSize : %d",sampleSize);
    	...
    	//SkDebugf("config : %d   dst_w:  %d  dst_h:  %d ",config,dst_w,dst_h);
        ...
        fImageIndex->reserve_bitmap = whole_bm;
        fImageIndex->sample_num = sampleSize;    	
       ...
       if ((kAlpha_8_SkColorType == config || kIndex_8_SkColorType == config)
        && 1 == sampleSize) {
        // A8 is only allowed if the original was GRAY.
        SkASSERT(config != kAlpha_8_SkColorType
                 || PNG_COLOR_TYPE_GRAY == colorType);
        for (int i = 0; i < number_passes; i++) {
            for (png_uint_32 y = 0; y < origHeight; y++) {
                uint8_t* bmRow = bitmap->getAddr8(0, y);
                png_read_rows(png_ptr, &bmRow, png_bytepp_NULL, 1);
            }
        }
    } 
    		...
    		SkScaledBitmapSampler::SrcConfig sc;
      		int srcBytesPerPixel = 4;
       	    ...
       	    if (colorTable != NULL) {
                sc = SkScaledBitmapSampler::kIndex;
                srcBytesPerPixel = 1;
        	} else if (kAlpha_8_SkColorType == config) {
            // A8 is only allowed if the original was GRAY.
         ...
         */
        SkAutoLockColors ctLock(colorTable);
        if (!sampler.begin(bitmap, sc, *this, ctLock.colors())) {
            return false;
        }
       ...
       //SkDebugf("height : %d",height);
        ...
        if (number_passes > 1) {
           ...
           //SkDebugf("sampler.srcDY()  : %d",sampler.srcDY() );
        ...
        if (0 != theTranspColor) {
            reallyHasAlpha |= substituteTranspColor(bitmap, theTranspColor);
        }
        
       if (reallyHasAlpha && this->getRequireUnpremultipliedColors() &&
            kN32_SkColorType != bitmap->colorType()) {
    ...
    if(image_enhance_func(bitmap->width(), bitmap->height(), config, whole_bm))
    {
        ...
        SkDebugf("image_enhance failed at %d...",__LINE__);   
            ...
            int filelen = 0; 
            int w,h;
            uint8_t *enhance_src = NULL;    
            enhance_src = (unsigned char *)bitmap->getPixels();
            w = bitmap->width();
            h = bitmap->height();
            SkDebugf("w:%d h:%d",w,h);           
            filelen =  (w*h)<<shift;
            fp = fopen("/data/bitmap.bin","wb");
            if(fp)      
                fwrite(enhance_src,1,filelen,fp);
            fclose(fp);
    ...
    else
    {
        ...
        fImageIndex->build_index = true;
    }
   fImageIndex->fPng_ptr = png_ptr;
   fImageIndex->fInfo_ptr = info_ptr;
...
bool SkPNGImageDecoder::onDecodeSubset(SkBitmap* bm, const SkIRect& region) {
...
#ifndef ANDROID_DEFAULT_CODE	
    if(fImageIndex->build_index == false)
        colorType  = fImageIndex->fColorType;
#endif /*ANDROID_DEFAULT_CODE*/
    decodedBitmap.setInfo(SkImageInfo::Make(sampler.scaledWidth(), sampler.scaledHeight(),
    ...
    */
	...
	{
		        /*fill the region bm*/      
        int bm_w = decodedBitmap.width();
        int bm_h = decodedBitmap.height();        
        int temp_size = fImageIndex->sample_num;        
        int srcw = fImageIndex->bitmap->width();
        int srch = (bm_h*sampleSize)/temp_size;
        int offset_y = actualTop/temp_size;           
        int const src_stride = fImageIndex->bitmap->rowBytes();
        uint8_t* src = (uint8_t*)fImageIndex->bitmap->getPixels();
        uint8_t* bmRow = (uint8_t*)decodedBitmap.getPixels();  
       // SkDebugf("==build index sucess\n");

        scale_param_t scale_dest;
        scale_param_t scale_src;
        
        scale_src.addr = src + offset_y*src_stride;
        scale_src.stride = src_stride;
        scale_src.width = srcw;
        scale_src.height = srch;
        scale_dest.addr =  bmRow;
