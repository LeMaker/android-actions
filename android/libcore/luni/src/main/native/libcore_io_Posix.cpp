...
static jobjectArray Posix_android_getaddrinfo(JNIEnv* env, jobject, jstring javaNode,
    ...
    if (rc != 0) {
#ifndef BUILD_HOST
		char stripv6[PROPERTY_VALUE_MAX];
		memset(stripv6, 0, sizeof(stripv6));
		property_get("ro.net.config", stripv6, "0");
		if(stripv6[0] == '0' && !strcmp(node.c_str(), "ipv6.google.com")) {
			//ALOGE("Posix_getaddrinfo: ipv6.google.com always return a valid address");
			jobjectArray result = env->NewObjectArray(1, JniConstants::inetAddressClass, NULL);
			if (result == NULL) {
				return NULL;
			}
			sockaddr_in6 ip6;
			...
			ip6.sin6_port = 0;
			ip6.sin6_flowinfo =0;
			ip6.sin6_addr.s6_addr16[0] = 1060;
			ip6.sin6_addr.s6_addr16[1] = 104;
			ip6.sin6_addr.s6_addr16[2] = 1344;
			ip6.sin6_addr.s6_addr16[3] = 520;
			ip6.sin6_addr.s6_addr16[4] = 0;
			ip6.sin6_addr.s6_addr16[5] = 0;
			ip6.sin6_addr.s6_addr16[6] = 0;
			ip6.sin6_addr.s6_addr16[7] = 2064;
			ip6.sin6_scope_id = 0;

			sockaddr_storage& address = *((sockaddr_storage*) &ip6);
			ScopedLocalRef<jobject> inetAddress(env, sockaddrToInetAddress(env, address, NULL));
			if (inetAddress.get() == NULL) {
				return NULL;
			}
			env->SetObjectArrayElement(result, 0, inetAddress.get());
			return result;
		} else
#endif
		{
