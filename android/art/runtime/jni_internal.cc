...
#include <utility>
#include <vector>
#if defined(TARGET_BUILD_ACTIONS)
...
extern "C" void loadNativeLibraryProfile(void* handle, const char *path)__attribute__((weak));
...
static const size_t kLocalsMax = 768;  // Arbitrary sanity check.
...
bool JavaVMExt::LoadNativeLibrary(const std::string& path,
  ...
  // actions_code(ywwang, profile start)
  if (loadNativeLibraryProfile && library && !library->GetPath().empty()) {
    loadNativeLibraryProfile(handle, library->GetPath().c_str());
  }
  // actions_code(ywwang, profile end)
