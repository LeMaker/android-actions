...
bool ClassLinker::LoadSuperAndInterfaces(Handle<mirror::Class> klass, const DexFile& dex_file) {
  ...
  if (super_class_idx != DexFile::kDexNoIndex16) {
    ...
    if (super_class == nullptr) {
      DCHECK(Thread::Current()->IsExceptionPending());
        // actions_code(ywwang 1: BUG00236743, todo, move into a method)
        Thread* const self = Thread::Current();
        StackHandleScope<2> hs(self);
        Handle<mirror::ClassLoader> class_loader(hs.NewHandle(klass.Get()->GetClassLoader()));
        const char* descriptor = dex_file.StringByTypeIdx(super_class_idx);
        const size_t hash = ComputeModifiedUtf8Hash(descriptor);
        const DexFile::ClassDef* dex_class_def = dex_file.FindClassDef(descriptor, hash);
        const char *location = dex_file.GetLocation().c_str();
        if (dex_class_def != nullptr && location && strncmp(location, "/data/dalvik-cache", 18)) {
            self->ClearException();
            super_class =
                    DefineClass(self, descriptor, hash, class_loader, dex_file, *dex_class_def);
            LOG(WARNING) << "dex_file " << __LINE__ << "   resolved= " << descriptor;
        }
        if (super_class == nullptr) {
          DCHECK(Thread::Current()->IsExceptionPending());
          return false;
  ...
  if (interfaces != nullptr) {
    for (size_t i = 0; i < interfaces->Size(); i++) {
      ...
      if (interface == nullptr) {
        DCHECK(Thread::Current()->IsExceptionPending());
        // actions_code(ywwang 2: todo, move into a method)
        Thread* const self = Thread::Current();
        StackHandleScope<2> hs(self);
        Handle<mirror::ClassLoader> class_loader(hs.NewHandle(klass.Get()->GetClassLoader()));
        const char* descriptor = dex_file.StringByTypeIdx(idx);
        const size_t hash = ComputeModifiedUtf8Hash(descriptor);
        const DexFile::ClassDef* dex_class_def = dex_file.FindClassDef(descriptor, hash);
        if (dex_class_def != nullptr) {
            self->ClearException();
            interface =
                    DefineClass(self, descriptor, hash, class_loader, dex_file, *dex_class_def);
            LOG(WARNING) << " dex_file " << __LINE__ << "   resolved= " << interface;
        }
        if (interface == nullptr) {
          DCHECK(Thread::Current()->IsExceptionPending());
          return false;
