...
import com.actions.ActionsConfig;
...
public class AudioService extends IAudioService.Stub {
   ...
   /** @hide Maximum volume index values for audio streams */
    ...
    //ActionsCode(author:yuchen, change_code): remove HDMI from fixed volume devices
    // Devices for which the volume is fixed and VolumePanel slider should be disabled    
    int mFixedVolumeDevices = ActionsConfig.ACTIONS_FEATURE_HDMI_ALLOW_VOLUME_ADJUSTING == 1?
    ...
    /** @hide */
    /**
     *NEW_FEATURE: Add configuration to enable/disable different devices using the same volume.
     *
     *NEW_FEATURE: Add safe media time configurations and warning mechanics 
     *to meet european standard.     
     ************************************
     *      
	...
	//ActionsCode(author:yuchen, change_code): add safe media time configuration
        mSafeMediaTime = mContext.getResources().getInteger(
                com.android.internal.R.integer.config_safe_media_time) * 3600 * 1000;
        mSafeMediaTimeWarningEnabled = mContext.getResources().getBoolean(
                com.android.internal.R.bool.config_safe_media_time_enabled);



	//ActionsCode(author:yuchen, change_code): add same media volume configuration
	if(ActionsConfig.ACTIONS_FEATURE_SAME_VOLUME_CONFIGURABLE > 0)
	{
		mSameMediaVolumeState = SystemProperties.getInt("ro.audio.samevolume", 0);
	}
    ...
    private void adjustStreamVolume(int streamType, int direction, int flags,
        ...
        //ActionsCode(author:yuchen, change_code): if same volume feature is enabled, 
        //enforce all devices apply the same volume
        if(ActionsConfig.ACTIONS_FEATURE_SAME_VOLUME_CONFIGURABLE > 0){
            if (mSameMediaVolumeState != 0) {
    	        if ((streamTypeAlias == AudioSystem.STREAM_MUSIC)) {
    		    enforceSameMediaVolume(oldIndex);
    		}
    	    }
    	}
    ...
    private void setStreamVolume(int streamType, int index, int flags, String callingPackage,
        ...
        synchronized (mSafeMediaVolumeState) {
            ...
            //ActionsCode(author:yuchen, change_code): if same volume is enabled,
            //enforce volume setting on all devices
            if(ActionsConfig.ACTIONS_FEATURE_SAME_VOLUME_CONFIGURABLE > 0){
                if (mSameMediaVolumeState != 0) {
                    if ((streamTypeAlias == AudioSystem.STREAM_MUSIC)) {
                        enforceSameMediaVolume(index);
                    }
                }
            }
            ...
            if (mSafeMediaVolumeState == SAFE_MEDIA_VOLUME_INACTIVE) {
                ...
                if ((device & mSafeMediaVolumeDevices) != 0) {
                    ...
                    if (AudioSystem.isStreamActive(AudioSystem.STREAM_MUSIC, 0) &&
                        ...
                        //ActionsCode(author:yuchen, change_code): use safe media dialog warning if feature enabled.
                        if (ActionsConfig.ACTIONS_FEATURE_SAFE_MEDIA_TIME_CONFIG_ON > 0) {                 
                            if (mMusicActiveMs > mSafeMediaTime)
                            {
                                Log.d(TAG, "onCheckMusicActive  mMusicActiveMs > " + mSafeMediaTime);
                                setSafeMediaVolumeEnabled(true);
                                if(mSafeMediaTimeWarningEnabled) {
                                    int flags = AudioManager.FLAG_SHOW_UI;
                                    mPendingVolumeCommand = new StreamVolumeCommand(AudioSystem.STREAM_MUSIC, 
                                                                              index, flags, device);                            
                                    mVolumeController.postDisplaySafeMediaTimeWarning(flags);
                                }
                                mMusicActiveMs = 0;
                            }
                        }
    ...
    /**
     *NEW_FEATURE: Stop music playing when usb accessory plugs in to prevent music data 
     *             not able to transfer to usb accessory..
     ************************************
     *      
     *ActionsCode(author:yuchen, change_code)
     */    
	...
	//ActionsCode(author:yuchen, change_code):
	// purpose: stop music playing when usb accessory plug in.
        // when music playing, and switch to usb accessory, music data can't transfer to usb.
	// just for usb accessory, cause to some weird reasons.
	if(ActionsConfig.ACTIONS_FEATURE_STOP_PLAY_WHEN_USB_ACCESSARY_PLUGGED > 0)
	{
            if ((state == 1) && (device == AudioSystem.DEVICE_OUT_USB_ACCESSORY)) {
                    sendMsg(mAudioHandler,
                            MSG_BROADCAST_AUDIO_BECOMING_NOISY,
                            SENDMSG_REPLACE,
                            0,
     ...
     ************************************
     *      
     *ActionsCode(author:yuchen, new_code): add 2 var mSafeMediaTime and mSafeMediaTimeWarningEnabled
     * to check configuration
     */                     
    private int mSafeMediaTime;
    private boolean mSafeMediaTimeWarningEnabled;    
    ...
    /**
     *NEW_FEATURE: Add configuration to enable/disable different devices using the same volume.
     * mSameMediaVolumeState record whether same volume feature is enabled
     ************************************
     *      
     *ActionsCode(author:yuchen, new_code)
     */
    private static int mSameMediaVolumeState;
    

    /**
     *NEW_FEATURE: Add configuration to enable/disable different devices using the same volume.
     * mSameMediaVolumeDevices lists the devices for which media volume index is the same
     ************************************
     *      
     *ActionsCode(author:yuchen, new_code)
     */
    private final int mSameMediaVolumeDevices = AudioSystem.DEVICE_OUT_WIRED_HEADSET |
                                                AudioSystem.DEVICE_OUT_WIRED_HEADPHONE |
     ...
     *ActionsCode(author:yuchen, new_code)
     */
    private void enforceSameMediaVolume(int index) {
        VolumeStreamState streamState = mStreamStates[AudioSystem.STREAM_MUSIC];
        int devices = mSameMediaVolumeDevices;
        int i = 0;
        ...
        while (devices != 0) {
            ...
            streamState.setIndex(index, device);
            sendMsg(mAudioHandler,
                    MSG_SET_DEVICE_VOLUME,
                    SENDMSG_QUEUE,
                    device,
     ...
     ************************************
     *      
     *ActionsCode(author:yuchen, change_code): add displaySafeMediaTimeWarning function;
     */                     
    public static class VolumeController {
         ...
         *ActionsCode(author:yuchen, new_code)
         */                     
        public void postDisplaySafeMediaTimeWarning(int flags) {
            if (mController == null)
                return;
            try {
                mController.displaySafeMediaTimeWarning(flags);
            } catch (RemoteException e) {
                Log.w(TAG, "Error calling displaySafeMediaTimeWarning", e);
            }
        }        
