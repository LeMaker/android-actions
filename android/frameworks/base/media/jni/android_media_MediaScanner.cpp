...
public:
            ...
            long long fileSize, bool isDirectory, bool noMedia)
    {
        ...
        // actions_code(lishiyuan, bugfix:fix BUG00235865. 
        // when file by scanning is not exist, mediascanner stop scanning.)
        FILE * fp = NULL;
        if (!isDirectory && fileSize > 0){
            if ((fp = fopen(path, "r")) == NULL)
            {
                ALOGW("cannot fopen'%s', skipping: %s.", path, strerror(errno));
                return UNKNOWN_ERROR;
            }else{
                char buf[20];
                if (fread(buf, 1,1,fp) != 1 )
                {
                    ALOGW("fread '%s' err , skipping: %s.", path, strerror(errno));
                    fclose(fp);
                    return UNKNOWN_ERROR;
                }
                fclose(fp);
