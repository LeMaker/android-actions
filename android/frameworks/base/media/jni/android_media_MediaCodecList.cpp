...
static jobject android_media_MediaCodecList_getCodecCapabilities(
    ...
    for (size_t i = 0; i < profileLevels.size(); ++i) {
        ...
        //actionsCode:wh hack for cts EncodeVirtualDisplayWithCompositionTest
        if(strcmp(get_process_name(),"com.android.cts.media") == 0){     
	        if(src.mLevel > 0x100 && isEncoder){
	        	env->SetIntField(profileLevelObj, levelField, 0x100);
	        	ALOGE("com.android.cts.media src.mProfile, %d src.mLevel %d --change to 0x100 ",src.mProfile,src.mLevel);
	        }
	    }
