...
public final class PowerManagerService extends SystemService
    ...
    // ActionsCode(authro:songzhining, comment: fix BUG00233114)
    private final AtomicBoolean mSystemReadyDone = new AtomicBoolean(false);
    ...
    //ActionsCode(author:wh, comment:new feature onoff close primary display device only )
    private static final int HDMI_ACTIVITED_FLAG = 0x01;
    private static final int WFD_ACTIVITED_FLAG = 0x02;
    private int mScreenBrightnessOverrideFromActionsOnOff = -1;
    private int mSecondDispActivted = 0;
    private int mHDMIStatus = 0;
    private boolean mAutoOnOffMode = false;
    private boolean isScreenOn = true;
    private boolean isPendingForHdmi = false;
    private static int isNeedCallUpdatePowerState = 0;
        ...
        public void onReceive(Context context, Intent intent) {
            synchronized (mLock) {
                if (WindowManagerPolicy.ACTION_HDMI_PLUGGED.equals(intent.getAction())) {
                    if(intent.getBooleanExtra(WindowManagerPolicy.EXTRA_HDMI_PLUGGED_STATE, false)){
                        mHDMIStatus = 1;
                    }else{
                        mHDMIStatus = 0;
                    }
                    if(mHDMIStatus == 0 && !isScreenOn){
                        checkActionsDisplayAndUpdateStatus(PowerManager.FULL_WAKE_LOCK);
                    }else{
                    ...
                    Slog.i(TAG, "receive HDMI_PLUGGED broadcast,plug flag = !!!!!!!!~~~~~~~~~~~" + mSecondDispActivted);                   
                }
    
            }
        }
    }    
    private void initActionsOnoffMode() {
        if (SystemProperties.get("ro.hdmi.onoffmode", "alwayson").equals("auto")) {
            mAutoOnOffMode = true;
        }
        if(!mAutoOnOffMode){
    ...
    private boolean checkActionsDisplayAndUpdateStatus(int reason) {
    	boolean result = false;
    	
        if (mAutoOnOffMode) {
            mSecondDispActivted = 0;
            return true; 
        } else {            
            if((mDisplayManager.getWifiDisplayStatus().getActiveDisplayState()== WifiDisplayStatus.DISPLAY_STATE_CONNECTED))
            {
                mSecondDispActivted |= WFD_ACTIVITED_FLAG;
            }else{
                mSecondDispActivted &= ~WFD_ACTIVITED_FLAG;
            }
            if(mHDMIStatus != 0){
                mSecondDispActivted |= HDMI_ACTIVITED_FLAG;
            }else{
                mSecondDispActivted &= ~HDMI_ACTIVITED_FLAG;
            }          
        } 
        isNeedCallUpdatePowerState ++; 
        if(isNeedCallUpdatePowerState >= 2){
        	return result;
        }
        //if there hdmi or wifi display actived ,we only close bright ness.
        if(mSecondDispActivted != 0 || isPendingForHdmi){
            if(isScreenOn){ 	
                
                mScreenBrightnessOverrideFromActionsOnOff = PowerManager.BRIGHTNESS_OFF;
                ...
                isPendingForHdmi = true;              
                
            }else if(reason != PowerManager.GO_TO_SLEEP_REASON_TIMEOUT){
                
                mScreenBrightnessOverrideFromActionsOnOff = -1;
                
                mDirty |= DIRTY_SETTINGS | DIRTY_WAKEFULNESS | DIRTY_USER_ACTIVITY;
                ...
                isPendingForHdmi = false;  
                
            }          
        }else{
            result = true;
        }
        isNeedCallUpdatePowerState = 0; 
        return result;
    }
    ...
    public void systemReady(IAppOpsService appOps) {
        synchronized (mLock) {
            ...
            //ActionsCode(author:wh, NEW_FEATURE:add hdmi onoff mode ,only close backlight)
            if(ActionsConfig.ACTIONS_FEATURE_HDMI_ONOFF_DEFAULT_ON > 0){
                initActionsOnoffMode();
            }
    ...
    private boolean goToSleepNoUpdateLocked(long eventTime, int reason, int flags, int uid) {
        ...
        //ActionsCode(author:wh, NEW_FEATURE:add hdmi onoff mode ,only close backlight)
        if(ActionsConfig.ACTIONS_FEATURE_HDMI_ONOFF_DEFAULT_ON > 0){
	        if(!checkActionsDisplayAndUpdateStatus(reason)){
	             return false;
	        }
    	}
    ...
    private boolean updateDisplayPowerStateLocked(int dirty) {
        ...
        if ((dirty & (DIRTY_WAKE_LOCKS | DIRTY_USER_ACTIVITY | DIRTY_WAKEFULNESS
            ...
            //ActionsCode(author:wh, NEW_FEATURE:add hdmi onoff mode ,only close backlight)
            if(ActionsConfig.ACTIONS_FEATURE_HDMI_ONOFF_DEFAULT_ON > 0 && isValidBrightness(mScreenBrightnessOverrideFromActionsOnOff)){
                screenBrightness = mScreenBrightnessOverrideFromActionsOnOff;
            ...
            if(ActionsConfig.ACTIONS_FEATURE_HDMI_ONOFF_DEFAULT_ON == 0 || !isValidBrightness(mScreenBrightnessOverrideFromActionsOnOff)){
                ...
                Slog.d(TAG, "updateDisplayPowerStateLocked  mScreenBrightnessOverrideFromActionsOnOff screenBrightness" + screenBrightness );
            }
    ...
    private final class BinderService extends IPowerManager.Stub {
        ...
        @Override // Binder call
        public int getWakelockSize(String flags) {
            synchronized (mLock) {
                int WakelockSize = mWakeLocks.size();
                if(WakelockSize > 0 && flags.equals("wifi")) {
                    for (WakeLock wl : mWakeLocks) {
                        Slog.w(TAG, "wakelock : " + wl.mTag);
                        if(wl.mTag.equals("WifiStarted") ||
                           wl.mTag.equals("Checkin Service") ||
                           wl.mTag.equals("*alarm*")) {
                            WakelockSize--;
                        }
                    }
                }
