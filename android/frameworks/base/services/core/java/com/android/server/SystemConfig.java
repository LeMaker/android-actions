...
public class SystemConfig {
    ...
    final ArrayMap<String, FeatureInfo> mExtraFeatures = new ArrayMap<>();
    ...
    // ActionsCode(songzhining, new code: add extra hardware feature support)
    public ArrayMap<String, FeatureInfo> getExtraFeatures() {
        return mExtraFeatures;
    }
    ...
    SystemConfig() {
        ...
        // ActionsCode(songzhining, new code: add extra hardware feature support)
        readPermissions(Environment.buildPath(
                Environment.getRootDirectory(), "etc", "permissions", "extras"), false, true);
        ...
        // ActionsCode(songzhining, new code: fix cts feature bug, testNoManagedUsersIfLowRamDevice)
        if (ActivityManager.isLowRamDeviceStatic()) {
            mAvailableFeatures.remove(PackageManager.FEATURE_MANAGED_USERS);
        }
    }

    // ActionsCode(songzhining, new code: add extra hardware feature support)
    private void readPermissions(File libraryDir, boolean onlyFeatures) {
        readPermissions(libraryDir, onlyFeatures, false);
    }

    void readPermissions(File libraryDir, boolean onlyFeatures, boolean extra) {
        ...
        for (File f : libraryDir.listFiles()) {
            ...
            readPermissionsFromXml(f, onlyFeatures, extra);
        }

        // Read platform permissions last so it will take precedence
        if (platformFile != null) {
            readPermissionsFromXml(platformFile, onlyFeatures, extra);
        }
    }

    private void readPermissionsFromXml(File permFile, boolean onlyFeatures, boolean extra) {
