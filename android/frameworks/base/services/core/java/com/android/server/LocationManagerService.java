...
public class LocationManagerService extends ILocationManager.Stub {
    ...
    //ActionsCode(authro:songzhining, comment: new code)
    private boolean supportgpswithwifi;
    ...
    private void loadProvidersLocked() {
        ...
        */
        ...
        //fixed latest google map can not location problem
        if (!GpsLocationProvider.isSupported() &&
            mContext.getPackageManager().hasSystemFeature(PackageManager.FEATURE_LOCATION_GPS)) {
            LocationProviderProxy vgpsProvider = LocationProviderProxy.createAndBind(
                mContext,
                ...
                com.android.internal.R.array.config_locationProviderPackageNames,
                mLocationHandler);
            if (vgpsProvider != null) {
                mRealProviders.put(LocationManager.GPS_PROVIDER, vgpsProvider);
                mProxyProviders.add(vgpsProvider);
                addProviderLocked(vgpsProvider);
            } else {
                Slog.w(TAG,  "fail to create virtual gps location provider");
            }
        }
