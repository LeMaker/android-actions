...
public final class CompatModePackages {
    ...
    /**
     *actions_code(ywwang, change_method)
     */
    public CompatModePackages(ActivityManagerService service, File systemDir, Handler handler) {
		...
		restoreCompatModes(mFile);
        //actions_code(ywwang, ACTIONS_OPT_COMPAT_PACKAGE)
        //add default compat-mode for some apks to improve user experience
         com.actions.hooker.CompatModePackagesCaller.restoreCompatModes(this);
        File rootDir = Environment.getRootDirectory();
        File etcDir = new File(rootDir, "etc");
        AtomicFile defFile = new AtomicFile(new File(etcDir, "packages-compat-default.xml"));
        restoreCompatModes(defFile);
		}

    /**
      *  actions_code(ywwang, new_method)
      *@hide 
      */
    public void restoreCompatModes(AtomicFile file) {
