...
public final class ActiveServices {
    ...
    public ActiveServices(ActivityManagerService service) {
        ...
        // ActionsCode(authro:songzhining, comment: increase bg num to 3 for low ram device)
        mMaxStartingBackground = maxBg > 0
                ? maxBg : ActivityManager.isLowRamDeviceStatic() ? 3 : 8;
    }
    ...
    final void killServicesLocked(ProcessRecord app, boolean allowRestart) {
        ...
        for (int i=app.services.size()-1; i>=0; i--) {
            ...
            //actions_code(ywwang, backgroud app control)
            boolean blockRestart = false;
             if(mAm.mAsp != null)
                 blockRestart = mAm.mAsp.shouldBlock(sr.packageName);
            if(blockRestart){
                Slog.w(TAG, "Block start service =  " +sr);
                allowRestart=false;
            }
