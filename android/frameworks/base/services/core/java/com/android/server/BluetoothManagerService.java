...
class BluetoothManagerService extends IBluetoothManager.Stub {
    ...
    private int yEnable = BluetoothAdapter.STATE_OFF;
    private int mState;
    private HandlerThread mThread;
    private final BluetoothHandler mHandler;
    public static final int POLL_PERIOD       =  5000;
    public static final int ENABLE_TRAFFIC_STATS_POLL_BT       =  71;
    /** @hide */
    public static final int TRAFFIC_STATS_POLL_BT              =  72;
    public static final int AIRPLANE_MODE_CHANGED              =  73;
    private int mErrorRecoveryRetryCounter;
	private int mSleepCount = 0;
	private boolean mScreenOff = false;
	private boolean mSaveState = false;
	private boolean mAirplaneModeChange = false;
    ...
    private PowerManager.WakeLock  mBtLock;
    ...
    private class AsyncServiceHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case ENABLE_TRAFFIC_STATS_POLL_BT: {
                    Log.w(TAG, "rev ENABLE_TRAFFIC_STATS_POLL_BT");
                    mSleepCount = 0;
                    if (msg.arg1 == 1) {
                        if (!mScreenOff) {
                            setBlueToothLock(false);
                            Log.w(TAG, "rev ENABLE_TRAFFIC_STATS_POLL_BT111");
                            if (mSaveState) {
                                mSaveState = false;
                                if(!isEnabled()) {
                                    Log.w(TAG, "ENABLE_BT");
                        ...
                        mTxBytes = mRxBytes = 0;
                        sendMessageDelayed(Message.obtain(this, TRAFFIC_STATS_POLL_BT,
                                0, 0), POLL_PERIOD);                    
                    }
                    break;
                }
                case TRAFFIC_STATS_POLL_BT: {
                    removeMessages(TRAFFIC_STATS_POLL_BT);
                    if (mScreenOff) {
                        if (isEnabled()) {
                            setBlueToothLock(true);
                            if (!isBtTransfering()) {
                                mSleepCount++;
                                Log.i(TAG, "bt locktime add:"+mSleepCount);
                            } else {
                                mSleepCount=0;
                            }
                            if (mSleepCount == 10) {
                                Log.i(TAG, "bt locktime on");
                                if(isEnabled()) {
                                    mSaveState = true;
                                    Log.w(TAG, "Disable BT");
                                    disable(false);
                ...
                case AIRPLANE_MODE_CHANGED: {
                    Log.e(TAG, "---AIRPLANE MODE CHANGED---");
                    yEnable = BluetoothAdapter.STATE_OFF;
                    synchronized(mConnection) {
                        try {
                            if(mBluetooth != null) {
                                yEnable = mBluetooth.getState();
                            }
                        } catch (RemoteException e) {
                            Log.e(TAG, "getState()", e);
                        }
                    }
                    if(yEnable != BluetoothAdapter.STATE_ON && yEnable != BluetoothAdapter.STATE_OFF) {
                        sendMessageDelayed(Message.obtain(this, AIRPLANE_MODE_CHANGED, 0, 0), POLL_PERIOD);
                    } else {
                        synchronized(mReceiver) {
                            mAirplaneModeChange = false;
    ...
    private boolean isBtTransfering()    
	{
        int tx = 0;
		int rx = 0;
        if (mBluetooth != null) {
            ...
            try {
				rx = mBluetooth.getRecvByte();
				tx = mBluetooth.getSendByte();
            } catch (RemoteException e) {
                Log.e(TAG,"Unable to call getbyte()",e);
            }
        }		
		boolean b = false;
		Log.i(TAG, "Bluetooth trans:"+rx+"-"+tx+"-prev:"+mRxBytes+"-"+mTxBytes);
		if (rx <= mRxBytes && tx <= mTxBytes)
		{
			b = false;    	  	      	  
		}
		else
    ...
    private final BroadcastReceiver mReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (BluetoothAdapter.ACTION_LOCAL_NAME_CHANGED.equals(action)) {
                ...
                synchronized(mReceiver) {
                    if(mAirplaneModeChange == false) {
                        mAirplaneModeChange = true;
                        Message.obtain(mAsyncServiceHandler,
                        AIRPLANE_MODE_CHANGED, 0, 0).sendToTarget();
                    }
                    /*if (isAirplaneModeOn()) {
			...
			} else if (action.equals(Intent.ACTION_BATTERY_CHANGED)) 
			{
				...
				* the already-set timer.                 */
				...
				if (DBG) {
					Log.i(TAG, "ACTION_BATTERY_CHANGED pluggedType: " + pluggedType); 
				}
    ...
    private void evaluateTrafficStatsPolling() {
        ...
        {		    
		        ...
		        msg = Message.obtain(mAsyncServiceHandler,
	                	ENABLE_TRAFFIC_STATS_POLL_BT, 0, 0);  
		...
		else
		{
			  setBlueToothLock(false);
				...
				return;
	           // msg = Message.obtain(mAsyncServiceHandler,
	           //            ENABLE_TRAFFIC_STATS_POLL_BT, 1, 0);             
    ...
    BluetoothManagerService(Context context) {
        ...
        HandlerThread BtThread = new HandlerThread("BtService");
        BtThread.start();
        mAsyncServiceHandler = new AsyncServiceHandler(BtThread.getLooper());
        PowerManager pm = (PowerManager)context.getSystemService(Context.POWER_SERVICE);   		
        mBtLock = pm.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "BTLock");	
	    mBtLock.setReferenceCounted(false);
	    {
    ...
    private void setBlueToothLock(boolean b)
    {
    	  if (mBtLock==null)
    	  {
    	  	  return;
    ...
    public boolean enable() {
        ...
        synchronized(mConnection) {
            try {
                if(mBluetooth != null && mBluetooth.getState() != BluetoothAdapter.STATE_OFF)
                    return false;
            } catch (RemoteException e) {
                Log.e(TAG, "getState()", e);
            }
