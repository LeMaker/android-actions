...
static const char* kSystemAssetsDirs[] = {"images","sounds","webkit"};
...
AssetDir* AssetManager::openDir(const char* dirName)
{
    ...
    while (i > 0) {
        ...
        if (ap.type == kFileTypeRegular) {
            ...
            //ActionsCode(authro:songzhining, comment: if dir dismatch in framework-res.apk, just ignore it)
            int matched = 0;
            const char *sysPath = strchr(ap.path.string() + 1, '/') + 1;
            if (strcmp(sysPath, kSystemAssets) == 0) {
                size_t j = sizeof(kSystemAssetsDirs) / sizeof(kSystemAssetsDirs[0]);
                while (j > 0) {
                    j--;
                    if (strcmp(dirName, kSystemAssetsDirs[j]) == 0) {
                        matched = 1;
                        break;
                    }
                }
