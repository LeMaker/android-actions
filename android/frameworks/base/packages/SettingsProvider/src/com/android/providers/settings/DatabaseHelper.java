...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    private void loadSecureSettings(SQLiteDatabase db) {
        ...
        try {
            ...
            /*      
             * ActionsCode(author:songzhining, type:change_code)
             *   (1) 512M: simple pinyin IME
             *   (2) others: google pinyin IME
             */
            if(SystemProperties.get("ro.product.locale.language", "en").equals("zh")){
                //hack for IME
                int sdramcap = SystemProperties.getInt("system.ram.total", 1024);
                String imeService;

                if(sdramcap <= 512) {
                    imeService = "com.android.inputmethod.pinyin/.PinyinIME";
                } else {
                    imeService = "com.google.android.inputmethod.pinyin/.PinyinIME";
                }

                loadSetting(stmt, Settings.Secure.DEFAULT_INPUT_METHOD, imeService);
            }
        } finally {
            if (stmt != null) stmt.close();
        }
    ...
    */
    private void loadSecure35Settings(SQLiteStatement stmt) {
        ...
        //ActionsCode(phchen, BUGFIX: BUG00236395 ),
        String transport = SystemProperties.get("ro.settings.backuptransport", "com.google.android.backup/.BackupTransportService");
        if(transport.equals("")){
        loadStringSetting(stmt, Settings.Secure.BACKUP_TRANSPORT,
                R.string.def_backup_transport);
        }else{
            loadSetting(stmt, Settings.Secure.BACKUP_TRANSPORT, transport);
        }
    }
    ...
    private void loadGlobalSettings(SQLiteDatabase db) {
        ...
        try {
            ...
            //ActionsCode(songzhining, BUGFIX: add config for captive_portal_detection)
            loadSetting(stmt, Settings.Global.CAPTIVE_PORTAL_DETECTION_ENABLED,
                    SystemProperties.get("ro.wifi.captive_portal", "1"));
        } finally {
            if (stmt != null) stmt.close();
        }
    }
