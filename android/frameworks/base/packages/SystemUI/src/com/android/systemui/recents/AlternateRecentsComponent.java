...
public class AlternateRecentsComponent implements ActivityOptions.OnAnimationStartedListener {
    ...
    *
    *
    *ActionsCode(liuxinxu, change_code: use system uptime to record toggle recent time)
    */
    /** Toggles the recents activity */
    void toggleRecentsActivity() {
        ...
        //ActionsCode(liuxinxu, workaround: use system uptime to record toggle recent time),
        if (SystemClock.uptimeMillis() - mLastToggleTime < sMinToggleDelay) {
            return;
        }
        ...
        if (topTask != null && mSystemServicesProxy.isRecentsTopMost(topTask, isTopTaskHome)) {
            ...
            mContext.sendBroadcastAsUser(intent, UserHandle.CURRENT);
            mLastToggleTime = SystemClock.uptimeMillis();
            return;
        } else {
