...
public class VolumePanel extends Handler implements DemoMode {
     ...
     */             
    private static final int MSG_DISPLAY_SAFE_MEDIA_TIME_WARNING = 17;
     ...
     */            
    private static AlertDialog sMediaTimeSafetyWarning;
     ...
     *ActionsCode(author:yuchen, new_code)
     */                
    private static class MediaTimeSafetyWarning extends SystemUIDialog
            implements DialogInterface.OnDismissListener, DialogInterface.OnClickListener {
        private final Context mContext;
        private final VolumePanel mVolumePanel;
        private final AudioManager mAudioManager;
        ...
        MediaTimeSafetyWarning(Context context, VolumePanel volumePanel, AudioManager audioManager) {
            super(context);
            mContext = context;
            mVolumePanel = volumePanel;
            mAudioManager = audioManager;

            setMessage(mContext.getString(com.android.internal.R.string.safe_media_time_warning));
            setButton(DialogInterface.BUTTON_POSITIVE,
                    mContext.getString(com.android.internal.R.string.yes), this);
            setButton(DialogInterface.BUTTON_NEGATIVE,
                    mContext.getString(com.android.internal.R.string.no), (OnClickListener) null);
        ...
        mDialog = new Dialog(context) {
            ...
            public boolean onTouchEvent(MotionEvent event) {
                if (isShowing() && event.getAction() == MotionEvent.ACTION_OUTSIDE &&
                        sSafetyWarning == null && sMediaTimeSafetyWarning == null) {
                    forceTimeout(0);
                    return true;
                }
                return false;
    ...
    private void updateSliderEnabled(final StreamControl sc, boolean muted, boolean fixedVolume) {
        ...
        if (sc.streamType == STREAM_REMOTE_MUSIC) {
            ...
            sc.icon.setClickable(false);
                ...
                (sSafetyWarning != null) || (sMediaTimeSafetyWarning != null)) {
            //ActionsCode(author:yuchen, change_code): check sMediaTimeSafetyWarning too;
            sc.seekbarView.setEnabled(false);
        } else {
     ...
     ************************************
     *      
     *ActionsCode(author:yuchen, change_code): check sMediaTimeSafetyWarning dialog too
     */             
    private void updateTimeoutDelay() {
        mTimeoutDelay = mDemoIcon != 0 ? TIMEOUT_DELAY_EXPANDED
                : (sSafetyWarning != null || sMediaTimeSafetyWarning != null) ? TIMEOUT_DELAY_SAFETY_WARNING
    ...
    public void postDisplaySafeMediaTimeWarning(int flags) {
        if (hasMessages(MSG_DISPLAY_SAFE_MEDIA_TIME_WARNING)) return;
        Log.d(TAG,"postDisplaySafeMediaTimeWarning");
        obtainMessage(MSG_DISPLAY_SAFE_MEDIA_TIME_WARNING, flags, 0).sendToTarget();
    }
     ...
     *ActionsCode(author:yuchen, new_code)
     */                
    protected void onDisplaySafeMediaTimeWarning(int flags) {
        if ((flags & (AudioManager.FLAG_SHOW_UI | AudioManager.FLAG_SHOW_UI_WARNINGS)) != 0
                || isShowing()) {
            synchronized (sSafetyWarningLock) {
                if (sMediaTimeSafetyWarning != null) {
                    return;
                }
                sMediaTimeSafetyWarning = new MediaTimeSafetyWarning(mContext, this, mAudioManager);
                sMediaTimeSafetyWarning.show();
            }
            updateStates();
        }
        if (mAccessibilityManager.isTouchExplorationEnabled()) {
    ...
    public void handleMessage(Message msg) {
	    ...
	    //ActionsCode(author:yuchen, change_code): add new message type to display
	    //safe media time warning
            case MSG_DISPLAY_SAFE_MEDIA_TIME_WARNING:
                onDisplaySafeMediaTimeWarning(msg.arg1);
		break;
    ...
    private void resetTimeout() {
        ...
        //ActionsCode(author:yuchen, change_code): check media time warning dialog too.
        if ((sSafetyWarning == null && sMediaTimeSafetyWarning == null) || !touchExploration) {
            removeMessages(MSG_TIMEOUT);
