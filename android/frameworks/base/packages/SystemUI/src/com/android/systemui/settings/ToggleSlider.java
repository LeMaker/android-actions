...
//ActionsCode(author:fanguoyong, add some import)
...
public class ToggleSlider extends RelativeLayout {
    public interface Listener {
        ...
        public void onChanged(ToggleSlider v, boolean tracking, boolean checked, int value);
		  ...
		  *
		  *NEW_FEATURE:
		  *add intelligent backlight control for statusbar
		  ************************************    
		  *ActionsCode(author:fanguoyong, add some variables for Intelligent_brightness)
		  */ 
    ...
    private boolean mTracking;
		...
		//ActionsCode(author:fanguoyong,change private to public)
    ...
    public ToggleSlider(Context context, AttributeSet attrs, int defStyle) {
        ...
        a.recycle();
			  ...
			  *add intelligent backlight control for statusbar
			  ************************************    
			  *ActionsCode(author:fanguoyong, send intent to start/close IntelligentBacklight)
			  */         
        mContext = context;
				mPerformance = new PerformanceManager();
				mIntelligentBacklightIntent = new Intent();
				mIntelligentBacklightIntent.setAction(mOpenIntelligentBacklightReceivce);
				mNotIntelligentBacklightIntent = new Intent();
				mNotIntelligentBacklightIntent.setAction(mCloseIntelligentBacklightReceivce);
    }
    ...
    public void setValue(int value) {
        ...
        if (mMirror != null) {
		  ...
		  *add intelligent backlight control for statusbar
		  ************************************    
		  *ActionsCode(author:fanguoyong, add method to start/close IntelligentBacklight)
		  */ 
    private final OnCheckedChangeListener mCheckListener = new OnCheckedChangeListener() {
        ...
        public void onCheckedChanged(CompoundButton toggle, boolean checked) {
            ...
            int pre_status_mode = Settings.System.getIntForUser(mContext.getContentResolver(), 
            				mIntelligentSettingsName, 0 , UserHandle.USER_CURRENT);
						if((pre_status_mode == 0) && checked) {
								mPerformance.enbleAutoAdjustBacklight();
								Settings.System.putIntForUser(mContext.getContentResolver(), 
										mIntelligentSettingsName, 1 , UserHandle.USER_CURRENT);
								setChecked(true);
								mContext.sendBroadcast(mIntelligentBacklightIntent);
						} else if((pre_status_mode == 1) && (!checked)) {
								mPerformance.disableAutoAdjustBacklight();
								Settings.System.putIntForUser(mContext.getContentResolver(),
										 mIntelligentSettingsName, 0 , UserHandle.USER_CURRENT);
								setChecked(false);
								mContext.sendBroadcast(mNotIntelligentBacklightIntent);
						}
