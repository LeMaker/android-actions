...
import com.android.systemui.MemoryClass;
...
class SaveImageInBackgroundTask extends AsyncTask<SaveImageInBackgroundData, Void,
	...
	//ActionsCode(author:fanguoyong,change_code):change png -> jpeg
    ...
    protected SaveImageInBackgroundData doInBackground(SaveImageInBackgroundData... params) {
        ...
        try {
            ...
            /*
			 *save picture before insert the screenshot to the MediaStore
			 *ActionsCode(author:fanguoyong,change_code)
			 */
            File f = new File(mImageFilePath);
            OutputStream out = new FileOutputStream(f);
            boolean result = image.compress(Bitmap.CompressFormat.JPEG, 100, out);
			if(!result) {            
            	Slog.e("GlobalScreenshot","compress fail");             
                out.close();              
                f.delete();              
                throw new Exception();            
            }
...
*/
    ...
    protected void onPostExecute(SaveImageInBackgroundData params) {
        ...
        if (params.result > 0) {
			...
			launchIntent.setDataAndType(params.imageUri, "image/jpeg");
...
class GlobalScreenshot {
	...
	//ActionsCode(author:fanguoyong,change_code):add mHwRotation
    ...
    public GlobalScreenshot(Context context) {
		...
		//ActionsCode(author:fanguoyong,change_code):enable FLAG_HARDWARE_ACCELERATED according Memory		
        if(!MemoryClass.isExtraLowMemDevice()){
        		mWindowLayoutParams.flags |= WindowManager.LayoutParams.FLAG_HARDWARE_ACCELERATED;
				}    
    ...
    private void startAnimation(final Runnable finisher, int w, int h, boolean statusBarVisible,
        ...
        mScreenshotLayout.post(new Runnable() {
            ...
            public void run() {
                ...
                // Try 5 times to wait for attaching to window
                for (int cnt = 0; cnt < 5; cnt ++) {
                    try {
                        mScreenshotView.buildLayer();
                        break;
                    } catch (IllegalStateException e) {
                        Slog.e("GlobalScreenshot", "This view is not attached to a window");
