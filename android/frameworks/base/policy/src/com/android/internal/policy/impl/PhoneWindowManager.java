...
import static com.actions.ActionsEnv.OPT_STARTWINDOW_RGB565;
/**
...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    public View addStartingWindow(IBinder appToken, String packageName, int theme,
        ...
        try {
           ...
           //actions_code(ywwang)
           // because starting window rendering use cpu
           // thus buffer.unlcok need to flush cache,
           // if use RGB565, buffer is smaller , so faster
           if(OPT_STARTWINDOW_RGB565){
	...
	/**
     *NEW_FEATURE: handleVolumeMute
     ************************************
     *      
     *ActionsCode(author:LauraWan, new_method)
     */
	private void handleVolumeMute() {
		...
		int stream=AudioManager.STREAM_MUSIC;//STREAM_ALARM STREAM_NOTIFICATION	
		
        if (audioService == null) {
            return;
        }
		try{			
			if(audioService.isStreamMute(stream)){
		    ...
		    Log.w(TAG, "handleVolumeMute() threw RemoteException " + e);
		}
    }
    ...
    public boolean isKeyguardDrawnLw() {
        synchronized (mLock) {
            ...
            if(ActionsConfig.ACTIONS_FEATURE_FINISH_ANIMATION_AFTER_KEYGUARD_ON == 1){
                if(mKeyguardDelegate != null){  		
                    return mKeyguardDelegate.isKeyguardShowCompleted();
                }else{        		
                    return false;
                }
            }
