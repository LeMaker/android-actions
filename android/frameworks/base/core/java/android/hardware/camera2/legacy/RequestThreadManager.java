...
public class RequestThreadManager {
    ...
    /**
    * relax preview timeout limits for module difference when cts test runs.
    *ActionsCode(author:liyuan, change_code)
    */
    private static final int PREVIEW_FRAME_TIMEOUT = 2500; // ms

    //ActionsCode(author:liuyiguang, change_code, change 3000ms to 8000ms for hdr, other function should limit in 3000ms, 
    //otherwise it would be failed in cts)
    /**
    * revert to android source code demands for cts test!  add JPEG_FRAME_TIMEOUT_COMPATHDR for hdr.
    *ActionsCode(author:liyuan, change_code)
    */
    private static final int JPEG_FRAME_TIMEOUT = 3000; // ms (same as CTS for API2)
    private static final int JPEG_FRAME_TIMEOUT_COMPATHDR = 8000; // ms 
    ...
    private final Camera.PictureCallback mJpegCallback = new Camera.PictureCallback() {
        ...
        public void onPictureTaken(byte[] data, Camera camera) {
            ...
            if (captureInfo == null || captureInfo.first == null) {
                ...
                /**
                * BUGFIX: fix for the connecting to camera fail, because of the app's previous release request timeout,
		* when both capture request and release request happens together!
                * ActionsCode(author:liyuan, change_code)
                */
                mReceivedJpeg.open();
    ...
    /**
     *
     * BUGFIX:  Fix calculation method for choosing best preview resolution.
     *
     ************************************
     * 
    ...
    private void configureOutputs(Collection<Pair<Surface, Size>> outputs) {
        ...
        try {
	    /**
            * BUGFIX: fix for 5.1 cts testStillPreviewCombination/testBurstVideoSnapshot/testJpeg fail . 
            *ActionsCode(author:liyuan, change_code)
    	    */
	    mPreviewRunning = true;
            stopPreview();
        ...
        if (previewOutputSizes.size() > 0) {
            ...
            //ActionsCode(author:liuyiguang, add_code, if the bestPreviewDimen is not ratio match largestOutput, fix it!)
            if (!checkAspectRatiosMatch(bestPreviewDimen, largestOutput))
            {
                bestPreviewDimen = largestOutput;
                Log.i(TAG, "configureOutputs - fix the best preview dimen : "+bestPreviewDimen.getWidth()+"x"+bestPreviewDimen.getHeight());
            }
                    ...
                    for (RequestHolder holder : requests) {
                        ...
                        try {
                            ...
                            if (holder.hasJpegTargets()) {
                                ...
                                if (!mReceivedJpeg.block(JPEG_FRAME_TIMEOUT_COMPATHDR)) {
    ...
    public long flush(ActiveRequestCount Reqcount) {
        Log.i(TAG, "Flushing all pending requests.");
        long lastFrame = mRequestQueue.stopRepeating();
	/**
	* BUGFIX: sometimes the app releasing will be hung on forever,which is the current luent's loophole. 
	*ActionsCode(author:liyuan, change_code)
	*/
        mCaptureCollector.failAll(Reqcount);
        return lastFrame;
    }
