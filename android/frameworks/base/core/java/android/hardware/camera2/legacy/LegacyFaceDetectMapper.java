...
public class LegacyFaceDetectMapper {
    ...
    public void mapResultFaces(CameraMetadataNative result, LegacyRequest legacyRequest) {
   	...
   	/**
    	* WORKAROUND: For uppper app cannot process the new android.hardware.camera2.params.Face'data,
	* here fill the android.hardware.camera2.params.Face with legacy android.hardware.Camera.Face'data which the app can process!
    	*ActionsCode(author:liyuan, change_code)
    	*/
	boolean fakeapi2 = true;
	...
	if(false == fakeapi2){
	      ...
	      for (Camera.Face face : faces) {
	          if (face != null) {
	              Face api2Face = new Face(face.rect, face.score, face.id, face.leftEye, face.rightEye, face.mouth);
		      convertedFaces.add(api2Face);
	          } else {
	              Log.w(TAG, "mapResultFaces - read NULL face from camera1 device");
	          }
	      }
