...
public class Environment {
    ...
    private static final String ENV_THIRDARY_STORAGE = "THIRDARY_STORAGE";
    ...
    /* ActionsCode(songzhining, new constants) */
    private static final String DIR_MNT_FUSE = "/storage";
    private static final String DIR_MNT_MEDIA = "/mnt/media_rw";
    ...
    public static class UserEnvironment {
         ...
         ************************************
         * 
         * actions_code(songzhining, change_code)
         * comment: Add externalForAll for sd-ext/uhost
         */
        public UserEnvironment(int userId) {
            ...
            ArrayList<File> externalForAll = Lists.newArrayList();
            ...
            if (!TextUtils.isEmpty(rawEmulatedTarget)) {
                ...
                externalForAll.add(buildPath(emulatedTargetBase, rawUserId));
                ...
                externalForAll.add(new File(rawExternalStorage));
            ...
            if (!TextUtils.isEmpty(rawSecondaryStorage) /*&& userId == UserHandle.USER_OWNER*/) {
                for (String secondaryPath : rawSecondaryStorage.split(":")) {
                    externalForAll.add(new File(secondaryPath));
                }
            }

            // Splice in any thirdary storage paths, but only for owner
            final String rawThirdaryStorage = System.getenv(ENV_THIRDARY_STORAGE);
            if (!TextUtils.isEmpty(rawThirdaryStorage) /*&& userId == UserHandle.USER_OWNER*/) {
                for (String thirdaryPath : rawThirdaryStorage.split(":")) {
                    externalForAll.add(new File(thirdaryPath));
                }
            }
            ...
            mExternalDirsForAll = externalForAll.toArray(new File[externalForAll.size()]);
        }
        ...
        /**
        @hide
         * Gets all external directory.
         * @return Returns all directory
         ************************************
         * 
         * actions_code(songzhining, new_method)
         */
        public File[] getExternalDirsForAll() {
            return mExternalDirsForAll;
        }
    ...
    /* ActionsCode(songzhining, new constants) */
    private static final File ASEC_STORAGE_DIRECTORY = getDirectory("ASEC_STORAGE", "/storage/sdcard");
    ...
    public static File getFlashStorageDirectory() {
        //throwIfUserRequired();
        return sCurrentUser.getExternalDirsForAll()[0];
    }
    ...
    public static File getTfcardStorageDirectory() {
        //throwIfUserRequired();
        return sCurrentUser.getExternalDirsForAll()[1];
    }
    ...
    public static File getUhostStorageDirectory() {
        //throwIfUserRequired();
        return sCurrentUser.getExternalDirsForAll()[2];
    }
     ...
     * actions_code(songzhining, new_method)
     */
    public static File getAsecStorageDirectory() {
        return ASEC_STORAGE_DIRECTORY;
    }
    ...
    public static String getFlashStorageState() {
        final File externalDir = sCurrentUser.getExternalDirsForAll()[0];
    ...
    public static String getTfcardStorageState() {
        final File externalDir = sCurrentUser.getExternalDirsForAll()[1];
        return getStorageState(externalDir);
    }
    ...
    public static String getUhostStorageState() {
        final File externalDir = sCurrentUser.getExternalDirsForAll()[2];
        return getStorageState(externalDir);
    }
     ...
     ************************************
     *      
     *actions_code(ywwang, optimize: translate fuse path to real path)
     *
     * @hide
     */
    public static File maybeTranslateEmulatedPathToInternal(File path) {
        ...
        try {
            ...
            if (rawPath.startsWith(DIR_MNT_MEDIA)) {
                return path;
            }
        } catch (IOException e) {
        ...
        // translate path: emulated sdcard --> data media
        if (Environment.isExternalStorageEmulated()
            ...
            if (rawPath.startsWith(CANONCIAL_EMULATED_STORAGE_TARGET)) {
                ...
                if (internalPath.exists()) {
                    ...
                    //Log.w(TAG, "Do not exist internalPath: " + internalPath);
                }
            }
        }

        // translate path: fused sdcard/sd-ext/uhost --> media_rw sdcard/sd-ext/uhost
        if (rawPath.startsWith(DIR_MNT_FUSE)) {
            final File internalPath = new File(DIR_MNT_MEDIA,
                    rawPath.substring(DIR_MNT_FUSE.length()));
            if (internalPath.exists()) {
                return internalPath;
            } else {
                //Log.w(TAG, "Do not exist internalPath: " + internalPath);
