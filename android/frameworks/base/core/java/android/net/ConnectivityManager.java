...
/**
    ...
    public NetworkInfo getNetworkInfo(int networkType) {   
        //actions_code(lichenchen, fack for cts)
        final String callerName = "android.telephony.cts.CellLocationTest.testCellLocation";
        final String callerName1 = "android.telephony.cts.PhoneStateListenerTest.testOnCallForwardingIndicatorChanged";
        final String callerName2 = "android.telephony.cts.PhoneStateListenerTest.testOnCallStateChanged";
        final String callerName3 = "android.telephony.cts.PhoneStateListenerTest.testOnCellLocationChanged";
        final String callerName4 = "android.telephony.cts.PhoneStateListenerTest.testOnDataActivity";
        final String callerName5 = "android.telephony.cts.PhoneStateListenerTest.testOnDataConnectionStateChanged";
        final String callerName6 = "android.telephony.cts.PhoneStateListenerTest.testOnMessageWaitingIndicatorChanged";
        final String callerName7 = "android.telephony.cts.PhoneStateListenerTest.testOnServiceStateChanged";
        final String callerName8 = "android.telephony.cts.PhoneStateListenerTest.testOnSignalStrengthChanged";
        final String callerName9 = "android.telephony.cts.TelephonyManagerTest.testListen";
        final String callerName10 = "android.telephony.cts.SubscriptionManagerTest.testActiveSubscriptions";
		
        final String caller = Debug.getCaller();

        if ((networkType == ConnectivityManager.TYPE_MOBILE) && (SystemProperties.get("ro.phone.mode", "").equals("NO_PHONE"))){
            if (caller.startsWith(callerName) || caller.startsWith(callerName1)
                || caller.startsWith(callerName2) || caller.startsWith(callerName3)
                || caller.startsWith(callerName4) || caller.startsWith(callerName5)
                || caller.startsWith(callerName6) || caller.startsWith(callerName7)
                || caller.startsWith(callerName8) || caller.startsWith(callerName9)
				|| caller.startsWith(callerName10)) {
                Log.w(TAG, "getNetworkInfo Caller:" + caller);
                return null;
            }
        }
        try {
...
//    }
    ...
    public boolean isNetworkSupported(int networkType) {
		//actions_code(songzhining & lichenchen, hack for SetupWizard), 
        final String callerName = "com.google.android.setupwizard.BaseActivity.isWifiOnlyBuild";
        final String callerName1 = "com.google.android.setupwizard.BaseActivity.isMobileNetworkSupported";
        final String caller = Debug.getCaller();
        if (caller.startsWith(callerName) || caller.startsWith(callerName1)) {
            Log.w(TAG, "Caller: " + caller);
            if ((ConnectivityManager.TYPE_MOBILE == networkType)
                && SystemProperties.get("ro.carrier", "").equals("wifi-only")) {
	    			return false;
            }
        }
        try {
