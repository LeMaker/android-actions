...
import static com.actions.ActionsEnv.DEBUG_APP_LAUNCH;
...
public class Instrumentation {
    ...
    public Activity startActivitySync(Intent intent) {
        ...
        synchronized (mSync) {
			...
			//ActionsCode(rongxing, bugfix for 5.1 cts android.mediastress test)
			ComponentName cls = intent.resolveActivity(getTargetContext().getPackageManager());
			if (cls != null) {         
				if(cls.getClassName().equals("android.mediastress.cts.MediaFrameworkTest")){
...
//      }
    ...
    public void callActivityOnResume(Activity activity) {
		...
		//actions_code(ywwang, notify pfmserver that we are resuming
		if(ActionsConfig.ACTIONS_PROFILE_LIBC_LD>0 &&activity!=null && activity.getClass()!=null )
            PerformanceManager.getInstance().appNotify(activity.getClass().getName());
        if (mActivityMonitors != null) {
            synchronized (mSync) {
                final int N = mActivityMonitors.size();
                for (int i=0; i<N; i++) {
    ...
    public ActivityResult execStartActivity(
		...
		if (target != null && target.getIntent() != null 
            &&  target.getIntent().getCategories() != null 
            && target.getIntent().getCategories().contains(Intent.CATEGORY_HOME)) {
             //actions_code(ywwang, notify pfmserver that we are start
            if(ActionsConfig.ACTIONS_PROFILE_LIBC_LD>0) 
                PerformanceManager.getInstance().appStart();
        }
        if (mActivityMonitors != null) {
            synchronized (mSync) {
                final int N = mActivityMonitors.size();
