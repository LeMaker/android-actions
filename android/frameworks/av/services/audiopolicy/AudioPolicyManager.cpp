...
status_t AudioPolicyManager::setDeviceConnectionStateInt(audio_devices_t device,
                                                         ...
                                                         const char *device_address)
{
    ALOGW("setDeviceConnectionState() device: %x, state %d, address %s",
	...
	ALOGW("mAvailableOutputDevices 0x%x, index %d", mAvailableOutputDevices.types(), index);
        ...
        case AUDIO_POLICY_DEVICE_STATE_UNAVAILABLE: {
            ...
            //ActionsCode(author:yuchen, change_code): special dealing with a2dp
            //FIXME: sometimes mediaserver restarts with a2dp on, AudioPolicyManager will keep
            //an initial config of mAvailableOutputDevices, which includes all 3 a2dp types
            //but now we only disconnect AUDIO_DEVICE_OUT_BLUETOOTH_A2DP after that scenario,
            //so a2dp output wont be closed if bluetooth disconnected, result in disconnected 
            //status when bluetooth connect and play again.
            //FIMXE: it might not be the best way to solve the problem.
	    if((device == AUDIO_DEVICE_OUT_BLUETOOTH_A2DP)||
	        (device == AUDIO_DEVICE_OUT_BLUETOOTH_A2DP_HEADPHONES)||
	        (device == AUDIO_DEVICE_OUT_BLUETOOTH_A2DP_SPEAKER))
	    {
	    	//ALOGE("mAvailableOutputDevices 0x%x", mAvailableOutputDevices.types());
	        sp<DeviceDescriptor> a2dpDesc = new DeviceDescriptor(String8(""), AUDIO_DEVICE_OUT_BLUETOOTH_A2DP);
	        if(mAvailableOutputDevices.types()&AUDIO_DEVICE_OUT_BLUETOOTH_A2DP)
	        {
	        	mAvailableOutputDevices.remove(a2dpDesc);
	        }

	        sp<DeviceDescriptor> a2dpDesc4 = new DeviceDescriptor(String8(""), AUDIO_DEVICE_OUT_ALL_A2DP);
	        if(mAvailableOutputDevices.types()&AUDIO_DEVICE_OUT_ALL_A2DP)
	        {
	        	mAvailableOutputDevices.remove(a2dpDesc4);
	        }
	        
	        sp<DeviceDescriptor> a2dpDesc2 = new DeviceDescriptor(String8(""), AUDIO_DEVICE_OUT_BLUETOOTH_A2DP_HEADPHONES);
	        if(mAvailableOutputDevices.types()&AUDIO_DEVICE_OUT_BLUETOOTH_A2DP_HEADPHONES)
	        {
	        	mAvailableOutputDevices.remove(a2dpDesc2);
	        }

	        sp<DeviceDescriptor> a2dpDesc3 = new DeviceDescriptor(String8(""), AUDIO_DEVICE_OUT_BLUETOOTH_A2DP_SPEAKER);
	        if(mAvailableOutputDevices.types()&AUDIO_DEVICE_OUT_BLUETOOTH_A2DP_SPEAKER)
	        {
	        	mAvailableOutputDevices.remove(a2dpDesc3);
	        }	        
	    ...
	    ALOGE("%s %d mAvailableOutputDevices 0x%x", __FUNCTION__, __LINE__, mAvailableOutputDevices.types());
...
/**
 *NEW_FEATURE: Add configuration to change audio output priority, 
 *which allows earphone precedes hdmi/miracast or hdmi/miracast precedes earphone.
 ************************************
 *      
...
AudioPolicyManager::AudioPolicyManager(AudioPolicyClientInterface *clientInterface)
    ...
    mBeaconMuted(false)
{
//ActionsCode(author:yuchen, change_code): string var to contain property value
#if ACTIONS_FEATURE_AUDIO_OUTPUT_PRIORITY_CONFIGURABLE==1
...
//ActionsCode(author:yuchen, change_code): retrieve audio output priority configuration
#if ACTIONS_FEATURE_AUDIO_OUTPUT_PRIORITY_CONFIGURABLE==1
    property_get("ro.audio.output.priority", value, "earphone");
    if(strncmp(value, "earphone", 8) == 0) {
		// 1: earphone > hdmi
		ALOGD("audio output: earphone is prior to hdmi");
		mAudioOutputPriority = 1;
    } else {
    	ALOGD("audio output: hdmi is prior to earphone");
    	mAudioOutputPriority = 0;
    }
    
    property_get("ro.audio.output.priority2", value, "earphone");
    if(strncmp(value, "earphone", 8) == 0) {
		// 1: earphone > hdmi
		ALOGD("audio output: earphone is prior to remote");
		mAudioOutputPriority = mAudioOutputPriority|0b10;
    } else {
    	ALOGD("audio output: remote is prior to earphone");
    	mAudioOutputPriority = mAudioOutputPriority|0b00;
    }    
#endif
...
/**
 *NEW_FEATURE: Allow HDMI volume to be ajusted from volume bar.
 ************************************
 *      
 *ActionsCode(author:yuchen, change_code)
 */    
...
//ActionsCode(author:yuchen, change_code): sync from former code 
//ActionsCode(author:yuchen, change_code): 20150119 following code is necessary for hdmi
//					no need to include in a marco.  	
//#if ACTIONS_FEATURE_HDMI_WITHOUT_FIXED_VOLUME==1    	
	// author: zxchen.
        // FIXME
...
//ActionsCode(author:yuchen, change_code): 20150618 on S500 which default output is hdmi, we should
//					this can happen on other devices, who should have played the audio,
//					but route to HDMI in this branch.
        ALOGE("%s %d to hdmi ", __FUNCTION__, __LINE__);
/*        
                                                             ...
                                                             bool fromCache)
{
    ...
    case STRATEGY_MEDIA: {
        ...
        if (strategy != STRATEGY_SONIFICATION) {
//ActionsCode(author:yuchen, change_code): if earphone priority configged to be higher than miracast,
//check earphone plug state first. if no earphone plugged in, audio data should be output from miracast.	
#if ACTIONS_FEATURE_AUDIO_OUTPUT_PRIORITY_CONFIGURABLE==1
            if(mAudioOutputPriority >= 2)
            {
...
//ActionsCode(author:yuchen, change_code): earphone got high priority over hdmi        
#if ACTIONS_FEATURE_AUDIO_OUTPUT_PRIORITY_CONFIGURABLE==1
        if (mAudioOutputPriority%2 == 1) {
        	if (device2 == AUDIO_DEVICE_NONE) {
                device2 = availableOutputDeviceTypes & AUDIO_DEVICE_OUT_WIRED_HEADPHONE;
            }
            if (device2 == AUDIO_DEVICE_NONE) {
...
//ActionsCode(author:yuchen, change_code): earphone got lower priority to hdmi
#if ACTIONS_FEATURE_AUDIO_OUTPUT_PRIORITY_CONFIGURABLE==1
        if (mAudioOutputPriority%2 == 0) {
            if (device2 == AUDIO_DEVICE_NONE) {
                device2 = availableOutputDeviceTypes & AUDIO_DEVICE_OUT_WIRED_HEADPHONE;
            }
            if (device2 == AUDIO_DEVICE_NONE) {
                                             ...
                                             const char* address)
{
    ...
    ALOGW("setOutputDevice() prevDevice %04x", prevDevice);
                                            ...
                                            audio_devices_t device)
{
    ...
    //ActionsCode(author:yuchen, change_code): send volume to audio hal to check
    if (stream == AUDIO_STREAM_MUSIC) {
	    AudioParameter param = AudioParameter();
	    param.addInt(String8(AudioParameter::keyVolumeIndex), (int)index);
                                           ...
                                           audio_devices_t device)
{
    ...
    ALOGW("setStreamMute() stream %d, mute %d, output %d, mMuteCount %d device %04x",
...
/**
 *NEW_FEATURE: allow music playing when booting.
 ************************************
 *      
 *ActionsCode(author:yuchen, change_code)
 */                       
    ...
    :   mIndexMin(0), mIndexMax(1), mCanBeMuted(true)
{
    ...
    //CRF-BUG00106580
    //add by wgh(3307)
    mIndexCur.add(AUDIO_DEVICE_OUT_SPEAKER, 1);	
    //end by wgh    
}
