...
CameraService::CameraService()
    :mSoundRef(0), mModule(0),mAsyncTask(NULL),mNumNeedsUpdate(false)
{
...
void CameraService::onFirstRef()
{
	...
	*ActionsCode(author:liyuan, change_code)
	*/
	mAsyncTask = new CameraAsyncTask((void *)initTask , (void *)this);
        if(mAsyncTask != NULL)
        {
            mAsyncTask->run();
        }
		...
		if(!strcmp(event->name, "video3")){
			...
			property_get("sys.camerahal.uvc_actmode_id", value, "0");
				...
				if(client!=0){
					ALOGD("mClient =%d\n",i);
					hasCameraOpend=true;
					if(i==0&&!strcmp(value, "1") || i==1&&!strcmp(value, "2")){
					   sp<Client> client;
					   CaptureResultExtras resultExtras; 
					   client = static_cast<Client*>(mClient[i].promote().get());
					   client->notifyError2(ICameraDeviceCallbacks::ERROR_CAMERA_SERVICE, resultExtras);
...
void * CameraService::monitor_uvc_hotplug(){             
	...
	int fd = inotify_init(); 
	int wd = inotify_add_watch(fd, monitordir, IN_CREATE|IN_ISDIR|IN_ONLYDIR|IN_DELETE);
...
void CameraService::Client::notifyError2(ICameraDeviceCallbacks::CameraErrorCode errorCode,
        const CaptureResultExtras& resultExtras) {
	notifyError(errorCode, resultExtras);
}
