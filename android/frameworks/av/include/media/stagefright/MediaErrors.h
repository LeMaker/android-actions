...
enum {
	...
	// used for actios atcvideoextractor
	INFO_NO_RAW_DATA_NOW     = MEDIA_ERROR_BASE - 15,
	ERROR_AUDIO_WAIT         = MEDIA_ERROR_BASE - 16,
	ERROR_VIDEO_WAIT         = MEDIA_ERROR_BASE - 17,
	
	/*used for actions to solve anr in awesomeplayer.
		*when Network signal in weak,reset or pause command
		*will be blocked by mLock which will result in anr
		************************************
    *  
		*ActionsCode(author:sunchengzhi, change_code)
		*/
	ERROR_NETWORK_READ_TIMEOUT        = MEDIA_ERROR_BASE - 18,
