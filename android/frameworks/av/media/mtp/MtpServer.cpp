...
//ActionsCode(huoysh, sync from gs702c_4.4.4,add action operations)
/* please reference to linux/usb/f_mtp.h */
...
void MtpServer::run() {
    ...
    while (1) {
        ...
        if(restart_flag) {
             usleep (20000);
             if(i++ > 100) {
                  ALOGD("mtp ioctl cmd:kernel restart.\n");
		    ioctl(mFD,ACTIONS_MTP_MODIFY_SERIALNO_RESTART,NULL);
                  i = 0;
             	}
             continue;
        }
...
bool MtpServer::handleRequest() {
    ...
    switch (operation) {
        ...
        case MTP_OPERATION_END_EDIT_OBJECT:
            ...
            break;
		//ActionsCode(huoysh, sync from gs702c_4.4.4,add action operations)
        case ACTIONS_MTP_OPERATION:
            response = doActionsMtpOper();
            break;
        default:
	...
	//ActionsCode(huoysh, sync from gs702c_4.4.4,add action operations)	
	if(restart_flag){
		ALOGD("execute kernel restart now\n");
		ioctl(mFD,ACTIONS_MTP_MODIFY_SERIALNO_RESTART,NULL);
	}
	if(switch_to_adfu_flag){
		ALOGD("%s %d. we getting into adfu\n",__FUNCTION__,__LINE__);
		ioctl(mFD,ACTIONS_MTP_SWITCH_TO_ADFU,NULL);
	}
    return true;
}
...
MtpResponseCode MtpServer::doSendObjectInfo() {
  ...
  if (format == MTP_FORMAT_ASSOCIATION) {
	...
	//ActionsCode(huoysh, fix:BUG00145418 file name too long, mtp drivers hangup) 
    if (strlen((const char *)name) > 255) {
        ALOGD("%s %d  the file name len:%d, path size:%d. path: %s\n", 
			__FUNCTION__, __LINE__, strlen((const char *)name), path.size(), (const char*)path);
        return MTP_RESPONSE_GENERAL_ERROR;
    }
...
MtpResponseCode MtpServer::doSendObject() {
    ...
    if (mfr.fd < 0) {
		//ActionsCode(huoysh, fix:BUG00145418 file name too long, mtp drivers hangup) 	
        ALOGD("%s %d  can not open file, path: %s\n", __FUNCTION__, __LINE__, (const char*)mSendObjectFilePath);
        result = MTP_RESPONSE_GENERAL_ERROR;
        goto done;
    }
...
/**
  ...
  ************************************
  *      
  *ActionsCode(author:huoyongsheng, change_code)
  */
MtpResponseCode MtpServer::doDeleteObject() {
	...
	/*ActionsCode(author:huoyongsheng, change_code)*/
	if (!strncmp(filePath, "/mnt/media", strlen("/mnt/media"))) {
...
*
*/
MtpResponseCode MtpServer::doActionsMtpOper(){
	int ret;
	int len;
	...
	int oper = mRequest.getParameter(1);
	ALOGD("%s %d. param1:0x%x\n",__FUNCTION__,__LINE__,oper);

	switch(oper){
		case ACTIONS_MODIFY_SERIAL_RESTART:
			restart_flag = true;
			break;
		case ACTIONS_MODIFY_SERIAL:
			len = mRequest.getParameter(2);
			ALOGD("%s %d. len:%d\n",__FUNCTION__,__LINE__,len);
			ret = mData.read(mFD);
            if (ret < 0) {
                ALOGE("serialno read returned %d, errno: %d", ret, errno);
                return MTP_RESPONSE_GENERAL_ERROR;
            }
			memcpy(&serial_number[1], mData.getData(),len);	
			serial_number[0] = (char)len;
			serial_number[len+1] = '\0';
			
			ALOGD("%s %d. serial_number:%s\n",__FUNCTION__,__LINE__,(char*)&serial_number[1]);
			
			ret = ioctl(mFD,ACTIONS_MTP_MODIFY_SERIALNO,(unsigned long)serial_number);
			if (ret){
				ALOGE("ioctl ACTIONS_MTP_MODIFY_SERIALNO error\n");
				return MTP_RESPONSE_GENERAL_ERROR;
			}
			mData.reset();
			break;
		case ACTIONS_GET_SERIAL:
			//get serial no
			
			ret = ioctl(mFD,ACTIONS_MTP_GET_SERIALNO,(unsigned long)serial_number);
			if (ret){
				ALOGE("ioctl ACTIONS_MTP_GET_SERIALNO error\n");
				return MTP_RESPONSE_GENERAL_ERROR;
			}
			/*ActionsCode(author:tangshaoqing, change code)*/
			mData.setOperationCode(mRequest.getOperationCode());
			mData.setTransactionID(mRequest.getTransactionID());
			ALOGD("%s %d. serial_number led:%d\n",__FUNCTION__,__LINE__,serial_number[0]);
			mData.writeData(mFD,(void *)&serial_number[1],serial_number[0]);
			break;		
		case ACTIONS_SWITCH_TO_ADFU:
		{
			mData.reset();
			switch_to_adfu_flag = true;
			break;
		}

		case ACTIONS_GET_MTP_MANUFACTURER:
		{
			char string[20];
			memcpy(&string[0], "ACTIONSMTP",10);
			/*ActionsCode(author:tangshaoqing, change code)*/	
			mData.setOperationCode(mRequest.getOperationCode());
			mData.setTransactionID(mRequest.getTransactionID());
			mData.writeData(mFD,(void *)&string[0],10);
			break;
		}

		case ACTIONS_SET_MISCINFO:
		{
			len = mRequest.getParameter(2);
			ALOGD("%s %d. len:%d\n",__FUNCTION__,__LINE__,len);

			if(len > ACTIONS_MISCINFO_MAX_LEN){
				return MTP_RESPONSE_GENERAL_ERROR;
			}

			ret = mData.read(mFD);
	                if (ret < 0) {
                   	    ALOGE("mData read returned %d, errno: %d", ret, errno);
      		            return MTP_RESPONSE_GENERAL_ERROR;
		        }

			miscinfo = (char *)malloc(ACTIONS_MISCINFO_MAX_LEN);
			if(!miscinfo){
				ALOGD("%s %d. malloc buf for misc info fail\n",__FUNCTION__,__LINE__);
				return MTP_RESPONSE_GENERAL_ERROR;
			}

			memset(miscinfo, 0, ACTIONS_MISCINFO_MAX_LEN);
			memcpy(miscinfo, mData.getData(),len);	
			//serial_number[0] = (char)len;
			//serial_number[len+1] = '\0';
			
			ret = ioctl(mFD,ACTIONS_MTP_SET_MISCINFO,(unsigned long)miscinfo);
			if (ret){
				ALOGE("ioctl ACTIONS_MTP_MODIFY_SERIALNO error\n");
				free(miscinfo);
				return MTP_RESPONSE_GENERAL_ERROR;
		...
		case ACTIONS_GET_MISCINFO:
		{
			...
			memset(miscinfo, 0, ACTIONS_MISCINFO_MAX_LEN);
			ret = ioctl(mFD,ACTIONS_MTP_GET_MISCINFO,(unsigned long)miscinfo);
			if (ret){
				ALOGE("ioctl ACTIONS_MTP_GET_SERIALNO error\n");
				free(miscinfo);
				return MTP_RESPONSE_GENERAL_ERROR;
			...
			mData.setOperationCode(mRequest.getOperationCode());
			mData.setTransactionID(mRequest.getTransactionID());
			mData.writeData(mFD,miscinfo,ACTIONS_MISCINFO_MAX_LEN);
			free(miscinfo);
			break;
		}
		default:
			ALOGE("we don't accept param:0x%x\n",oper);
			return MTP_RESPONSE_GENERAL_ERROR;
			break;
	}
