...
//actions_code(lishiyuan,bugfix: fix BUG00243003)
static int64_t mLivefirstTimeUs = -1ll;
...
/**
  *
  *bugfix for BUG00210699 when length of ts segment is zero
  *2015-03-25 bugfix:BUG00269663
  ************************************
  *      
...
void PlaylistFetcher::onDownloadNext() {
    ...
    ssize_t bytesRead;
    do {
		//ActionsCode(author:rongxing, bugfix:BUG00269663)
		if(mSession->mSeekInProgress){
			ALOGV("stop download when seeking");
			return;
		}
    ...
    if (tsBuffer == NULL) {
    	  //ActionsCode(sunchengzhi,bugfix BUG00210699)
    	  if(!buffer->size()){
      		notifyError(ERROR_END_OF_STREAM);
      		return;
     	  }
    ...
    for (size_t i = mPacketSources.size(); i-- > 0;) {
        ...
        while (source->hasBufferAvailable(&finalResult)
            ...
            if (mStartup) {
                ...
                if (!mPlaylist->isComplete() && !mPlaylist->isEvent()
                     && mStartTimeUs <= 0){
                    mLivefirstTimeUs = timeUs;
                    ALOGV("mLivefirstTimeUs  == %lld",mLivefirstTimeUs);
                }

                if (!mFirstPTSValid) {
                    if (!mPlaylist->isComplete() && !mPlaylist->isEvent()
						&& mStartTimeUs > 0 && mLivefirstTimeUs > 0)
                    {
                        mFirstTimeUs = mLivefirstTimeUs;
                        ALOGV("1====mFirstTimeUs == %lld", mFirstTimeUs);
                    }else{
                        mFirstTimeUs = timeUs;
                        ALOGV("2====mFirstTimeUs == %lld", mFirstTimeUs);
                    }
                    mFirstPTSValid = true;
                }
