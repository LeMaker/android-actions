        ...
        case kWhatFetcherNotify:
        {
            ...
            switch (what) {
                ...
                case PlaylistFetcher::kWhatStopped:
                {
                    ...
                    if (mContinuation != NULL) {
                        ...
                        if (--mContinuationCounter == 0) {
							...
							//ActionsCode(author:rongxing, bugfix:BUG00270082)
                            //if (mSeekReplyID != 0) {
                ...
                case PlaylistFetcher::kWhatDurationUpdate:
                {
                    ...
                    /**
                                 * actions_code(author:lishiyuan, BUGFIX: fix BUG00232957.
                                 * mediaserver crash because in LiveSession it uses editValueFor()
                                 * to get the item that has been removed.)
                                 **/
                    if (mFetcherInfos.indexOfKey(uri) < 0) {
                        ALOGE("xxxx  couldn't find uri");
                        break;
                    }
...
void LiveSession::onChangeConfiguration2(const sp<AMessage> &msg) {
    ...
    if (changedMask == 0) {
        ...
        onChangeConfiguration3(msg);
		//ActionsCode(author:rongxing, bugfix:BUG00270082 BUG00269663)
		if (mSeekReplyID != 0) {
			mSeekInProgress = false;
        	CHECK(mSeekReply != NULL);
        	mSeekReply->setInt32("err", OK);
	...
	//ActionsCode(author:rongxing, bugfix:BUG00270121)
	sp<AnotherPacketSource> sources[kMaxStreams];
    for (size_t j = 0; j < kMaxStreams; ++j) {
        sources[j] = mPacketSources.valueFor(indexToType(j));
		sources[j]->clear();
		ALOGV("sources[%d]->clear() is %p",j,sources[j].get());
    }
