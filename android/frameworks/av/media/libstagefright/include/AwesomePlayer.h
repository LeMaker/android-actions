...
struct AwesomePlayer {
   ...
   /*ActionsCode(author:sunchengzhi,actvideoextrator uses mListenser to getCurrentPosition when subtilte selecttrack)*/
    wp<MediaPlayerBase> actgetlistener(){
    	return mListener;
    }
    /*ActionsCode(author:sunchengzhi,actvideoextrator call this to judge if video is StreamingHTTP)*/
    bool actgetstreaminghttp(){
    	return isStreamingHTTP();
    }
   /*ActionsCode(author:sunchengzhi,fix cts testChangeSubtitleTrack,testDeselectTrack,testGetTrackInfo)*/ 
    int32_t mSubTrackIndexSelected;
    int32_t mAudioTrackIndexSelected;
    
    /*ActionsCode(author:sunchengzhi,fix cts MediaPlayerFlakyNetworkTest)*/
    bool mDisconnecting;
    
    /*ActionsCode(author:sunchengzhi,for adaptive drop B-Frame)*/
    bool mIs4kVideo;
