...
void MediaCodec::extractCSD(const sp<AMessage> &format) {
    ...
    AString mime;
    size_t i = 0;
    format->findInt32("encoder", &encoder);
    CHECK(format->findString("mime", &mime));
    if(!strncasecmp(mComponentName.c_str(), "OMX.Action.Video.Decoder",24) && !strncasecmp(mime.c_str(), "video/avc", 9)){    //for actions avc decoder	 
    	 sp<ABuffer> csdbuffer=new ABuffer(2048);
    	 csdbuffer->setRange(0, 0);    	
    	 for (;;) {
    	 	 sp<ABuffer> csd;
         if (!format->findBuffer(StringPrintf("csd-%u", i).c_str(), &csd)) {
            break;
         }
       	 memcpy(csdbuffer->data() + csdbuffer->size(),csd->data(),csd->size());
       	 csdbuffer->setRange(0, csdbuffer->size() + csd->size());
       	 ++i;
       }
       if(csdbuffer->size()>0){
       	ALOGD("avc csd buffer size is %d \n",csdbuffer->size());
       	csdbuffer->meta()->setInt64("timeUs", 0);
       	mCSD.push_back(csdbuffer);
       }
       
    }else if(!strncasecmp(mComponentName.c_str(), "OMX.Action.Audio.Decoder",24)&& !strncasecmp(mime.c_str(), "audio/vorbis", 12)){
    	 sp<ABuffer> csd[3];
    	 size_t   crdbuffersize =0;
    	 for (i=0;i<3;i++) {
         if (!format->findBuffer(StringPrintf("csd-%u", i).c_str(), &csd[i])) {
            break;
         }
         crdbuffersize += csd[i]->size();
		 		ALOGD("csd[%d] size:%d",i,csd[i]->size());
       }
       if(crdbuffersize>0){
       	 sp<ABuffer> csdbuffer=new ABuffer(crdbuffersize+36);
       	 GetVorbisSpecificData(csd[0],csd[2],csd[1],csdbuffer);
       	 ALOGD("Csdbuffer size:%d",csdbuffer->size());
		  	 mCSD.push_back(csdbuffer);
       }
    } else{
    	for (;;) {
        ...
        mCSD.push_back(csd);
        ++i;
        /*ActionsCode(author:sunchengzhi, for android vorbis decoders only need two csd buffer)*/ 
        if(!strncasecmp(mime.c_str(), "audio/vorbis", 12) && !encoder){
        	if(i==2){
        		break;
        	}
        }
