...
int32_t MyVorbisExtractor::packetBlockSize(MediaBuffer *buffer) {
 ...
 /**
  *
  *xxxxx add kKeyActFirstVorbisBook to adaptive Actions Vorbis decoder
  *
  *
  ************************************
  *      
        ...
        case 3:
        {
            if (0 != _vorbis_unpack_comment(&mVc, &bits)) {
                return ERROR_MALFORMED;
						...
						mMeta->setData(kKeyActFirstVorbisBook, 0, data, size);/*ActionsCode(author:sunchengzhi, Actions Vorbis decoder need VorbisBook as config pkt)*/
            parseFileMetaData();
            break;
        }
...
sp<MetaData> OggExtractor::getMetaData() {
 ...
 /**
  *
  *xxxxx bugfix for ogm video format being recognized as ogg audio format
  *
  *
  ************************************
  *      
...
bool SniffOgg(
  ...
  /*--ActionsCode(author:sunchengzhi, bugfix start)--*/
    MyVorbisExtractor *m = new MyVorbisExtractor(source);// actions bug fix 

    status_t mCheck = m->seekToOffset(0);
    if(mCheck != OK){
        delete m;
        return false;
    }
    ...
    delete m;
  /*--ActionsCode(author:sunchengzhi, bugfix end)--*/
