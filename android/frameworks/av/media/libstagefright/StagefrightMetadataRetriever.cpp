...
VideoFrame *StagefrightMetadataRetriever::getFrameAtTime(
 ...
 /*ActionsCode(author:sunchengzhi,use actions hardware decoder,so we change OMXCodec::kPreferSoftwareCodecs to 0 to select hardware decoder)*/
    VideoFrame *frame =
...
//actions_code(author:lishiyuan,bugfix)
void StagefrightMetadataRetriever::parseMetaData() {
    ...
    for (size_t i = 0; i < kNumMapEntries; ++i) {
        ...
        if (meta->findCString(kMap[i].from, &value)) {
            //actions_code(lishiyuan,bugfix:BUG00240652,fix messy world when ID3 parse)
            if (kMap[i].name) {
				// add to charset detector
				if((value[0] == 0xcf) && (value[1] == 0xcf)){
                   value += 2;
				}
