...
#define ACTIONS_VIDEO_DECODER_DEINTERLACE 	ACTIONS_PREFIX".Video.Decoder.Deinterlace"
...
#define VIDDEC_COMPONENTROLES_WMV2				"video_decoder.wmv2"
#define VIDDEC_COMPONENTROLES_VP9				"video_decoder.vp9"
...
void OMXCodec::findMatchingCodecs(
    ...
    /*ActionsCode(author:sunchengzhi, audio decoders use android codecs)*/
    if(!strncasecmp(mime, "audio/", 6) && !createEncoder){
...
uint32_t OMXCodec::getComponentQuirks(
    ...
    if (info->hasQuirk("input-buffer-sizes-are-bogus")) {
        	quirks |= kInputBufferSizesAreBogus;
   	}
   	if (info->hasQuirk("needs-flush-before-disable")) {
        	quirks |= kNeedsFlushBeforeDisable;
    }
  	if (info->hasQuirk("kRequiresLoadedToIdleAfterAllocation")) {
...
sp<MediaSource> OMXCodec::Create(
    ...
    CHECK(success);
    /*ActionsCode(author:sunchengzhi,because actions vp9 and hevc decoders are not adaptive,in order to pass testAllVideoDecodersAreAdaptive ,we can not 
    put vp9 and hevc in media_codecs.xml,so we use actions defined mimetype)*/
    if(!createEncoder){
    	  if(!strcmp(mime, "video/hevc")){
    	  	findMatchingCodecs(
            "video/hevc_actions", createEncoder, matchComponentName, flags, &matchingCodecs);
    	  }else if(!strcmp(mime, "video/x-vnd.on2.vp9")){
    	  	findMatchingCodecs(
            "video/vp9_actions", createEncoder, matchComponentName, flags, &matchingCodecs);
    	  }else{
    	  	findMatchingCodecs(
    ...
    for (size_t i = 0; i < matchingCodecs.size(); ++i) {
        ...
        if(!strcmp(componentName, ACTIONS_VIDEO_DECODER)){
        	 ...
        	 meta->findInt32(kKeyHeight, &height);
		 	 ...
		 	 if(!strcmp(mime, MEDIA_MIMETYPE_VIDEO_MPEG2) && width>=1280 && height>=720){
		 	 		ALOGI("using deinterlace component\n");
					continue;
	  		 }
	  	}
        /*ActionsCode(author:sunchengzhi,add code to distinguish antions extractor or android original extractor,
        now is not used,but is reserver for future use)*/
        if(!strcmp(componentName, ACTIONS_VIDEO_DECODER) || !strcmp(componentName, ACTIONS_VIDEO_DECODER_DEINTERLACE)){
        	void * MediaInfo=NULL;
        	if(meta->findPointer(kKeyActMediaInfo, &MediaInfo)){
        	 	ALOGI("==using action extractor===\n");
        	 	flags |= kActUseActionsExtractor;
        	}else{
        	 	ALOGI("==using android extractor===\n");
        	 	flags |= kActUseAndroidExtractor;
        	}			
        }
...
/**
  *add  actions CodecSpecificData for actions decoders when call addCodecSpecificData
  ************************************
  *      
  *ActionsCode(author:sunchengzhi, change_code)
  */
status_t OMXCodec::configureCodec(const sp<MetaData> &meta) {
    ...
    if (!(mFlags & kIgnoreCodecSpecificData)) {
        ...
        const void *data;
        size_t size;
      /*ActionsCode(author:sunchengzhi, add  actions CodecSpecificData for actions video decoders)*/
      if(!strcmp(mComponentName, ACTIONS_VIDEO_DECODER) || !strcmp(mComponentName, ACTIONS_VIDEO_DECODER_DEINTERLACE)){

        	if(meta->findData(kKeyActVideoPrivateData, &type, &data, &size)){
	 						addCodecSpecificData(data,size);
	  		}
			if(meta->findData(kKeyActGLBL, &type, &data, &size)){
	 						addCodecSpecificData(data,size);
	  		}
    	}
	     
    	 /*ActionsCode(author:sunchengzhi, add  actions CodecSpecificData for actions audio decoders)*/
    	if(!strcmp(mComponentName, ACTIONS_AUDIO_DECODER) ){
	  			if(meta->findData(kKeyActAudioPrivateData, &type, &data, &size)){
	 						addCodecSpecificData(data,size);
	  			}
    	}
    	if (meta->findData(kKeyESDS, &type, &data, &size)) {
            ...
            if(!strcmp(mComponentName, ACTIONS_AUDIO_DECODER)){
								if(!strcmp(mMIME,MEDIA_MIMETYPE_AUDIO_AAC)){
									sp<ABuffer> esdsbuffer = new ABuffer(12);
									int32_t channelcount=0;
									meta->findInt32(kKeyActChannelCount, &channelcount);
									GetAACCodecSpecific(esdsbuffer,codec_specific_data,type,channelcount);            			
            			addCodecSpecificData(esdsbuffer->data(),3*sizeof(int));
								}
            }else{
            	 if(strcmp(mComponentName, ACTIONS_VIDEO_DECODER) || strcmp(mMIME,MEDIA_MIMETYPE_VIDEO_MPEG2)){
            	 	 addCodecSpecificData(
                    codec_specific_data, codec_specific_data_size);//actions mpeg2 not need config pkt
            	 }
      	...
      	if(!strcmp(mComponentName, ACTIONS_AUDIO_DECODER)){
						...
						csdsize+=size;
            if (!meta->findData(kKeyActFirstVorbisBook, &type, &data, &size)){
            	CODEC_LOGI("++++Actions Vorbis codec need kKeyACTFirstVorbisBook that defined by actions+++++\n");
            	return BAD_VALUE;
            }
				 		sp<ABuffer> buffer1 = new ABuffer(size);
				 		memcpy(buffer1->data(), data, size);
            csdsize+=size;
            if(!meta->findData(kKeyVorbisBooks, &type, &data, &size)){
            	CODEC_LOGI("++++missing kKeyVorbisBooks+++++\n");
            	return BAD_VALUE;
            }
						sp<ABuffer> buffer2 = new ABuffer(size);
				 		memcpy(buffer2->data(), data, size);
	          csdsize+=size;
	          sp<ABuffer> csdbuffer = new ABuffer(csdsize+36);
						GetVorbisSpecificData(buffer,buffer1,buffer2,csdbuffer);
     	 			addCodecSpecificData(csdbuffer->data(),csdbuffer->size());
     	 	}else{
            	addCodecSpecificData(data, size);
    ...
    if (!strcasecmp(MEDIA_MIMETYPE_AUDIO_AMR_NB, mMIME)) {
        ...
        // a variable number of channels.
         /*ActionsCode(author:sunchengzhi,sampleRate can be variable  )*/
        int32_t numChannels, sampleRate;
        CHECK(meta->findInt32(kKeyChannelCount, &numChannels));
		CHECK(meta->findInt32(kKeySampleRate, &sampleRate));
    ...
    if (!strncasecmp(mMIME, "video/", 6)) {
        ...
        if (mIsEncoder) {
            setVideoInputFormat(mMIME, meta);
        	 ...
        	 /*ActionsCode(author:sunchengzhi,setParameter OMX_IndexThumbnail to notify video compoonent 
        	 that thumbnail is decoded except video)*/
        	 if((mFlags&kClientNeedsFramebuffer)  && strcmp(mMIME,MEDIA_MIMETYPE_VIDEO_VP9) && (!strcmp(mComponentName, ACTIONS_VIDEO_DECODER)|| !strcmp(mComponentName, ACTIONS_VIDEO_DECODER_DEINTERLACE))){	 
        		 int param;
        		 mOMX->setParameter(
        		 mNode, (OMX_INDEXTYPE)OMX_IndexThumbnail, &param, sizeof(param));
        	 }
    ...
    if (meta->findInt32(kKeyMaxInputSize, &maxInputSize)) {
    	  if(!strcmp(mComponentName, ACTIONS_VIDEO_DECODER) || !strcmp(mComponentName, ACTIONS_VIDEO_DECODER_DEINTERLACE) ){
    	  	if(maxInputSize < 64 * 1024){
    	  		 maxInputSize = 64 *1024 + 256;
    	  	}else{
    	  		 maxInputSize += 256 ;
    	  	}
    	  }
...
#if 1
        CODEC_LOGI("portIndex: %u, index: %u, eCompressionFormat=%d eColorFormat=%d",
             portIndex,
...
/**
  *
  ********bugfix for 720p videoRecorder Huaping when  driver do not support 720p****************************
  *      
  *ActionsCode(author:sunchengzhi, change_code)
  */
void OMXCodec::setVideoInputFormat(
	...
	/*ActionsCode(author:sunchengzhi, bugfix for 720p videoRecorder Huaping start)*/ 
    ...
    if(mIsEncoder && mIsVideo){
    	...
    	ALOGD("preview width b4... %d",frameWidthActual);
    	if(ret == 0){
    		frameWidthActual = width;
    		frameHeightActual = height;
    	}
    	ALOGD("preview width %d",frameWidthActual);
    }
	/*ActionsCode(author:sunchengzhi, bugfix for 720p videoRecorder Huaping end)*/ 
    CHECK(success);
    CHECK(stride != 0);
...
/**
  *use actions video hardware decoders
  ************************************
  *      
  *ActionsCode(author:sunchengzhi, change_code)
  */
status_t OMXCodec::setVideoOutputFormat(
 ...
 //ActionsCode(author:sunchengzhi,use actions video hardware decoders instead of android original decoders)
    OMX_VIDEO_CODINGTYPE compressionFormat = OMX_VIDEO_CodingUnused;
 if(!strcmp(mComponentName, ACTIONS_VIDEO_DECODER) || !strcmp(mComponentName, ACTIONS_VIDEO_DECODER_DEINTERLACE) ){
	 if (!strcasecmp(MEDIA_MIMETYPE_VIDEO_AVC, mime)) {
		 compressionFormat = OMX_VIDEO_CodingAVC;
		 ...
		 compressionFormat = OMX_VIDEO_CodingMPEG4;
	 } else if (!strcasecmp(MEDIA_MIMETYPE_VIDEO_ACT_RV, mime)) {
	 ...
	 } else if(!strcasecmp(MEDIA_MIMETYPE_VIDEO_ACT_VP6, mime)){
			 compressionFormat = (OMX_VIDEO_CODINGTYPE)OMX_VIDEO_CodingVP6;
	 }
	 else if (!strcasecmp(MEDIA_MIMETYPE_VIDEO_MPEG2, mime)) {
		 compressionFormat = OMX_VIDEO_CodingMPEG2;
	 } else if (!strcasecmp(MEDIA_MIMETYPE_VIDEO_ACT_DIV3, mime)) {
		 compressionFormat = (OMX_VIDEO_CODINGTYPE)OMX_VIDEO_CodingDIV3;
	 } else if (!strcasecmp(MEDIA_MIMETYPE_VIDEO_ACT_FLV1, mime)) {
		  compressionFormat = (OMX_VIDEO_CODINGTYPE)OMX_VIDEO_CodingFLV1;
	 } else if(!strcasecmp(MEDIA_MIMETYPE_VIDEO_ACT_VC1, mime)) {
		 compressionFormat = (OMX_VIDEO_CODINGTYPE)OMX_VIDEO_CodingVC1;
	 }else if(!strcasecmp(MEDIA_MIMETYPE_VIDEO_ACT_WMV8, mime)) {
		 compressionFormat = (OMX_VIDEO_CODINGTYPE)OMX_VIDEO_CodingWMV2;
     }else if(!strcasecmp(MEDIA_MIMETYPE_VIDEO_ACT_AVS, mime)) {
		 compressionFormat = (OMX_VIDEO_CODINGTYPE)OMX_VIDEO_CodingAVS;
	 }else if(!strcasecmp(MEDIA_MIMETYPE_VIDEO_VP9, mime)) {
		 compressionFormat = OMX_VIDEO_CodingVP9;
	 }else if (!strcasecmp(MEDIA_MIMETYPE_VIDEO_ACT_RVG2, mime)) {
		 compressionFormat = (OMX_VIDEO_CODINGTYPE)OMX_VIDEO_CodingRVG2;
	 }else if (!strcasecmp(MEDIA_MIMETYPE_VIDEO_ACT_MJPG, mime)) {
		 compressionFormat = OMX_VIDEO_CodingMJPEG;
    }else if (!strcasecmp(MEDIA_MIMETYPE_VIDEO_HEVC, mime)) {
        compressionFormat =  OMX_VIDEO_CodingHEVC;
    }else if (!strcasecmp(MEDIA_MIMETYPE_VIDEO_ACT_HEVC_91, mime)) {
        compressionFormat =  (OMX_VIDEO_CODINGTYPE)OMX_VIDEO_CodingHEVC_91;
	 }else {
		 ALOGE("Not a supported video mime type: %s", mime);
		 CHECK(!"Should not be here. Not a supported video mime type.");
	 }
...
#if 1
    {
		...
		/*ActionsCode(author:sunchengzhi, bugfix BUG00126415 start)
		*****pass xFramerate to video decoder componet to ask decoder to discard B-Frames 
		******when Performance is not enough for 60fps 1080p
		*/
        if(!strcmp(mComponentName, ACTIONS_VIDEO_DECODER) || !strcmp(mComponentName,ACTIONS_VIDEO_DECODER_DEINTERLACE) ){
    	  	int32_t fps=0;
        	meta->findInt32(kKeyFrameRate, &fps);        	
    	  	format.xFramerate = (fps<<16);
        }
		/*ActionsCode(author:sunchengzhi, bugfix BUG00126415 end)*/
    ...
    def.nBufferSize =
        (((width + 15) & -16) * ((height + 15) & -16) * 3) / 2;  // YUV420
 		...
 		/*ActionsCode(author:sunchengzhi,memory optimization except vp9
 		  for decoding thumbnail,in which caese,only three output buffers 
 		  is need in video decoder component)*/
    if((mFlags&kClientNeedsFramebuffer) && strcmp(mime,MEDIA_MIMETYPE_VIDEO_VP9) && (!strcmp(mComponentName, ACTIONS_VIDEO_DECODER)|| !strcmp(mComponentName, ACTIONS_VIDEO_DECODER_DEINTERLACE))){		 
...
void OMXCodec::setComponentRole(
    ...
    static const MimeToRole kMimeToRole[] = {
        ...
        { MEDIA_MIMETYPE_AUDIO_ACT_AAC,
            AUDDEC_COMPONENTROLES_AAC, ""}, /*ActionsCode(author:jinsongxue, use actions aac decoder bug00243946)*/
        ...
        { MEDIA_MIMETYPE_VIDEO_ACT_DIV3,
          VIDDEC_COMPONENTROLES_DIV3, "" },
        { MEDIA_MIMETYPE_VIDEO_ACT_VP6,
          VIDDEC_COMPONENTROLES_VP6, "" },
        { MEDIA_MIMETYPE_VIDEO_ACT_RVG2,
          VIDDEC_COMPONENTROLES_RVG2, "" },
        { MEDIA_MIMETYPE_VIDEO_ACT_FLV1,
          VIDDEC_COMPONENTROLES_FLV1, "" },
        { MEDIA_MIMETYPE_VIDEO_ACT_RV,
          VIDDEC_COMPONENTROLES_RV,"" },
        { MEDIA_MIMETYPE_VIDEO_ACT_VC1,
          VIDDEC_COMPONENTROLES_VC1,"" },
        { MEDIA_MIMETYPE_VIDEO_ACT_HEVC_91,
          VIDDEC_COMPONENTROLES_HEVC_91,"" }, 
        { MEDIA_MIMETYPE_VIDEO_ACT_WMV8,
          VIDDEC_COMPONENTROLES_WMV2,"" },
        { MEDIA_MIMETYPE_VIDEO_ACT_AVS,
          VIDDEC_COMPONENTROLES_AVS,"" },
    };
...
OMXCodec::~OMXCodec() {
    ...
    free(mMIME);
    mMIME = NULL;
    if(mActCodecSpecificData!=NULL){
    	free(mActCodecSpecificData);
    	mActCodecSpecificData = NULL;
    }
...
status_t OMXCodec::allocateBuffersOnPort(OMX_U32 portIndex) {
    ...
    for (OMX_U32 i = 0; i < def.nBufferCountActual; ++i) {
		...
		//ActionsCode(author:rongxing, bugfix:BUG00283278)
		info.mNetWorkTimeOut = false;
...
status_t OMXCodec::allocateOutputBuffersFromNativeWindow() {
    ...
    {
    	case OMX_COLOR_FormatYUV420Planar:
				grallocColorFormat =  HAL_PIXEL_FORMAT_ACT_YU12;
				break;
			case OMX_COLOR_FormatYUV420SemiPlanar:
     ...
     /*ActionsCode(author:sunchengzhi,distinguish if hde exist)*/ 
    int hde_fd = -1;
    int alignh = 15;
    char* d="/dev/hde";
    ...
    if(!strcasecmp(mMIME, "video/hevc")) {
    	if(hde_fd>=0){
    		alignh = 7;
    		ALOGD("hde exist,need 8 bytes aligned\n");    		
    	}
    }   
    ...
    /*ActionsCode(author:sunchengzhi,GPU at least need width and height 16 aligned)*/
    err = native_window_set_buffers_geometry(
	            mNativeWindow.get(),
              (def.format.video.nFrameWidth + 15)&(~15),
              (def.format.video.nFrameHeight+ alignh)&(~alignh),
              grallocColorFormat);
    ...
    /*ActionsCode(author:sunchengzhi,actions softwareCodec use gralloc cache buffer to upgrade performance)*/    
    if(!strcasecmp(mMIME, "video/hevc_91") || !strcasecmp(mMIME, "video/x-vnd.on2.vp9") || !strcasecmp(mMIME, "video/wmv8") || !strcasecmp(mMIME, "video/avs")){ //fix by actions software video codec
		...
		if(!strcasecmp(mMIME, "video/hevc")){
			if(hde_fd<0){
				 usage |=GRALLOC_USAGE_SW_WRITE_OFTEN;
				 ALOGD("===hde not exist,use hevc software===\n");
			}else{
				usage |=GRALLOC_USAGE_HW_VIDEO_H265;
				ALOGD("===hde exist,use hde haraware codec===\n");
			}
   	...
   	if(hde_fd>=0){
  		...
  		if(pp_enabled==true){
  			...
  			if(!strcasecmp(mMIME, "video/hevc") && def.format.video.nFrameWidth*def.format.video.nFrameHeight>1920*1088){
   				usage |= GRALLOC_USAGE_HW_VIDEO_PP;
   				ALOGD("hevc enable pp\n");
   			}
 
   			if(!strcasecmp(mMIME, "video/avc") && def.format.video.nFrameWidth*def.format.video.nFrameHeight>1920*1088){
   				usage |= GRALLOC_USAGE_HW_VIDEO_PP;
   				ALOGD("avc enable pp\n");
   			}
  		}
 ...
 /*ActionsCode(author:sunchengzhi,actions video decoder component does not support extra buffer)*/ 
#if 0
    ...
    switch (msg.type) {
        ...
        case omx_message::EMPTY_BUFFER_DONE:
        {
            ...
            if (mPortStatus[kPortIndexInput] == DISABLING) {
                CODEC_LOGI("Port is disabled, freeing buffer on port:%d, %u",kPortIndexInput, buffer);
        ...
        case omx_message::FILL_BUFFER_DONE:
        {
            ...
            if (mPortStatus[kPortIndexOutput] == DISABLING) {
                CODEC_LOGI("Port is disabled, freeing buffer %u", buffer);
                ...
                if (mTargetTimeUs >= 0) {
                    ...
                    mTargetTimeUs = -1;
                } //bug fixed for actions video decoder
								if(!strcasecmp(mComponentName, ACTIONS_VIDEO_DECODER) ||!strcasecmp(mComponentName, ACTIONS_VIDEO_DECODER_DEINTERLACE)){
									if (!(msg.u.extended_buffer_data.flags & OMX_BUFFERFLAG_DECODEONLY)){
...
/**
 * 2014-12-19 BUGFIX: BUG00234982 when receive OMX_EventBufferFlag
 *                   event we tigger the mBufferFilled condition to notify EOS
 *                   status
 ************************************
 *      
 *ActionsCode(author:rongxing, change_code)
...
void OMXCodec::onEvent(OMX_EVENTTYPE event, OMX_U32 data1, OMX_U32 data2) {
    switch (event) {
        ...
        {
            CODEC_LOGI("OMX_EventPortSettingsChanged(port=%u, data2=0x%08x)",
                       data1, data2);

            if (data2 == 0 || data2 == OMX_IndexParamPortDefinition) {
            	if (data1 == kPortIndexOutput) { //bug fixed for actions video decoder
            		if(!strcmp(mComponentName, ACTIONS_VIDEO_DECODER) || !strcmp(mComponentName, ACTIONS_VIDEO_DECODER_DEINTERLACE)){
        ...
        case OMX_EventBufferFlag:
        {
            ...
            if (data1 == kPortIndexOutput && !mIsEncoder) {
                mNoMoreOutputData = true;
				//ActionsCode(rongxing, bugfix BUG00234982), 
				mBufferFilled.signal();
            }
            break;
        }
...
void OMXCodec::onCmdComplete(OMX_COMMANDTYPE cmd, OMX_U32 data) {
    switch (cmd) {
        ...
        case OMX_CommandPortDisable:
        {
            ...
            CODEC_LOGI("PORT_DISABLED(%u)", portIndex);
        ...
        case OMX_CommandFlush:
        {
            ...
            CODEC_LOGI("FLUSH_DONE(%u)", portIndex);
            ...
            if (mState == RECONFIGURING) {
                ...
                if (mPortStatus[kPortIndexInput] == ENABLED
                    && mPortStatus[kPortIndexOutput] == ENABLED) {
                    CODEC_LOGI("Finished flushing both ports, now continuing from"
                         " seek-time.");
...
bool OMXCodec::flushPortAsync(OMX_U32 portIndex) {
    ...
    CODEC_LOGI("flushPortAsync(%ld): we own %d out of %d buffers already.",
...
void OMXCodec::disablePortAsync(OMX_U32 portIndex) {
    ...
    CODEC_LOGI("sending OMX_CommandPortDisable(%ld)", portIndex);
    status_t err =
...
bool OMXCodec::drainInputBuffer(BufferInfo *info) {
    bool audioclearts = false;//for actions audio decoder
    if (info != NULL) {
        CHECK_EQ((int)info->mStatus, (int)OWNED_BY_US);
    }
    ...
    if((!strcasecmp(mComponentName, ACTIONS_VIDEO_DECODER) ||!strcasecmp(mComponentName, ACTIONS_VIDEO_DECODER_DEINTERLACE)) \
    	&& (!strcasecmp(mMIME,MEDIA_MIMETYPE_VIDEO_AVC) || !strcasecmp(mMIME,MEDIA_MIMETYPE_VIDEO_HEVC) || !strcasecmp(mMIME,MEDIA_MIMETYPE_VIDEO_ACT_HEVC_91))\
    	&& mCodecSpecificDataIndex < mCodecSpecificData.size()){
    	int i=0;
    	size_t size = 0;
    	static const uint8_t kNALStartCode[4] =
                    { 0x00, 0x00, 0x00, 0x01 };
    	for(i=0;i<mCodecSpecificData.size();i++){
    		const CodecSpecificData *specific = mCodecSpecificData[mCodecSpecificDataIndex];
    		memcpy(info->mData+size, kNALStartCode, 4);
    		size += 4;
    		memcpy((uint8_t *)info->mData + size,
    ...
    //actions_code(lishiyuan, fix BUG00214124)
    if (mPaused) {
		//ALOGE("xxxxxxxxxx mPaused true xxx");
		if (mSeekTimeUs >= 0){
			ALOGE("XXXXXXXXXXXX mSeekTimeUs >=0 ");
            return false;
		}
    }
    status_t err;
    ...
    for (;;) {
        ...
        if (mSeekTimeUs >= 0) {
            ...
            mLeftOverBuffer = NULL;
            if(!strcasecmp(mComponentName, ACTIONS_AUDIO_DECODER)){
            	audioclearts = true;
            }
        ...
        /*ActionsCode(author:sunchengzhi, bugfix:BUG00241678)*/
        if(err==ERROR_NETWORK_READ_TIMEOUT){
        	CODEC_LOGI("ERROR_NETWORK_READ_TIMEOUT\n");
			//ActionsCode(author:rongxing, bugfix:BUG00283278)
        	info->mNetWorkTimeOut = true;
        	return false;
		}
		//ActionsCode(author:rongxing, bugfix:BUG00283278)
		info->mNetWorkTimeOut = false;
        ...
        if (!strcasecmp(MEDIA_MIMETYPE_AUDIO_VORBIS, mMIME)) {
			...
			if(strcmp(mComponentName, ACTIONS_AUDIO_DECODER)){/*not need this parameter JinSongXue*/
    ...
    if (signalEOS) {
        flags |= OMX_BUFFERFLAG_EOS;
        CODEC_LOGI("signalEOS,mFinalStatus:%d,mSignalledEOS:%d\n",mFinalStatus,mSignalledEOS);
    } else {
        mNoMoreOutputData = false;
    }
   ...
   if(audioclearts){ // for actions audio decoder
       timestampUs =0ll;
   }
...
status_t OMXCodec::waitForBufferFilled_l() {
    ...
    if (err != OK) {
        ...
        err = ERROR_NETWORK_READ_TIMEOUT;
...
/**
  *
  ********sometimes ,sampleRate is variable,so do not fix sampleRate to 8000,add a param for actual sampleRate******************
  *      
  *ActionsCode(author:sunchengzhi, change_code)
  */
void OMXCodec::setG711Format(int32_t numChannels, int32_t sampleRate) {
    ...
    ALOGD("G711 sampleRate is %d",sampleRate);
    setRawAudioFormat(kPortIndexInput, sampleRate, numChannels);
}
...
status_t OMXCodec::stopOmxComponent_l() {
    CODEC_LOGI("stopOmxComponent_l mState=%d", mState);
    ...
    switch (mState) {
        ...
        case EXECUTING:
        {
            ...
            if (mQuirks & kRequiresFlushBeforeShutdown) {
                CODEC_LOGI("This component requires a flush before transitioning "
            ...
            CODEC_LOGI("===wait loaded start ,now mState is :%d===\n",mState);
            while (mState != LOADED && mState != ERROR) {
                mAsyncCompletion.wait(mLock);
            }
            CODEC_LOGI("===wait loaded end ,now mState is :%d===\n",mState);
            if (isError) {
...
/**
  ...
  *ActionsCode(author:rongxing, change_code)
  */
bool OMXCodec::isNetworkTimeout(){
	Vector<BufferInfo> *buffers = &mPortBuffers[kPortIndexInput];
    size_t i = 0;
    while (i < buffers->size()) {
		if((*buffers)[i].mNetWorkTimeOut){
			CODEC_LOGV("(buffer: %p) isNetworkTimeout",(*buffers)[i].mBuffer);
			return true;
		}
    	++i;
    }
...
status_t OMXCodec::read(
    ...
    *buffer = NULL;
    ...
    /*ActionsCode(author:sunchengzhi, bugfix:BUG00241678)*/
	//ActionsCode(author:rongxing, bugfix:BUG00283278)
	if(isNetworkTimeout() && !mIsEncoder){
		if(!seeking){
			drainInputBuffers();
		}
		if(mFilledBuffers.empty() && isNetworkTimeout()){
			return ERROR_NETWORK_READ_TIMEOUT;
		}
	}
...
/**
 *BUGFIX: BUG000107012
 *Recorder: Recorded AAC file cant be played. Due to original Android code transmit 
 * wrong AAC profile parameter. 
 ************************************
 *      
        ...
        case OMX_PortDomainAudio:
        {
            ...
            if (audio_def->eEncoding == OMX_AUDIO_CodingPCM) {
                ...
                if(!strcmp(mComponentName, ACTIONS_AUDIO_DECODER) )
                {
                    ...
                    /* ActionsCode(author:sunchengzhi, special change_code for actions audio decoders)*/
										if( !strcasecmp(mMIME, MEDIA_MIMETYPE_AUDIO_AAC) &&  sampleRate <= 24000)
										{
											sampleRate *=2;	
										}
                  	if(numChannels>2) 
                ...
                //ActionsCode(author:yuchen, change_code): enforce an AAC profile if not found.
                // cz_20131209 sync 4.2 fix BUG00107012, aac recorder error  need aac profile
        ...
        case OMX_PortDomainVideo:
        {
            ...
            if (!mIsEncoder) {
                ...
                if (err == OK) {
                  ...
                  /*ActionsCode(author:sunchengzhi, Because actions video hardware decoder width is 32 aligned,and height is 16 alinged,CHECK_LE will err when getConfig is failed,so we ajust the check value)*/
                  CHECK_LE(rect.nLeft + rect.nWidth - 1, (video_def->nFrameWidth+31)&(~31));
                  CHECK_LE(rect.nTop + rect.nHeight - 1, (video_def->nFrameHeight+15)&(~15));
