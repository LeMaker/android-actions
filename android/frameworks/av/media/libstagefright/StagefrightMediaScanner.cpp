...
//ActionsCode(authro:songzhining, comment: add extension for media)
...
MediaScanResult StagefrightMediaScanner::processFileInternal(
    ...
    if (status) {
        ...
        //ActionsCode(authro:songzhining, comment: fix BUG00126950, ignore error .dat file)
        if (!strcasecmp(extension, ".dat")) {
            client.setMimeType("");
            ALOGD("Ignore error (.dat) file!\n");
        }
