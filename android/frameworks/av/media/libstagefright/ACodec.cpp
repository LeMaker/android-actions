...
status_t ACodec::configureOutputBuffersFromNativeWindow(
    ...
    if(!strcasecmp(mMIME, "video/hevc_91") || !strcasecmp(mMIME, "video/vp9") || !strcasecmp(mMIME, "video/wmv8")){ //fix by actions software video codec
    ...
    /*ActionsCode(author:sunchengzhi,add GRALLOC_USAGE_HW_VIDEO )*/
...
/*ActionsCode(author:sunchengzhi, add video_decoder codecs that are not supported by android,but supported by actions)*/
...
/*ActionsCode(author:sunchengzhi, add audio_decoder codecs that are not supported by android,but supported by actions)*/		  
         ...
         {MEDIA_MIMETYPE_AUDIO_ACT_PCM,
            "audio_decoder.pcm", "audio_encoder.pcm" },		
    ...
    /*ActionsCode(author:sunchengzhi,back-up the mime for usage)*/
    mMIME = strdup(mime);
...
status_t ACodec::configureCodec(
    ...
    if (video) {
    ...
    } else if (!strcasecmp(mime, MEDIA_MIMETYPE_AUDIO_ACT_PCM)){
        int32_t numChannels;
        int32_t sampleRate;
        if (!msg->findInt32("channel-count", &numChannels)
                || !msg->findInt32("sample-rate", &sampleRate)) {
        ...
        ALOGD("set pcm output:%d",encoder);
           err = setupRawAudioFormat(encoder ? kPortIndexInput : kPortIndexOutput, sampleRate, numChannels);
        }
    } else if (!strcasecmp(mime, MEDIA_MIMETYPE_AUDIO_ACT_ACELP)) {
        int32_t numChannels;
        int32_t sampleRate;
        if (!msg->findInt32("channel-count", &numChannels)
                || !msg->findInt32("sample-rate", &sampleRate)) {
        ...
        ALOGD("set acelp output1:%d",encoder);
           err = setupRawAudioFormat(encoder ? kPortIndexInput : kPortIndexOutput, sampleRate, numChannels);
        }
    } else if (!strcasecmp(mime, MEDIA_MIMETYPE_AUDIO_ACT_COOK)) {
        int32_t numChannels;
        int32_t sampleRate;
        if (!msg->findInt32("channel-count", &numChannels)
                || !msg->findInt32("sample-rate", &sampleRate)) {
        ...
        ALOGD("set cook output1:%d",encoder);
           err = setupRawAudioFormat(encoder ? kPortIndexInput : kPortIndexOutput, sampleRate, numChannels);
        }
    } else if (!strcasecmp(mime, MEDIA_MIMETYPE_AUDIO_ACT_WMAPRO)) {
        int32_t numChannels;
        int32_t sampleRate;
        if (!msg->findInt32("channel-count", &numChannels)
                || !msg->findInt32("sample-rate", &sampleRate)) {
        ...
        ALOGD("set wmapro output1:%d",encoder);
           err = setupRawAudioFormat(encoder ? kPortIndexInput : kPortIndexOutput, sampleRate, numChannels);
        }
    } else if (!strcasecmp(mime, MEDIA_MIMETYPE_AUDIO_ACT_DTS)) {
        int32_t numChannels;
        int32_t sampleRate;
        if (!msg->findInt32("channel-count", &numChannels)
                || !msg->findInt32("sample-rate", &sampleRate)) {
        ...
        ALOGD("set dts output1:%d",encoder);
           err = setupRawAudioFormat(encoder ? kPortIndexInput : kPortIndexOutput, sampleRate, numChannels);
        }
    ...
    if (msg->findInt32("max-input-size", &maxInputSize)) {
    	  /*ActionsCode(author:sunchengzhi,somethimes ,actions video decoder component will add packet_header_t in buffer header,so enlarge it)*/
    	  if(video && !encoder){
...
status_t ACodec::setupRawAudioFormat(
   ...
   if(!strcmp(mComponentName.c_str(),"OMX.Action.Audio.Decoder")){
    	if(numChannels>2 ){
    		numChannels =2;
			}
		}
...
/*ActionsCode(author:sunchengzhi, add codecs that are not supported by android,but supported by actions)*/
    { MEDIA_MIMETYPE_VIDEO_ACT_VP6, OMX_VIDEO_CodingVP6 }, 
    { MEDIA_MIMETYPE_VIDEO_ACT_FLV1,OMX_VIDEO_CodingFLV1}, 
    { MEDIA_MIMETYPE_VIDEO_ACT_MJPG,OMX_VIDEO_CodingMJPEG}, 
    { MEDIA_MIMETYPE_VIDEO_ACT_DIV3,OMX_VIDEO_CodingDIV3}, 
    { MEDIA_MIMETYPE_VIDEO_ACT_VC1,OMX_VIDEO_CodingVC1}, 
    { MEDIA_MIMETYPE_VIDEO_ACT_RV,OMX_VIDEO_CodingRV}, 
    { MEDIA_MIMETYPE_VIDEO_ACT_AVS,OMX_VIDEO_CodingAVS},
    {MEDIA_MIMETYPE_VIDEO_ACT_HEVC_91,OMX_VIDEO_CodingHEVC_91},
    { MEDIA_MIMETYPE_VIDEO_ACT_WMV8,OMX_VIDEO_CodingWMV2},
    { MEDIA_MIMETYPE_VIDEO_ACT_RVG2,OMX_VIDEO_CodingRVG2},
    { "video/hevc_actions", OMX_VIDEO_CodingHEVC },
    { "video/vp9_actions",  OMX_VIDEO_CodingVP9 },
...
status_t ACodec::setupVideoEncoder(const char *mime, const sp<AMessage> &msg) {
   ...
   /*ActionsCode(author:sunchengzhi, bugfix for 720p videoRecorder Huaping start)*/ 
    int32_t frameWidthActual=0;
    int32_t frameHeightActual=0;
    int32_t stride;
    if(msg->findInt32("pwdt",&frameWidthActual) && msg->findInt32("phgt",&frameHeightActual)){
...
status_t ACodec::setupAVCEncoderParameters(const sp<AMessage> &msg) {
    ...
    if (!msg->findFloat("frame-rate", &frameRate)) {
        ...
        /*ActionsCode(author:sunchengzhi,fix cts tests for android.media.cts.EncodeVirtualDisplayWithCompositionTest and android.media.cts.EncodeVirtualDisplayTest)*/
        if(tmp != 29 && (iFrameInterval == 10 || iFrameInterval == 8640000)){
        	OMX_INDEXTYPE index;
           const char *name = "OMX.actions.android.index.MediaFormatConvert";
           status_t err_tmp = mOMX->getExtensionIndex(mNode,name, &index);		
           if (err_tmp == OK) {
               ALOGE("OMX_GetExtensionIndex %s succssed", name);
                OMX_ACT_CONFIG_CommonParams act_comparams;
                memset(&act_comparams, 0, sizeof(act_comparams));
                act_comparams.nSize = sizeof(act_comparams);
                
                // Version: 1.0.0.0
                act_comparams.nVersion.s.nVersionMajor = 1;					
                act_comparams.nPortIndex = kPortIndexInput;
                act_comparams.bEnable = OMX_TRUE;
                mOMX->setParameter(mNode, index, &act_comparams, sizeof(act_comparams));							
           }
        }
        ...
        case OMX_PortDomainVideo:
        {
            ...
            switch ((int)videoDef->eCompressionFormat) {
                case OMX_VIDEO_CodingUnused:
                {
                  ...
                  //  CHECK_LE(rect.nLeft + rect.nWidth - 1, (videoDef->nFrameWidth+31)&(~31));
                  //  CHECK_LE(rect.nTop + rect.nHeight - 1, (videoDef->nFrameHeight+15)&(~15));
...
bool ACodec::BaseState::onOMXFillBufferDone(
    ...
    switch (mode) {
        ...
        case RESUBMIT_BUFFERS:
        {
            ...
            if(flags & OMX_BUFFERFLAG_DECODEONLY){ //fix by actions
            	ALOGD("[%s] discard flush buffer ts:%lld\n",
                     mCodec->mComponentName.c_str(),timeUs);
            	break;
            }
...
bool ACodec::UninitializedState::onAllocateComponent(const sp<AMessage> &msg) {
    ...
    if (msg->findString("componentName", &componentName)) {
        ...
        uint32_t flags = 0;
         /*ActionsCode(author:sunchengzhi,for vp9 and hevc video deocders in Acodec,
         we pass OMXCodec::kPreferSoftwareCodecs to OMXCodec::findMatchingCodecs 
         that select android original decoders*/

        if(!encoder && (!strcmp(mime.c_str(),MEDIA_MIMETYPE_VIDEO_VP9) || !strcmp(mime.c_str(),MEDIA_MIMETYPE_VIDEO_HEVC))){
        	flags = OMXCodec::kPreferSoftwareCodecs;
        }
				if(!encoder && !strncmp(mime.c_str(),"audio/",6)){
           flags = OMXCodec::kPreferSoftwareCodecs; 
		  		 ALOGI("Audio codec Softwarecodec");
				}
