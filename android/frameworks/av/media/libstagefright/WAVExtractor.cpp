...
////////////////////////////////////////////////////////////////////////////////
/*
  * Distinguish between wav with dts format.
  ************************************
  *      
...
bool SniffWAV(
    ...
    /*ActionsCode(author:jinsongxue, checkout DTS format)*/
	int ret = 0;
	...
	init_storage_io(storage_io, source);
	ret = format_check(storage_io, mime_type);
	dispose_storage_io(storage_io);
	if (mime_type[0] == '\0' || ret != 0){
		ALOGE("media content detected error !!!");
	}
	else if(!memcmp(mime_type, "DTS", 3)){
		return false;
	}	
