...
/**
  *
  ********for calculate right timestamp******************
  *      
  *ActionsCode(author:jingsongxue, change_code)
  */
void SoftVorbis::onQueueFilled(OMX_U32 portIndex) {
    ...
    while ((!inQueue.empty() || (mSawInputEos && !mSignalledOutputEos)) && !outQueue.empty()) {
        ...
        if (inHeader) {
            ...
            if (inHeader->nFilledLen || !mSawInputEos) {
                ...
                if (inHeader->nOffset == 0 && inHeader->nTimeStamp) {/*ActionsCode(author:jingsongxue, change_code)*/
  ...
  /*
  *fix android5.1 CTS item testDecodeWithEOSOnLastBuffer.      
  *ActionsCode(author:jingsongxue, change_code)
  */
void SoftVorbis::onPortFlushCompleted(OMX_U32 portIndex) {
    ...
    mAnchorTimeUs = 0; /*ActionsCode(author:jingsongxue, need to reset.)*/
}
