...
/**
  *sometimes,aac input port nbuffersize is not big enough,so we ajust both input and output nbuffersize to double size
  ************************************
  *      
...
void SoftAAC2::initPorts() {
    ...
    /*ActionsCode(author:sunchengzhi, sometimes,input port nbuffersize 8092 is not big enough,so we double it to 16384)*/
    ...
    def.nBufferCountActual = def.nBufferCountMin;
   /*ActionsCode(author:sunchengzhi, Becase input port nbuffersize is doubled,so output port nbuffersize is also doubled)*/
    def.nBufferSize = 8192 * MAX_CHANNEL_COUNT;
    def.bEnabled = OMX_TRUE;
    def.bPopulated = OMX_FALSE;
    def.eDomain = OMX_PortDomainAudio;
    def.bBuffersContiguous = OMX_FALSE;
...
/**
  *sometimes,aac parser package data is more than one frame length, its timestamp needs to adjust accordingly,
  *or some video is out of sync with AAC's timestamp.
  **Since AAC decoder in calculating the time stamp of the time does't limit with non-zero conditions, division by zero occurs, 
  * leading to MediaServer to quit unexpectedly.
  ************************************
  *      
    ...
    while ((!inQueue.empty() || mEndOfInput) && !outQueue.empty()) {
        if (!inQueue.empty()) {
            ...
            if ((inHeader->nFlags & OMX_BUFFERFLAG_CODECCONFIG) != 0) {
				...
				inBuffer[0][0] = (inBuffer[0][0]&0x7)|0x10;
			...
			if (inHeader->nOffset == 0) {
	            if(inHeader->nTimeStamp!=0) //cz_20131209 sync 4.2 bug fix add 3GPP-awb and android AAC fix tudou video
	            {
        ...
        while (!outQueue.empty()
			...
			/*ActionsCode(author:jinsongxue, in case of devision by zero occured)*/
			if( (mStreamInfo->sampleRate>0)&&(mStreamInfo->numChannels>0) )
			{
            //outHeader->nTimeStamp = currentTime;
            outHeader->nTimeStamp =
                mAnchorTimeUs
                    + (mNumSamplesOutput * 1000000ll) / mStreamInfo->sampleRate;
			/*ActionsCode(author:jinsongxue, aac parser package data is more than one frame length)*/
            mNumSamplesOutput += (numSamples/mStreamInfo->numChannels);            
			}
            mOutputBufferCount++;
