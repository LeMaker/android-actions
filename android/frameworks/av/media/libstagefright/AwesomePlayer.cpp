...
/*ActionsCode(author:sunchengzhi,adaptive drop B-Frames when video is late than audio)*/
#define ADAPTIVE_SKIP_BFRAME 1
#if ADAPTIVE_SKIP_BFRAME
#define LOW_WATER_FOR_SKIP_BFRAME 		-40000 // 0
#define HIGH_WATER_FOR_SKIP_BFRAME 	0 			//40000
#endif

/*ActionsCode(author:sunchengzhi, sometimes,software video decoders performance
 is not enough,so,video is always late audio >40000us.In this situation,video
 is always dropped.we change FRAME_DROP_FREQ to 2,so that we drop one frame 
 every two frames when video is late audio>40000us)*/
#define FRAME_DROP_FREQ 2
extern void TimeCompensation(int64_t *time);
namespace android {
...
/**
  *************add mSubTrackIndexSelected,mAudioTrackIndexSelected for cts test***********************
  *      
...
AwesomePlayer::AwesomePlayer()
    : mQueueStarted(false),
      ...
      mIs4kVideo(false),
      mSubTrackIndexSelected(-1),
      mAudioTrackIndexSelected(-1){
...
/**
  *2014-11-27 added support for actions inband subtitle
  *2014-12-19 fix BUG00235024 .skip the track whose
  *                 format is unsupport
  ************************************
  *      
  *ActionsCode(author:sunchengzhi, change_code)
...
status_t AwesomePlayer::setDataSource_l(const sp<MediaExtractor> &extractor) {
    ...
    for (size_t i = 0; i < extractor->countTracks(); ++i) {
        ...
        if (!haveVideo && !strncasecmp(mime.string(), "video/", 6)) {			
			...
			/*ActionsCode(author:sunchengzhi,  fix BUG00235024 skip unsupport audio format)*/
        	if (!(strncasecmp(mime.string(), "audio/unsupport", sizeof("audio/unsupport")))) {
        		ALOGW("skip: unsupport audio format(%s)!!!", mime.string());
				haveAudio = false;
				continue;
			}
        ...
        } else if (!strcasecmp(mime.string(), MEDIA_MIMETYPE_TEXT_ACT)) {//ActionsCode(author:sunchengzhi,added support for actions inband subtile)
...
void AwesomePlayer::reset() {
    if(mCachedSource!= NULL){
    	 if ((mFlags & PREPARING) && !mDisconnecting){
    	 	mCachedSource->disconnect();
    	 	mDisconnecting = true;
    	 }
    	 mCachedSource->enableTimedout();
    }
    Mutex::Autolock autoLock(mLock);
    reset_l();
    if(mCachedSource!= NULL){
    			mCachedSource->disableTimedout();
    }
}


/**
  *stop actvideoextraor thread for actions Extractor before videoplayer quit
  * 2015-07-30 bugfix:BUG00335333
  ************************************
  *      
  *ActionsCode(author:sunchengzhi, change_code)
  */
void AwesomePlayer::reset_l() {
    ...
    mDisplayWidth = 0;
    mDisplayHeight = 0;
	/*ActionsCode(author:rongxing, bugfix:BUG00335333)*/
	property_set("debug.decoder.dropbframe","0");
    ...
    if (mFlags & PREPARING) {
        ...
        if (mConnectingDataSource != NULL && !mDisconnecting) {
    ...
    while (mFlags & PREPARING) {
	 ...
	 /*ActionsCode(author:sunchengzhi, stop actvideoextrator paring thread)*/
    if (mVideoSource != NULL){
    	 mExtractor->stopextractor();
    }
...
/**
  *Because actvideoextrator parsing thread will not stop when pause,so real cachedDataRemaining 
  * valeu should has a size compensation that actvideoextrator has consume
  ************************************
  *      
  *ActionsCode(author:sunchengzhi, change_code)
  */
...
bool AwesomePlayer::getCachedDuration_l(int64_t *durationUs, bool *eos) {
    ...
    if (mCachedSource != NULL && getBitrate(&bitrate) && (bitrate > 0)) {
        ...
        /*ActionsCode(author:sunchengzhi,  add size compensation that actvideoextrator has consume)*/
        size_t cachedDataRemaining = mCachedSource->approxDataRemaining(&finalStatus) + mExtractor->getreadsize();
...
/**
  *Because audioTimeUs is get from  mAudioPlayer->getMediaTimeUs() which is not realtime
  *for audio has played,but a accumulation time for audio has decoded which is always larger 
  *than realtime,so the videoLateByUs is meaningless.Based on this consideration,we add time
  *compensation
  ************************************
  *      
  *ActionsCode(author:sunchengzhi, change_code)
...
void AwesomePlayer::onVideoLagUpdate() {
    ...
    /*ActionsCode(author:sunchengzhi, because audioTimeUs is not larger than realtime,so we need time compensation)*/
    TimeCompensation(&videoLateByUs);
            ...
            if (getCachedDuration_l(&cachedDurationUs, &eos2) && mDurationUs > 0) {
                ...
                if ((mFlags & PLAYING) && !eos
                    ...
                    if (mFlags & CACHE_UNDERRUN) {
                        ...
                        ALOGI("cache has filled up (> %zu), prepare is done",
                             kHighWaterMarkBytes);
                        finishAsyncPrepare_l();
                    }
                }
...
void AwesomePlayer::onStreamDone() {
    ...
    if ((mFlags & LOOPING)
        ...
        ALOGI("MEDIA_PLAYBACK_COMPLETE,looping \n");
        seekTo_l(0);
...
status_t AwesomePlayer::play() {
    ...
    Mutex::Autolock autoLock(mLock);
	//ActionsCode(author:rongxing, bugfix:BUG00283145)
	if(mAudioTearDown){
        return OK;
	}
    status_t err = play_l();
...
/**
  *call mExtrctor->setListener that  uses AwesomePlayer as a Listenser to actvideoextrator
  ************************************
  *      
  *ActionsCode(author:sunchengzhi, change_code)
  */
status_t AwesomePlayer::play_l() {
    ...
    /*ActionsCode(author:sunchengzhi,uses AwesomePlayer as a Listenser to actvideoextrator)*/
    if(mExtractor!=NULL){
    	mExtractor->setListener(this);
    }
...
/**
  ...
  ************************************
  *      
  *ActionsCode(author:sunchengzhi, change_code,bugfix:BUG00241678)
  */
...
status_t AwesomePlayer::pause_l(bool at_eos) {
    if (!(mFlags & PLAYING)) {
        if (mAudioTearDown && mAudioTearDownWasPlaying) {
            ALOGD("pause_l() during teardown and finishSetDataSource_l() mFlags %x" , mFlags);
...
status_t AwesomePlayer::setSurfaceTexture(const sp<IGraphicBufferProducer> &bufferProducer) {
	  if(mCachedSource!= NULL && (mFlags & PREPARED)){
    	mCachedSource->enableTimedout();
    }
    Mutex::Autolock autoLock(mLock);
...
/**
  * 2015-01-13 bugfix:BUG00241678
  * 2015-02-02 bugfix:BUG00254350
  * 2015-04-22 bugfix:BUG00283145
  ************************************
  *      
  *ActionsCode(author:sunchengzhi, change_code)
...
status_t AwesomePlayer::seekTo(int64_t timeUs) {
    ...
    if (mExtractorFlags & MediaExtractor::CAN_SEEK) {
        ...
        Mutex::Autolock autoLock(mLock);
        status_t err = seekTo_l(timeUs);
        if(mCachedSource!= NULL){
    			mCachedSource->disableTimedout();
    		}
        return err;
    }else{
		//ActionsCode(author:rongxing, bugfix:BUG00254350)
    	notifyListener_l(MEDIA_SEEK_COMPLETE);
		//ActionsCode(author:rongxing, bugfix:BUG00283145)
		if(mAudioTearDown)
...
status_t AwesomePlayer::initVideoDecoder(uint32_t flags) {
    ...
    if (mVideoSource != NULL) {
        ...
        /*ActionsCode(author:sunchengzhi,get video info for adaptive drop B-Frame)*/   
        int32_t width = 0;
				int32_t height = 0;
				const char *mime;
        mVideoTrack->getFormat()->findCString(kKeyMIMEType, &mime);
   			mVideoTrack->getFormat()->findInt32(kKeyWidth, &width);
   			mVideoTrack->getFormat()->findInt32(kKeyHeight, &height);
   			if (!strcmp(mime,MEDIA_MIMETYPE_VIDEO_AVC) && width >=2048){
   				mIs4kVideo = true;
   				ALOGD("===4k video===\n");
   			}
    ...
    if (mVideoSource != NULL) {
       ...
       /*ActionsCode(author:sunchengzhi, set actions slow_decoder_hack )*/ 
        static const char *kPrefix = "OMX.Action.Video.Decoder";
...
/**
  * 2014-11-25 Fix BUG00205887 for act parser
  *                  when the AwesomePlayer seek the video source,
  *                  act parser will seek both audio and video data.In order to 
  *                  synchronize the audio with video data, we must stop reading audio data
  *                  when seeking.
  * 2015-02-02 Fix BUG00254319 BUG00254335
  ************************************
  *      
  *ActionsCode(author:rongxing, change_code)
  */
...
void AwesomePlayer::onVideoEvent() {
    ...
    if (mSeeking != NO_SEEK) {
		...
		//ActionsCode(rongxing, bugfix BUG00205887)
    ...
    if (!mVideoBuffer) {
        ...
        for (;;) {
            ...
            if(err == ERROR_NETWORK_READ_TIMEOUT && isStreamingHTTP()){
            ...
            if (err != OK) {
                ...
                ALOGI("Videosource read return %d,and post postStreamDoneEvent_l \n",err);
    ...
    if (wasSeeking == NO_SEEK) {
...
#if ADAPTIVE_SKIP_BFRAME
				if(mIs4kVideo){
					if (latenessUs > HIGH_WATER_FOR_SKIP_BFRAME) {
				 		property_set("debug.decoder.dropbframe","1");	
					}else if( latenessUs < LOW_WATER_FOR_SKIP_BFRAME){
				 		property_set("debug.decoder.dropbframe","0");		
					}
				}
#else
        ...
        ATRACE_INT("Video Lateness (ms)", latenessUs / 1E3);
/*ActionsCode(author:sunchengzhi, Actions extractor can not support SEEK_VIDEO_ONLY,so this part is commented)*/
#if 0
...
/*ActionsCode(author:rongxing, Optimize for 60fps video)*/
    ...
    if (wasSeeking == NO_SEEK) {
        ...
        //ActionsCode(rongxing, bugfix BUG00254319 BUG00254335)
...
void AwesomePlayer::onCheckAudioStatus() {
    ...
    if (mWatchForAudioEOS && mAudioPlayer->reachedEOS(&finalStatus)) {
        ...
        modifyFlags(FIRST_FRAME, SET);
        ALOGI("AudioPlayer reachedEOS and postStreamDoneEvent_l,finalStatus is %d \n",finalStatus);
            ...
            if (mCachedSource != NULL) {
                ...
                for (;;) {
                    ...
                    if (metaDataSize < 0
                        ...
                        if(mCachedSource!=NULL){
                        	mCachedSource->setContentType(sniffedMIME);
                        }
...
/**
  *added support for actions inband subtitle
  ************************************
  *      
  *ActionsCode(author:sunchengzhi, change_code)
  */
status_t AwesomePlayer::getTrackInfo(Parcel *reply) const {
    ...
    for (size_t i = 0; i < mExtractor->countTracks(); ++i) {
        ...
        } else if (!strcasecmp(mime.string(), MEDIA_MIMETYPE_TEXT_3GPP)|| (!strcasecmp(mime.string(), MEDIA_MIMETYPE_TEXT_ACT))) { //ActionsCode(author:sunchengzhi,added support for actions inband subtitle)
...
/**
  *add setAudioTrack for actions Extractor
  ************************************
  *      
  *ActionsCode(author:sunchengzhi, change_code)
  */
status_t AwesomePlayer::selectAudioTrack_l(
    ...
    /*ActionsCode(author:sunchengzhi, add setAudioTrack for actions Extractor start)*/
    sp<MetaData> meta= mExtractor->getMetaData();
    const char *mime;
    CHECK(meta->findCString(kKeyMIMEType, &mime));
    if(strcasecmp(mime, MEDIA_MIMETYPE_CONTAINER_MPEG4) &&  strcasecmp(mime, MEDIA_MIMETYPE_CONTAINER_MATROSKA) && !strncasecmp(mime, "video/", 6)){ 
    	mExtractor->setAudioTrack(trackIndex,curTimeUs);
    }
    /*ActionsCode(author:sunchengzhi, add setAudioTrack for actions Extractor end)*/
...
/**
  *added support for actions inband subtitle selecttrack
  ************************************
  *      
  *ActionsCode(author:sunchengzhi, change_code)
  */
status_t AwesomePlayer::selectTrack(size_t trackIndex, bool select) {
    ...
    bool isActionsInbandSub = false;//ActionsCode(author:sunchengzhi,mark actions inband subtitle)
    if (trackIndex < mExtractor->countTracks()) {
        ...
        /*ActionsCode(author:sunchengzhi,change_code start)*/
        if (!isAudioTrack && strcasecmp(mime, MEDIA_MIMETYPE_TEXT_3GPP ) != 0 && strcasecmp(mime, MEDIA_MIMETYPE_TEXT_ACT) != 0) {
            ALOGE("Track %zu is not either audio or timed text", trackIndex);
            return ERROR_UNSUPPORTED;
        }
        if(!strcasecmp(mime, MEDIA_MIMETYPE_TEXT_ACT)){
        	isActionsInbandSub = true;
        }
		/*ActionsCode(author:sunchengzhi,change_code end)*/
    }
    ...
    if (isAudioTrack) {
        ...
        mAudioTrackIndexSelected = trackIndex;
    ...
    if (select) {
        ...
        if (err == OK) {
            ...
            if (/*mFlags & PLAYING &&*/!(mFlags & TEXT_RUNNING)) {//ActionsCode(author:sunchengzhi,AwesomePlayer may not be playing status ,so we remove playing condition Restrictions
               if(isActionsInbandSub==true){//ActionsCode(author:sunchengzhi,Becuase Actions inband subtitle switch does not support Seamless,so we need to call seekTo_l before selectTrack
                	int64_t curTimeUs;
                  	CHECK_EQ(getPosition(&curTimeUs), (status_t)OK);
                	seekTo_l(curTimeUs);
                }           
        ...
        mSubTrackIndexSelected = trackIndex;
    } else {
...
/**
  *************fix testChangeSubtitleTrack,testDeselectTrack and testGetTrackInfo***********************
  *      
  *ActionsCode(author:sunchengzhi, change_code)
  */
status_t AwesomePlayer::invoke(const Parcel &request, Parcel *reply) {
    ...
    switch(methodId) {
        ...
        /*ActionsCode(author:sunchengzhi,add for cts testChangeSubtitleTrack,testDeselectTrack and testGetTrackInfo)*/
         case INVOKE_ID_GET_SELECTED_TRACK:
        {
            int32_t type = request.readInt32();
            int selectedTrack;
            if(type==MEDIA_TRACK_TYPE_AUDIO){
            	selectedTrack = mAudioTrackIndexSelected;
            }
            
            if(type==MEDIA_TRACK_TYPE_TIMEDTEXT){
            	selectedTrack = mSubTrackIndexSelected;
            }
