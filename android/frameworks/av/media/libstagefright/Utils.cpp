...
/**
  *
  ********converting SpecificData to matching actions Vorbis decoder****************************
  *      
  *ActionsCode(author:sunchengzhi, new_method)
  */
void GetAACCodecSpecific(sp<ABuffer> esdsbuffer,const void *codec_specific_data,uint32_t type,int32_t channel_count){
	unsigned char buf[2];
	int *ptr;	
	ptr=(int*)(esdsbuffer->data());			
	memcpy(buf, codec_specific_data, 2);
	ptr[0] = (buf[0]>>3)&0x7;
	ptr[1] = ((buf[0]&7)<<1)|(buf[1]>>7);
	if(type != 0){
		ptr[2] = channel_count;
		if(0 == ptr[2]){
			ptr[2] = (buf[1]&0x7f)>>3;
...
/**
  ...
  *ActionsCode(author:sunchengzhi, new_method)
  */
void GetVorbisSpecificData(sp<ABuffer> VorbisInfo,sp<ABuffer> FirstVorbisBook,sp<ABuffer> VorbisBooks,sp<ABuffer> VorbisSpecialData){
	const void*pb0, *pb1, *pb2;
	void *data;
	size_t sz0, sz1, sz2;
	int *ptr = NULL;
	sz0 = sz1 = sz2 = 0;
	...
	pb1 = FirstVorbisBook->data();
	sz1 = FirstVorbisBook->size();

	pb2 = VorbisBooks->data();
	sz2 = VorbisBooks->size();
			
	char *cptr = (char*)(VorbisSpecialData->data());
	*cptr++ = 'o';
	*cptr++ = 'g';
	*cptr++ = 'c';
	*cptr++ = 'z';			
...
status_t convertMetaDataToMessage(
    ...
    if (!strncasecmp("video/", mime, 6)) {
		...
		if(meta->findInt32('pwdt',&frameWidthActual) && meta->findInt32('phgt',&frameHeightActual)){
        	msg->setInt32("pwdt", frameWidthActual);
          	msg->setInt32("phgt", frameHeightActual);
        }
    ...
    if (meta->findData(kKeyAVCC, &type, &data, &size)) {
        ...
        buffer->meta()->setInt64("timeUs", 0);
        msg->setBuffer("csd-1", buffer);
		   /*ActionsCode(author:sunchengzhi, Actions Vorbis decoder need VorbisBook as config pkt)*/
        ...
        buffer->meta()->setInt64("timeUs", 0);
        msg->setBuffer("csd-0", buffer);
    } else if (meta->findData(kKeyActVideoPrivateData, &type, &data, &size)) {/*ActionsCode(author:sunchengzhi, set actions video decoders special data)*/
        sp<ABuffer> buffer = new ABuffer(size);
        memcpy(buffer->data(), data, size);
        ...
        buffer->meta()->setInt64("timeUs", 0);
        msg->setBuffer("csd-0", buffer);
    }   else if (meta->findData(kKeyActAudioPrivateData, &type, &data, &size)) { /*ActionsCode(author:sunchengzhi, set actions audio decoders special data)*/
        sp<ABuffer> buffer = new ABuffer(size);
        memcpy(buffer->data(), data, size);
