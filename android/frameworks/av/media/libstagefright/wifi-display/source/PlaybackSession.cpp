...
status_t WifiDisplaySource::PlaybackSession::addSource(
    ...
    if (isVideo) {
		int scfg = getwfd_info("ro.wfd.scfg",0);
		
		if(scfg == 0){
		    int ow = getwfd_info("ro.wfd.ow",1280);
    		int oh = getwfd_info("ro.wfd.oh",720);    		
			format->setInt32("wfd-out-width", ow);
			format->setInt32("wfd-out-height", oh);
		}
...
status_t WifiDisplaySource::PlaybackSession::addVideoSource(
    ...
    CHECK(VideoFormats::GetProfileLevel(
                ...
                &levelIdc,
                &constraintSet));
		int w = getwfd_info("ro.wfd.iw",1280/*1024*/);
		int h = getwfd_info("ro.wfd.ih",720/*600*/);
		int scfg = getwfd_info("ro.wfd.scfg",0);
	...
	ALOGD("addVideoSource->new SurfaceMediaSource: width %d, height %d",width,height);
