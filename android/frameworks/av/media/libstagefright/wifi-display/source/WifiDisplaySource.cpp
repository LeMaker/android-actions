    ...
    switch (msg->what()) {
        ...
        case kWhatReapDeadClients:
        {
            ...
            if (mClientInfo.mPlaybackSession->getLastLifesignUs()
                ...
                /*ActionsCode(author:fanguoyong),when first time recevie M16 timeout,ingore it */
                ALOGI("playback session timed out, reaping.timeoutM16 = %d",timeoutM16);
                timeoutM16 -= 1;
                if(timeoutM16 >= 0) {
...
status_t WifiDisplaySource::onReceiveM16Response(
    ...
    //ALOGI("ReceiveM16");
    /*ActionsCode(author:fanguoyong),reset timeoutM16 when receive M16 */
    timeoutM16 = 1;
