...
namespace android {
const static int64_t kNetWorkReadTimeOutNs =3000000000LL;
...
/**
  ...
  */
void NuCachedSource2::setContentType(AString sniffedMIME){
	if(sniffedMIME.empty()){
		return;
	}
	if(!strncasecmp(sniffedMIME.c_str(),"video/mp4",9)){
		mIsMp4 = true;
	}else{
		mIsMp4 = false;
	}
}
void NuCachedSource2::enableTimedout(){
	if(mIsMp4==true){		 
		 mTimedoutEnabled = true;	 
		 if(mWaitCondition==true){
		 	 mCondition.signal();		 	  
		 }
...
void NuCachedSource2::disableTimedout(){
	if(mIsMp4==true){
		 mTimedoutEnabled = false ;
	}	
}
...
void NuCachedSource2::fetchInternal() {
    ...
    if (n == 0 || mDisconnecting) {
        ...
        if (n == ERROR_UNSUPPORTED || n == -EPIPE) {
            ...
            //actions_code(lishiyuan, fix BUG00220997)
            mNumRetriesLeft = 1;
        }
...
void NuCachedSource2::onRead(const sp<AMessage> &msg) {
    ...
    if(mIsMp4){
			mAsyncResult->setInt64("offset", offset);
		}
...
ssize_t NuCachedSource2::readAt(off64_t offset, void *data, size_t size) {
    ...
    /*ActionsCode(author:sunchengzhi, bugfix:BUG00241678)*/
    if(mIsMp4){
    	msg->setInt32("generation",++mReadGeneration);
    }else{
    	CHECK(mAsyncResult == NULL);
    }
    ...
    while (mAsyncResult == NULL && !mDisconnecting) {       
        ...
        if(mIsMp4){
        	if(mTimedoutEnabled){
        		status_t err = mCondition.waitRelative(mLock,kNetWorkReadTimeOutNs);
        		if (err != OK) {
        			ALOGD("----wait network read timeout,%d-----\n",err);
        			mTimedoutcnt++;
        			if(mTimedoutcnt>20 || mDisconnecting){
        				ALOGD("----wait network read timeout exceed 20 times,so return end of stream,mDisconnecting:%d-----\n",mDisconnecting);
        				mAsyncResult.clear();
        				return ERROR_END_OF_STREAM;
        			}
		   		 		mAsyncResult.clear();
    ...
    CHECK(mAsyncResult->findInt32("result", &result));
		...
		int64_t off_set;
		if(mIsMp4){		 
    		mAsyncResult->findInt64("offset", &off_set);
		}
    ...
    if (result > 0) {
		...
		if(mIsMp4){
    	if(result>0 &&off_set!=offset){		
				ALOGD("===offset not right===");	
				return ERROR_NETWORK_READ_TIMEOUT;	
			}
		}	
