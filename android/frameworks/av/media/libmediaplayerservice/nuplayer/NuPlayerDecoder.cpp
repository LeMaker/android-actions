...
bool NuPlayer::Decoder::handleAnInputBuffer() {
    ...
    if (!mCSDsToSubmit.isEmpty()&&(mComponentName.find(ACTIONS_PREFIX,0)==-1)) {
        sp<AMessage> msg = new AMessage();
        msg->setSize("buffer-ix", bufferIx);
...
void NuPlayer::Decoder::rememberCodecSpecificData(const sp<AMessage> &format) {
    ...
    if(encoder==false && !strncasecmp(mime.c_str(), "video/avc", 9)){    //for actions avc decoder
        ...
        memcpy(csdbuffer->data() + csdbuffer->size(),buffer->data(),buffer->size());
       	csdbuffer->setRange(0, csdbuffer->size() + buffer->size());
    	}
    	if(csdbuffer->size()>0){
       	ALOGD("rememberCodecSpecificData avc csd buffer size is %d \n",csdbuffer->size());
       	csdbuffer->meta()->setInt64("timeUs", 0);
       	mCSDsForCurrentFormat.push(csdbuffer);
      }    	
    }else{
    	for (i = 0; ; ++i) {
    	 AString tag = "csd-";
