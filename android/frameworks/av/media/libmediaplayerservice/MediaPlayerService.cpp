...
MediaPlayerService::Client::~Client()
{
    ALOGW("Client(%d) destructor pid = %d", mConnId, mPid);
    mAudioOutput.clear();
...
{
    ALOGW("disconnect(%d) from pid %d", mConnId, mPid);
...
{
    ALOGW("player type = %d", playerType);
...
{
    ALOGW("setDataSource fd=%d, offset=%lld, length=%lld", fd, offset, length);
    struct stat sb;
    int ret = fstat(fd, &sb);
    if (ret != 0) {
...
{
    ALOGI("[%d] setVideoSurfaceTexture(%p)", mConnId, bufferProducer.get());
    ...
    ALOGI("[%d] setVideoSurfaceTexture(%p) end", mConnId, bufferProducer.get());
    return err;
}
...
#endif
    ALOGW("[%d] prepareAsync end", mConnId);
    return ret;
}

status_t MediaPlayerService::Client::start()
{
    ALOGW("[%d] start", mConnId);
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;
    p->setLooping(mLoop);
    ...
    status_t ret; 
    ret = p->start();
    ALOGW("[%d] start end", mConnId);
    return ret;
}

status_t MediaPlayerService::Client::stop()
{
    ALOGW("[%d] stop", mConnId);
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;
    status_t ret;
    ret = p->stop();
    ALOGW("[%d] stop end", mConnId);
    return ret;
}

status_t MediaPlayerService::Client::pause()
{
    ALOGW("[%d] pause", mConnId);
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;
    status_t ret;
    ret = p->pause();
    ALOGW("[%d] pause end", mConnId);
    return ret;
}
...
{
    ALOGW("[%d] seekTo(%d)", mConnId, msec);
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;
    status_t ret;
    ret = p->seekTo(msec);
    ALOGW("[%d] seekTo(%d) end", mConnId, msec);
    return ret;
}

status_t MediaPlayerService::Client::reset()
{
    ALOGW("[%d] reset", mConnId);
    mRetransmitEndpointValid = false;
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;
    status_t ret;
    ret = p->reset();
    ALOGW("[%d] reset end", mConnId);
    return ret;
}
...
////////////////////////////////////////////////////////////////////////////////
/**
 *BUGFIX: Android original code to calculate mFrameCount using heap size here
 results in a heavy performance drop when loading SoundPool Samples using AwesomePlayer, 
 use default mFrameCount value which is 1024.
 ************************************ 
 *      
        ...
        const audio_offload_info_t* /*offloadInfo*/)
{
    ...
    //ActionsCode(author:yuchen, change_code): restore below code line after NuPlayer is ready.
