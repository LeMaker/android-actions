...
/**
 *BUGFIX: keytune caused noise in eq sound.
 *Soundpool: using shared buffer in sound pool will cause noise in eq, so disable shared buffer 
 *in sound pool.
 ************************************
 *      
 *ActionsCode(author:yuchen, change_code): only use shared buffer for 64bit libmedia.so,
 *                                         for no problem caused in GS705A
 */                     
#ifdef	__LP64__
