...
bool MediaProfiles::sIsCamcorderInitialized = false;
long MediaProfiles::last_modify_time = 0;
...
void MediaProfiles::addImageEncodingQualityLevel(int cameraId, const char** atts)
{
	...
	for(i = 0;i < levels->mLevels.size();i++)
		{
			if(levels->mLevels[i] == quality)
				{
		  			tmp = false;
				}
		}
...
int
MediaProfiles::initCamcorderProfilesFromXmlFile(MediaProfiles *profiles, const char *xml)
{
    int ret = 0;
    FILE *fp = NULL;
    int max_count = 100;
    ...
    if(fp == NULL)
    {
        ALOGE("initCamcorderProfilesFromXmlFile from xml(%s) failed, can not open file.", xml);
        return -1;
    }
#endif
...
/**
* NEW_FEATURE: Add xml update checkout and do refreshing for UVC module support .
*ActionsCode(author:liyuan, add_code)
*/
bool MediaProfiles::needrefresh(const char **xmlpath)
{
	bool ret = false;
	FILE * fp = NULL;
        int fd;
...
*/
void MediaProfiles::initCamcorderProfiles()
{
	...
	ALOGE("camcorder.settings.xml is null!");
    } else {
        ALOGE("camcorder.settings.xml exist!");
        xmlFile = (char *)&value;
    }

    if(xmlFile != NULL && needrefresh(&xmlFile)){
        for (size_t i = 0; i < mCamcorderProfiles.size(); ++i) {
    	    mCamcorderProfiles.removeItemsAt(i);
    	}
	mCamcorderProfiles.clear();
	sIsCamcorderInitialized = false;
    }
    
    if (sIsCamcorderInitialized) {
	ALOGE("sIsCamcorderInitialized is true!");
        return;
    }

    ALOGE("initCamcorderProfiles!");
    if(xmlFile != NULL)
    {
        initCamcorderProfilesFromXmlFile(this, xmlFile);
    }
    checkAndAddRequiredProfilesIfNecessary();

    sIsCamcorderInitialized = true;
...
bool MediaProfiles::hasCamcorderProfile(int cameraId, camcorder_quality quality)
{
#if 1
    initCamcorderProfiles();
#endif
...
Vector<int> MediaProfiles::getImageEncodingQualityLevels(int cameraId)
{
    Vector<int> result;
#if 1
    initCamcorderProfiles();
#endif
	//android::CallStack cs("MediaProfiles::getImageEncodingQualityLevels");
    ImageEncodingQualityLevels *levels = findImageEncodingQualityLevels(cameraId);
    if (levels != NULL) {
...
int MediaProfiles::getStartTimeOffsetMs(int cameraId) {
    int offsetTimeMs = -1;
#if 1
    initCamcorderProfiles();
#endif
