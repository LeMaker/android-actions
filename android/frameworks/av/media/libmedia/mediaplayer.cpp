...
bool MediaPlayer::isPlaying()
{
    ...
    if (mPlayer != 0) {
		...
		if ((mCurrentState & MEDIA_PLAYER_PAUSED) && temp) {
            ALOGE("internal/external state mismatch corrected : %d",temp);
            mCurrentState = MEDIA_PLAYER_STARTED;
        }
        return temp;
    }
...
void MediaPlayer::notify(int msg, int ext1, int ext2, const Parcel *obj)
{
	...
	//ActionsCode(author:rongxing, bugfix:BUG00253699)
    if (mLockThreadId != getThreadId() && (!mLockThreadId || mPlayer == NULL
		|| mPlayer->asBinder()->localBinder() == NULL)) {
        mLock.lock();
        locked = true;
    }
