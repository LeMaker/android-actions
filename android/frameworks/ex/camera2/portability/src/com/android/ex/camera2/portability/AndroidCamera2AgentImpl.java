...
class AndroidCamera2AgentImpl extends CameraAgent {
    ...
    private class Camera2Handler extends HistoryHandler {
    	...
    	/**
    	* NEW_FEATURE: Implement 5.0's empty interfaces needed for face detect.
    	*ActionsCode(author:liyuan, change_code)
    	*/
	private CameraFaceDetectionCallback mFaceListener;
        ...
        public void handleMessage(final Message msg) {
		    ...
		    */
			...
			mFaceListener = (CameraFaceDetectionCallback) msg.obj;
                        break;
                    }
		    /**
		    * NEW_FEATURE: Implement 5.0's empty interfaces needed for face detect.
		    ...
		    */
			...
			mPersistentSettings.set(CaptureRequest.STATISTICS_FACE_DETECT_MODE, CaptureRequest.STATISTICS_FACE_DETECT_MODE_SIMPLE);
			try {
				mSession.setRepeatingRequest(
				mPersistentSettings.createRequest(mCamera, CameraDevice.TEMPLATE_PREVIEW, mPreviewSurface),
				/*listener*/mCameraResultStateCallback,/*handler*/this);
								
			} catch(CameraAccessException ex) {
				Log.w(TAG, "Unable to START_FACE_DETECTION", ex);
			}
                        break;
                    }
		    /**
		    ...
		    */
			...
			mPersistentSettings.set(CaptureRequest.STATISTICS_FACE_DETECT_MODE, CaptureRequest.STATISTICS_FACE_DETECT_MODE_OFF);
			...
			try {
				...
				Log.w(TAG, "Unable to STOP_FACE_DETECTION", ex);
			}
                        break;
                    }
		...
		/**
	        ...
	        *ActionsCode(author:liyuan, change_code)
	        */
                Integer facedetectMaybe = result.get(CaptureResult.STATISTICS_FACE_DETECT_MODE);
               
                if (facedetectMaybe != CaptureResult.STATISTICS_FACE_DETECT_MODE_OFF) {
			android.hardware.camera2.params.Face[] api2facesMaybe = result.get(CaptureResult.STATISTICS_FACES);
			List<android.hardware.Camera.Face> convertedFaces = new ArrayList<>();
					 
			if(api2facesMaybe != null && mFaceListener != null) {
		       	    for (android.hardware.camera2.params.Face api2Face : api2facesMaybe) {
				if (api2Face != null) {
				    android.hardware.Camera.Face legacyFace = new android.hardware.Camera.Face();
			            legacyFace.rect  = api2Face.getBounds();
				    legacyFace.score = api2Face.getScore();
				    legacyFace.id    = api2Face.getId();
				    legacyFace.leftEye   = api2Face.getLeftEyePosition();
				    legacyFace.rightEye  = api2Face.getRightEyePosition();
				    legacyFace.mouth     = api2Face.getMouthPosition();

				    convertedFaces.add(legacyFace);

				 } else {
				    Log.w(TAG, "mapResultFaces - read NULL face");
				 }
			    }
						 
			    api2facesMaybe = null;
			    mFaceListener.onFaceDetection(convertedFaces.toArray(new android.hardware.Camera.Face[0]), mCameraProxy);
			}
		}
            }
	...
	*/
        // TODO: Implement
        @Override
        public void setFaceDetectionCallback(final Handler handler, final CameraFaceDetectionCallback callback)
        {
		mDispatchThread.runJob(new Runnable() {
                   @Override
                   public void run() {
                   	CameraFaceDetectionCallback cbForward = null;
                   	if (callback != null) {
                           cbForward = new CameraFaceDetectionCallback() {
                              @Override
                              public void onFaceDetection( final android.hardware.Camera.Face[] faces, final CameraProxy camera) {
                                 handler.post(new Runnable() {
                                 @Override
                                 public void run() {
                                    callback.onFaceDetection(faces, camera);
                                 }});
                           }};
                        }
