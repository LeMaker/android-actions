...
class WifiController extends StateMachine {
    ...
    private String wifi_modules = SystemProperties.get("ro.wifi.suspend_policy");
    private boolean mSetWifiOff = true;
    private PowerManager powerManager;
    private PendingIntent mWifiOnDealayIntent;
    ...
    private static final String ACTION_DEVICE_SCREEN_DEALAY =
            "com.android.server.WifiManager.action.DEVICE_DELAY";
    ...
    WifiController(Context context, WifiServiceImpl service, Looper looper) {
        ...
        Intent wifiOnDealayIntent = new Intent(ACTION_DEVICE_SCREEN_DEALAY, null);
        mWifiOnDealayIntent = PendingIntent.getBroadcast(mContext, IDLE_REQUEST, wifiOnDealayIntent, 0);
        ...
        filter.addAction(ACTION_DEVICE_SCREEN_DEALAY);
                ...
                new BroadcastReceiver() {
                    ...
                    public void onReceive(Context context, Intent intent) {
                        ...
                        if (action.equals(ACTION_DEVICE_IDLE)) {
                            if(mSetWifiOff == false) {
                                if (DBG) Slog.d(TAG, "(mSetWifiOff == false), CMD_DEVICE_IDLE");
                            } else {    //here remove wifi driver
                                if(checkLocks()) {
                                    Slog.d(TAG, "mAlarmManager again start");
                                    mIdleMillis = 30 * 1000;
                                    mAlarmManager.set(AlarmManager.RTC,
                                    System.currentTimeMillis() + mIdleMillis, mIdleIntent);
                                } else {
                                    if(wifi_modules != null && !wifi_modules.isEmpty() && wifi_modules.equals("RMMOD_WLAN_KO")) {
                                        if (true) Slog.d(TAG, "Ready remove wifi driver!!!!");
                                        transitionTo(mApStaDisabledState);
                                        sendMessage(CMD_WIFI_TOGGLED);
                                    } else {
                                        if (true) Slog.d(TAG, "Ready stop wifi driver!!!!");
                        ...
                        } else if (action.equals(ACTION_DEVICE_SCREEN_DEALAY)) {
                            mSetWifiOff = false;
                            updateBatteryWorkSource();
                            if (mSettingsStore.isWifiToggleEnabled()) {
                                if(wifi_modules != null && !wifi_modules.isEmpty() && wifi_modules.equals("RMMOD_WLAN_KO")) {
                                    if (DBG) Slog.d(TAG, "mSettingsStore.isWifiToggleEnabled(), sendMessage(CMD_WIFI_TOGGLED)");
                                    transitionTo(mStaEnabledState);
                                    sendMessage(CMD_WIFI_TOGGLED);
                                } else {
                                    if (DBG) Slog.d(TAG, "Ready start wifi driver!!!!");
                                    mWifiStateMachine.setDriverStart(true);
                                }
                            } else if (!mSettingsStore.isWifiToggleEnabled() && mSettingsStore.isScanAlwaysAvailable()) {
                                if(wifi_modules != null && !wifi_modules.isEmpty() && wifi_modules.equals("RMMOD_WLAN_KO")) {
                                    if (DBG) Slog.d(TAG, "mSettingsStore.isScanAlwaysAvailable(), sendMessage(CMD_WIFI_TOGGLED)");
                ...
                case CMD_SCREEN_OFF:
                    ...
                    */
                    if (!shouldWifiStayAwake(mPluggedType)) {
                        ...
                        mAlarmManager.cancel(mWifiOnDealayIntent);
                        mAlarmManager.set(AlarmManager.RTC,
                                System.currentTimeMillis() + mIdleMillis, mIdleIntent);
                    }
                    break;
                ...
                case CMD_BATTERY_CHANGED:
                    ...
                    */
                    ...
                    if (mScreenOff && shouldWifiStayAwake(mPluggedType) &&
                        ...
                        mAlarmManager.set(AlarmManager.RTC, triggerTime, mIdleIntent);
                    }
    ...
    private boolean checkLocks() {
        ...
        int WakelockSize = powerManager.getWakelockSize("wifi");

        if(WakelockSize > 0) {
            Slog.d(TAG, "Wakelock size = " + WakelockSize);
            result = true;
        }
