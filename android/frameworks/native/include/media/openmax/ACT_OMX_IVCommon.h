...
/**
* decide uvc used as front or rear
*/
typedef enum OMX_UVCMODE{
	OMX_UVC_NONE = 0,
	OMX_UVC_AS_REAR,
	OMX_UVC_AS_FRONT,
}OMX_UVCMODE;
...
typedef struct OMX_ACT_CAPTYPE {
	...
	OMX_COLOR_FORMATTYPE    ePreviewFormats[16];
	OMX_U16                 ulImageFormatCount;     // supported image pixelformat count
	OMX_COLOR_FORMATTYPE    eImageFormats[16];
	OMX_ACT_VARRESTYPE      tPreviewRes[64];       // supported preview resolution range
	...
	OMX_S32                 xEVCompensationStep;    // Fixed point value stored as Q16
	...
	OMX_ACT_VARFPSTYPE      tPrvVarFPSModes[64];
	OMX_U16                 ulCapVarFPSModesCount;  // supported variable FPS capture modes count
	OMX_ACT_VARFPSTYPE      tCapVarFPSModes[64];
...
typedef enum OMX_ACT_IMAGE_FOCUSCONTROLTYPE {
    ...
    OMX_IMAGE_FocusControlZone,
		OMX_IMAGE_FocusControlMacro,
		OMX_IMAGE_FocusControlFACE,
		OMX_IMAGE_FocusControlQuickCapture,
...
typedef struct OMX_ACT_CONFIG_FlipParams {
    OMX_U32 nPortIndex;	
    OMX_BOOL mhflip;
    OMX_BOOL mvflip;    
} OMX_ACT_CONFIG_FlipParams;

typedef struct OMX_ACT_CONFIG_CommonParams{
		OMX_U32 nSize;
    OMX_VERSIONTYPE nVersion;
    OMX_U32 nPortIndex;
    OMX_BOOL bEnable;   
}OMX_ACT_CONFIG_CommonParams;
