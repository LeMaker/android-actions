...
static String8 generateDescriptor(InputDeviceIdentifier& identifier) {
	...
	ALOGE("on generateDescriptor, raw %s\n",rawDescriptor.string());
...
void EventHub::assignDescriptorLocked(InputDeviceIdentifier& identifier) {
    ...
    ALOGE("Created descriptor: raw=%s, cooked=%s", rawDescriptor.string(),
                    ...
                    for (size_t i = 0; i < count; i++) {
                        ...
                        if (event->when >= now + 10 * 1000000000LL) {
                            ...
                            if (event->when > time) {
                                ...
                                event->when = time;
				//FIX BUG00276147
				//We don't know exactly why system time was changed. But we can set the time to what we need, and then try it again.
				int clockId = CLOCK_MONOTONIC;
    				!ioctl(device->fd, EVIOCSCLOCKID, &clockId);
				time = systemTime(SYSTEM_TIME_MONOTONIC);
                            	if (event->when > time) {
					ALOGW("We have tried again, but it still failed:"
...
status_t EventHub::openDeviceLocked(const char *devicePath) {
	...
	ALOGW("caichsh added, going to add device: %s \n", devicePath);
	//FIX BUG00276147
	//These has possibility that we open a device twice when eventhub get a wrong time system from kernel.
	//May we should shoot the kernel's trouble.
	Device* dv = getDeviceByPathLocked(devicePath);
	if(dv) {
		ALOGE("caichsh added, device %s has already been added !!!!\n", devicePath);
		return -1;
	}
