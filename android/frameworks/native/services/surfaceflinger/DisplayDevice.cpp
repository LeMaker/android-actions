      ...
      mActiveConfig(0)
{
    ...
    mDefaultRotation = DisplayState::eOrientationDefault;
#endif
    ...
    //ActionsCode(wh, NEW_FEATURE : support hw rotation)
            ...
            switch (mFlinger->sPropertiesState.mDefaultRotation) {
                case 90:
                    mDefaultRotation = DisplayState::eOrientation90;
                    break;
                case 180:
                    mDefaultRotation = DisplayState::eOrientation180;
                    break;
                case 270:
...
#if ACTIONS_FEATURE_HW_ROTATION_DEFAULT_ON
    setProjection(mDefaultRotation, mViewport, mFrame);
#else
    setProjection(DisplayState::eOrientationDefault, mViewport, mFrame);
#endif
#if ACTIONS_FEATURE_HDMI_USE_OPAQUE_FORMAT
    //ActionsCode(wh, OPTIMIZE : HDMI default used RGB 565)    	
    if (mType == DisplayDevice::DISPLAY_EXTERNAL){
    	if(mFlinger->sPropertiesState.mHdmiDefaultFormat == PIXEL_FORMAT_RGB_565){
...
void DisplayDevice::setProjection(int orientation,
    ...
    //ActionsCode(wh, NEW_FEATURE : support hw rotation)
#if ACTIONS_FEATURE_HW_ROTATION_DEFAULT_ON
        if((mDefaultRotation & DisplayState::eOrientation90) != 0){
        	swap(viewport.right, viewport.bottom);
        }
#endif
    }
