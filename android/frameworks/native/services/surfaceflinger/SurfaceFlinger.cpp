        ...
        mHasColorMatrix(false)
{
    ...
    sPropertiesState.mDefaultRotation = atoi(value) * 90; 
    ALOGI_IF(sPropertiesState.mHwRotation, "hwrotation %d  mDefaultRotation %d",sPropertiesState.mHwRotation,sPropertiesState.mDefaultRotation);
#endif 
#if ACTIONS_FEATURE_HDMI_USE_OPAQUE_FORMAT
	property_get("ro.sf.ext_disp_format", value, "RGB_565");
	if(strcmp(value,"RGBA_8888") == 0){
		sPropertiesState.mHdmiDefaultFormat = HAL_PIXEL_FORMAT_RGBA_8888; 
	}else{
		sPropertiesState.mHdmiDefaultFormat = PIXEL_FORMAT_RGB_565;
	} 
...
#if ACTIONS_FEATURE_CHECK_GPU_READY
bool SurfaceFlinger::checkGPUReady()const
{
	FILE *fd=NULL;
	int timeout = 0;
	fd = fopen("/data/pvrsrv","rb");
	while(fd==NULL && timeout <= 50){
		fd = fopen("/data/pvrsrv","rb");
		usleep(10000);
		timeout++;
	}
...
#if ACTIONS_FEATURE_CHECK_GPU_READY
	if(!checkGPUReady()){
		ALOGE("GPU not initialized before surfaceFlinger init");
	}
#endif	
    status_t err;
    Mutex::Autolock _l(mStateLock);
...
static  bool sDebugShowFps = false;// whether to show fps
static  int sDebugShowFpsInv = 500;// how often to show fps
void SurfaceFlinger::debugShowFPS() const
{
	static int sFrameCount = 0;
	static nsecs_t sLastFpsTime = 0;
	double fps = 0;
	++sFrameCount;
	nsecs_t now = systemTime();
	nsecs_t diff = now - sLastFpsTime;
	if (diff > ms2ns(sDebugShowFpsInv)) {
		fps =  ( sFrameCount * double(s2ns(1))) / diff;
		ALOGD("fps:%.2f", fps);
		sFrameCount = 0;
		sLastFpsTime = now;
	}
...
status_t SurfaceFlinger::dump(int fd, const Vector<String16>& args)
{
    ...
    if ((uid != AID_SHELL) &&
        ...
        if (numArgs) {
            ...
            //ActionsCode(wh, NEW_FEATURE : add for debug fps)
            if ((index < numArgs) &&
                    (args[index] == String16("--showfps"))) {               
                 index++;
                 char value[PROPERTY_VALUE_MAX];
                 property_get("debug.sf.showfps", value, "0");
                        sDebugShowFps = static_cast<bool>(atoi(value));
                 property_get("debug.sf.showfpsinv", value, "500");
                 sDebugShowFpsInv = atoi(value);
                 dumpAll = false;
             }
#endif 
        }
...
*
************************************
*convert crop bounds and rotation
************************************
*
*ActionsCode(liuxinxu, new_method: convert crop bounds and rotation)
*/
#if ACTIONS_FEATURE_HW_ROTATION_DEFAULT_ON
void SurfaceFlinger::convertCropBoundsAndRotation(Rect& sourceCrop, uint32_t hw_h, uint32_t hw_w, int hwOrientation, Transform::orientation_flags& rotation)
{
    //convert to original crop bounds.
    ...
    switch (rotation) {
        ...
        case Transform::ROT_90:
            tmp = sourceCrop.right;
            sourceCrop.right = hw_w - sourceCrop.top;
            sourceCrop.top = sourceCrop.left;
            sourceCrop.left = hw_w - sourceCrop.bottom;
            sourceCrop.bottom = tmp;
            break;
        case Transform::ROT_180:
            tmp = sourceCrop.top;
            sourceCrop.top = hw_h - sourceCrop.bottom;
            sourceCrop.bottom = hw_h - tmp;
            tmp = sourceCrop.right;
            sourceCrop.right = hw_w - sourceCrop.left;
            sourceCrop.left = hw_w - tmp;
            break;
        case Transform::ROT_270:
            tmp = sourceCrop.top;
            sourceCrop.top = hw_h - sourceCrop.right;
            sourceCrop.right = sourceCrop.bottom;
            sourceCrop.bottom = hw_h - sourceCrop.left;
            sourceCrop.left = tmp;
            break;
        default:
            break;
    ...
    if (rotation & Transform::ROT_90) {
    	rotation = (rotation == Transform::ROT_90) ? Transform::ROT_270 : Transform::ROT_90;
    }

    uint32_t rotate = 0;
    switch (rotation) {
        case Transform::ROT_0:
            rotate = hwOrientation + DisplayState::eOrientationDefault;
            break;
        case Transform::ROT_90:
            rotate = hwOrientation + DisplayState::eOrientation90;
            break;
        case Transform::ROT_180:
            rotate = hwOrientation + DisplayState::eOrientation180;
            break;
        case Transform::ROT_270:
            rotate = hwOrientation + DisplayState::eOrientation270;
            break;
        default:
            break;
    }

    rotate %= DisplayState::eOrientation270 + 1;
    if (rotate == DisplayState::eOrientation90 || rotate == DisplayState::eOrientation270) {
    	rotate = (rotate == DisplayState::eOrientation90) ? DisplayState::eOrientation270 : DisplayState::eOrientation90;
    }

    //convert to target bounds.
    switch (rotate) {
    	case DisplayState::eOrientationDefault:
            rotation = Transform::ROT_0;
            break;
        case DisplayState::eOrientation90:
            rotation = Transform::ROT_90;
            tmp = sourceCrop.top;
            sourceCrop.top = hw_w - sourceCrop.right;
            sourceCrop.right = sourceCrop.bottom;
            sourceCrop.bottom = hw_w - sourceCrop.left;
            sourceCrop.left = tmp;
            break;
        ...
        case DisplayState::eOrientation270:
            ...
            tmp = sourceCrop.top;
            sourceCrop.top = sourceCrop.left;
            sourceCrop.left = hw_h - sourceCrop.bottom;
            sourceCrop.bottom = sourceCrop.right;
            sourceCrop.right = hw_h - tmp;
            break;
        default:
            break;
    }
        ...
        bool useIdentityTransform, Transform::orientation_flags rotation)
{
    ...
    //ActionsCode(liuxinxu, OPTIMIZE : support hw rotation)
#if ACTIONS_FEATURE_HW_ROTATION_DEFAULT_ON
    convertCropBoundsAndRotation(sourceCrop, hw_h, hw_w, hw->getHwOrientation(), rotation);
#endif
