...
#define VIDDEC_DEFAULT_OUTPUT_BUFFER_NUM_HEVC	10
#define VIDDEC_DEFAULT_OUTPUT_BUFFER_NUM_VP9	10
...
#define VIDDEC_MIMETYPWMV2                      		"WMV2"
#define VIDDEC_MIMETYPVP9                      		"vp9"
...
typedef enum VIDDEC_DEFAULT_INPUT_INDEX
{
	...
	VIDDEC_DEFAULT_INPUT_INDEX_WMV2,
	VIDDEC_DEFAULT_INPUT_INDEX_VP9,
...
#define VIDDEC_COMPONENT_PRIVATE_FIELDS omx_base_filter_PrivateType_FIELDS \
  ...
  OMX_BOOL IsSWcodec   ;\
  OMX_U32 is_outputbuffer_flushing_pending ;\
  OMX_U32 is_inputbuffer_flushing_pending ;\
  OMX_BOOL is_fifo_disposing ;\
  OMX_BOOL is_error_status ;\
  OMX_BOOL is_videodec_fifo_init  ;\
  OMX_BOOL is_Thumbnail ;\
  OMX_BOOL is_h264_Thumbnail_outputed ;\
  ...
  OMX_U32  framebuffernum; \
  OMX_U32  videobuffersize ;\
  OMX_U32  err_counter; \
  OMX_S32  suspend_flag;   \
  OMX_BOOL bConfigPktStored;\
  OMX_BOOL bthirdparty;\
  OMX_BOOL bthirdpartychecked;\
  OMX_BOOL bStoreMediadata[NUM_OF_PORTS];\
  OMX_BOOL bResizeEnabled;\
  OMX_BOOL bResizeRealloced;\
  OMX_BOOL bComponentFlush;\
  OMX_BOOL bBufferNotEngouh;\
  OMX_BOOL bHdeExsit;\
  OMX_BOOL bLowRam;\
  OMX_U8* pstrbuf;\
