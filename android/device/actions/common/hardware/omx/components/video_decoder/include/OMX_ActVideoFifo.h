...
typedef struct port_s{
    ...
    frame_buf_handle *(*get_rbuf_postprocess)(struct port_s *port);
    void (*put_rbuf)(struct port_s *port);        
    void (*fifo_queue)(struct port_s *por,void *pAppPrivate,void *buffer,OMX_BOOL IsSWcodec);
    int (*is_fifo_bequeued)(struct port_s *por,void *pAppPrivate); 
    void (*dump_info)(struct port_s *port);
    int (*get_rbuf_num)(struct port_s *port);
    int (*get_rbuf_num_postprocess)(struct port_s *port);
...
int raw_fifo_init_useBuffer(port_t *port, void* data, int total_buf_num,unsigned int size,void *pAppPrivate,int is_ANativeWindow_buffer_handle,OMX_BOOL IsSWCodec);
