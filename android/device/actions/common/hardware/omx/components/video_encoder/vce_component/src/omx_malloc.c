...
int omx_munmap_ion_fd(void *vir_addr, int length)
{
	...
	if(ret != 0)
		DEBUG(DEB_LEV_ERR, "err!munmap fail,ret:%d! %s, %d\n", ret, __FILE__, __LINE__);
	return ret;
}
...
{
	int ret = 0;
	ret = IGralloc_getPhys((void *)handle, paddr);
...
{
	int ret = 0;
	ret = IGralloc_getBufferInfo((void *)handle, (int *)width, (int *)height, (int *)format, (int *)size);
...
{
	int ret = 0;
	ret = IGralloc_lock((void *)handle, usage, vaddr);
...
{
	int ret = 0;
	ret = IGralloc_unlock((void *)handle);
