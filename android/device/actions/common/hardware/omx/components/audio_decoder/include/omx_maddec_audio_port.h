...
#define DEFAULT_AUDIO_IN_BUFFER_SIZE  64 * 1024
/** Default size of the internal output buffer */
#define DEFAULT_AUDIO_OUT_BUFFER_SIZE 64 * 1024 /* TODO - check this size is ok */
...
DERIVEDCLASS(omx_maddec_audio_PortType, omx_base_PortType)
#define omx_maddec_audio_PortType_FIELDS omx_base_PortType_FIELDS \
  /** @param sAudioParam Domain specific (audio) OpenMAX port parameter */ \
  OMX_AUDIO_PARAM_PORTFORMATTYPE sAudioParam; 
ENDCLASS(omx_maddec_audio_PortType)
...
OMX_ERRORTYPE omx_audio_port_Constructor(
...
OMX_ERRORTYPE omx_audio_port_Destructor(
  omx_base_PortType *openmaxStandPort);
