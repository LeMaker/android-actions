...
//  printf("Out of %s for component %p\n", __func__, hComponent);
  return err;
}
...
//  printf("In %s for component %p\n", __func__, openmaxStandComp);
  ...
  prctl(PR_SET_NAME, (unsigned long)"BellagioMessage", 0, 0, 0);
  ...
  while(1){
    ...
    if(omx_base_component_Private->state == OMX_StateInvalid) {
        DEBUG(DEB_LEV_ERR,"In %s Destructor has been called. So exit from the loop\n", __func__);
      break;
    }
    /* Dequeue it */
