...
static void nv12ToYuv(uint8_t* dst, uint8_t* y, uint8_t* u,uint8_t* v, int width)
{
    uint8_t *uv;
    if (!dst || !y || !v || !u)
    {
...
static void nv21ToYuv(uint8_t* dst, uint8_t* y, uint8_t* u,uint8_t* v, int width)
{
    uint8_t *uv;
    if (!dst || !y || !v || !u)
    {
...
static void yx12ToYuv(uint8_t* dst, uint8_t* y, uint8_t* u,uint8_t* v, int width)
{
    if (!dst || !y || !v ||!v)
    {
        return;
...
static void yuyvToYuv(uint8_t* dst, uint8_t* y, uint8_t* u,uint8_t* v, int width)
{
    if (!dst || !y )
    {
        return;
...
static void rgb565ToRgb888(uint8_t* dst, uint8_t* src, int width)
{
    if (!dst || !src)
    {
        return;
...
static void uyvyToYuv(uint8_t* dst, uint32_t* src, int width)
{
    if (!dst || !src)
    {
        return;
...
static void resizeNV12(Encoder_libjpeg::params* params, uint8_t* src,  uint8_t* dst_buffer)
{
    RGBOrYUV_t in, out;
    ...
    in.y = src;
    in.u = src +  ALIGN16(params->orig_width) * params->orig_height;
    in.v = in.u + 1;

    out.y = dst_buffer;
    out.u = dst_buffer +  ALIGN16(params->out_width) * params->out_height;
    out.v = out.u + 1;

    resize.inputW = params->orig_width;
    resize.inputH = params->orig_height;
    resize.inputCrop.x = params->left_crop;
    resize.inputCrop.y = params->top_crop;
    resize.inputCrop.w =  params->in_width;
    resize.inputCrop.h = params->in_height;

    resize.outputW = params->out_width;
    resize.outputH = params->out_height;
    ...
    {
        memcpy(dst_buffer, src, (ALIGN16(resize.inputW)*resize.inputH*12)>>3);
    }
    else
    {
...
static void resizeNV21(Encoder_libjpeg::params* params, uint8_t* src,  uint8_t* dst_buffer)
{
    RGBOrYUV_t in, out;
    ImageResize_t resize;
    ...
    in.y = src;
    in.v = src +  ALIGN16(params->orig_width) * params->orig_height;
    in.u = in.v + 1;

    out.y = dst_buffer;
    out.v = dst_buffer +  ALIGN16(params->out_width) * params->out_height;
    out.u = out.v + 1;
...
static void resizeYUYV(Encoder_libjpeg::params* params, uint8_t* src,  uint8_t* dst_buffer)
{
    ...
    if((resize.inputW == resize.inputCrop.w)
        ...
        &&(resize.outputH == resize.inputH))
    {
        memcpy(dst_buffer, src, (resize.inputW*resize.inputH*16)>>3);
    }
    else
    {
...
static void resizeYU12(Encoder_libjpeg::params* params, uint8_t* src, uint8_t* dst_buffer)
{
    RGBOrYUV_t in, out;
    ImageResize_t resize;
    ...
    in.y = src;
    in.u = src +  ALIGN16(params->orig_width) * params->orig_height;
    in.v = in.u + ALIGN16(params->orig_width>>1) * (params->orig_height>>1);

    out.y = dst_buffer;
    out.u = dst_buffer +  ALIGN16(params->out_width) * params->out_height;
    out.v = out.u + ALIGN16( params->out_width>>1) * (params->out_height>>1);
    ...
    if((resize.inputW == resize.inputCrop.w)
        ...
        &&(resize.outputH == resize.inputH))
    {
        memcpy(dst_buffer, src, ALIGN16(resize.inputW)*resize.inputH+ALIGN16(resize.inputW>>1)*resize.inputH);
    }
    else
    {
...
static void resizeYV12(Encoder_libjpeg::params* params, uint8_t* src, uint8_t* dst_buffer)
{
    RGBOrYUV_t in, out;
    ImageResize_t resize;
    ...
    in.y = src;
    in.v = src +  ALIGN16(params->orig_width) * params->orig_height;
    in.u = in.v + ALIGN16( params->orig_width>>1) * ALIGN16(params->orig_height>>1);

    out.y = dst_buffer;
    out.v = dst_buffer +  ALIGN16(params->out_width) * params->out_height;
    out.u = out.v + ALIGN16( params->out_width>>1) * ALIGN16(params->out_height>>1);
...
static void resizeRGB565(Encoder_libjpeg::params* params, uint8_t* src,  uint8_t* dst_buffer)
{
    RGBOrYUV_t in, out;
    ImageResize_t resize;
...
void ExifElementsTable::insertExifToJpeg(unsigned char* jpeg, size_t jpeg_size)
{
    ...
    {
        jpeg_opened = true;
        create_EXIF(table, exif_tag_count, gps_tag_count, has_datetime_tag, has_focal_length_tag);
        //Once wirte exif, reset position and table value
		position = 0;
...
ExifElementsTable::~ExifElementsTable()
{
    ...
    CAMHAL_LOGVA("~ExifElementsTable");
    for (int i = 0; i < num_elements; i++)
    {
        CAMHAL_LOGVA("~ExifElementsTable2");
        if (table[i].Value)
        {
            CAMHAL_LOGVB("~ExifElementsTable, free=%p",table[i].Value);
            free(table[i].Value);
        }
    }
...
status_t ExifElementsTable::insertElement(const char* tag, const char* value)
{
    ...
    else
    {
        ...
        //Add table[position].Format
		if (strcmp(tag, TAG_FOCALLENGTH) == 0)
		{
            has_focal_length_tag = true;
			table[position].Format = FMT_URATIONAL;
		}
		if (strcmp(tag, TAG_MODEL) == 0
			|| strcmp(tag, TAG_MAKE) == 0
			|| strcmp(tag, TAG_DATETIME) == 0)
		{
			table[position].Format = FMT_STRING;
		}
		if (strcmp(tag, TAG_IMAGE_WIDTH) == 0
			|| strcmp(tag, TAG_IMAGE_LENGTH) == 0
			|| strcmp(tag, TAG_ORIENTATION) == 0)
		{
			table[position].Format = FMT_USHORT;
		}
    }
...
size_t Encoder_libjpeg::encode(params* input)
{
    ...
    uint8_t* row_uv = NULL, *row_u=NULL, *row_v=NULL; // used only for NV12
    int out_width = 0, in_width = 0, orig_width = 0;
    int out_height = 0, in_height = 0, orig_height = 0;
    int bpp = 2; // for uyvy
    int left_crop = 0, top_crop = 0, start_offset = 0;
    ...
    cameraStartTime(&start_time);

    out_width = input->out_width;
    in_width = input->in_width;
    orig_width = input->orig_width;
    out_height = input->out_height;
    in_height = input->in_height;
    orig_height= input->orig_height;
    left_crop = input->left_crop;
    top_crop = input->top_crop;
    start_offset = input->start_offset;
    input->jpeg_size = 0;

    //ActionsCode(author:liuyiguang, change_code)
    //src = (uint8_t *) ((unsigned int)input->src + input->src_offset);
    src = (uint8_t *) ((long)input->src + input->src_offset);
    ...
    CAMHAL_LOGEB("encode parameter:  \n\t"
                 ...
                 "dst size:%d \n\t"
                 "quality:%d \n\t",
                 orig_width,in_width, out_width, orig_height,in_height,
                 out_height, src,input->dst,input->left_crop,input->top_crop, input->src_size,
                 input->dst_size, input->quality);
    ...
    if ((in_width < 2) || (out_width < 2) || (in_height < 2) || (out_height < 2) ||
            ...
            (input->dst_size < 1))
    {
        CAMHAL_LOGEA("encode parameters error");
        goto exit;
    }

    resize_size = CameraFrame::getFrameLength((CameraFrame::FrameFormat)input->format, out_width, out_height);
    CAMHAL_LOGEB("resize size:%d", resize_size);

    if ((in_width != out_width) || (in_height != out_height)||(orig_width != out_width) || (orig_height != out_height))
    {
        need_resize =1;
    }
    ...
    switch(input->format)
    {
        case CameraFrame::CAMERA_FRAME_FORMAT_YUV420SP:
        {
            ...
            if (need_resize)
            {
                resize_src = (uint8_t*) malloc(resize_size);
                resizeNV21(input, src, resize_src);
        ...
        case CameraFrame::CAMERA_FRAME_FORMAT_YUV420SP_NV12:
        {
            ...
            {
                resize_src = (uint8_t*) malloc(resize_size);
                resizeNV12(input, src, resize_src);
                if (resize_src) src = resize_src;
            }
            break;
        ...
        case CameraFrame::CAMERA_FRAME_FORMAT_RGB565:
        {
            ...
            {
                resize_src = (uint8_t*) malloc(resize_size);
                resizeRGB565(input, src, resize_src);
                if (resize_src) src = resize_src;
            }
            break;
        }
        case CameraFrame::CAMERA_FRAME_FORMAT_YUV422I:
        {
            ...
            {
                resize_src = (uint8_t*) malloc(resize_size);
                resizeYUYV(input, src, resize_src);
                if (resize_src) src = resize_src;
            }
            break;
        ...
        case CameraFrame::CAMERA_FRAME_FORMAT_YUV420P:
        {
            ...
            {
                resize_src = (uint8_t*) malloc(resize_size);
                resizeYV12(input, src, resize_src);
                if (resize_src) src = resize_src;
            }
            break;
        }
        case CameraFrame::CAMERA_FRAME_FORMAT_YUV420P_YU12:
        {
            ...
            {
                resize_src = (uint8_t*) malloc(resize_size);
                resizeYU12(input, src, resize_src);
                if (resize_src) src = resize_src;
            }
            break;
        }
        default:
        {
            ALOGD("Encoder: resizing is not supported for this format: %d", input->format);
            goto exit;
        }
    }

    cameraDeltaTime("libjpeg resize", &start_time);
    ...
    if (input->format == CameraFrame::CAMERA_FRAME_FORMAT_RGB565)
    {
    ...
    row_uv = src + ALIGN16(out_width) * out_height * bpp;
    row_u = src + ALIGN16(out_width) * out_height * bpp;
    row_v = row_u + ALIGN16(out_width>>1) * (out_height>>1) * bpp;
    while ((cinfo.next_scanline < cinfo.image_height) )
    {
        ...
        switch(input->format)
        {
            case CameraFrame::CAMERA_FRAME_FORMAT_YUV420SP:
            {
                nv21ToYuv(row_tmp, row_src, row_uv+1, row_uv, out_width);
                row[0] = row_tmp;
                break;
            }
            case CameraFrame::CAMERA_FRAME_FORMAT_YUV420SP_NV12:
            {
                nv12ToYuv(row_tmp, row_src, row_uv,row_uv+1, out_width);
                row[0] = row_tmp;
                break;
            }
            case CameraFrame::CAMERA_FRAME_FORMAT_YUV420P:
            {
                yx12ToYuv(row_tmp, row_src, row_v, row_u, out_width);
                row[0] = row_tmp;
                break;
            }
            case CameraFrame::CAMERA_FRAME_FORMAT_YUV420P_YU12:
            {
                yx12ToYuv(row_tmp, row_src, row_u, row_v, out_width);
                row[0] = row_tmp;
                break;
            }
            case CameraFrame::CAMERA_FRAME_FORMAT_YUV422I:
            {
                yuyvToYuv(row_tmp, row_src, NULL, NULL, out_width);
                row[0] = row_tmp;
                break;
            }
            case CameraFrame::CAMERA_FRAME_FORMAT_RGB565:
            {
                rgb565ToRgb888(row_tmp, row_src, out_width);
                row[0] = row_tmp;
                break;
            }
            default:
            {
                ALOGD("Encoder: other format=%d", input->format);
                goto exit;
            }
        }
        ...
        if (input->format==CameraFrame::CAMERA_FRAME_FORMAT_YUV420SP
            || input->format==CameraFrame::CAMERA_FRAME_FORMAT_YUV420SP_NV12)
        {
            row_src = row_src + ALIGN16(out_width)*bpp;
            if (!(cinfo.next_scanline % 2))
                row_uv = row_uv +  ALIGN16(out_width) * bpp;
        }
        else if(input->format==CameraFrame::CAMERA_FRAME_FORMAT_YUV420P
            || input->format==CameraFrame::CAMERA_FRAME_FORMAT_YUV420P_YU12)
        {
            row_src = row_src + ALIGN16(out_width)*bpp;
            if (!(cinfo.next_scanline % 2))
            {
                row_u = row_u +  ALIGN16(out_width>>1) * bpp;
                row_v = row_v +  ALIGN16(out_width>>1) * bpp;
            }
        }
        else
        {
...
exit:
    ...
    cameraDeltaTime("libjpeg encode", &start_time);

    CAMHAL_LOGDB("Encoder: jpeg size: %d", dest_mgr.jpegsize);
