...
void CameraHal::PPM(const char* str)
{
    ...
    CAMHAL_LOGVB("PPM: %s :%ld.%ld ms", str, ( ppm.tv_sec /1000 ), ( ppm.tv_sec % 1000 ));
}
...
void CameraHal::PPM(const char* str)
{
    ...
    CAMHAL_LOGVB("PPM: %s :%llu.%llu ms : %llu ms", str, ( elapsed /1000 ), ( elapsed % 1000 ), absolute);
}
...
void CameraHal::PPM(const char* str, struct timeval* ppm_first, ...)
{
    ...
    CAMHAL_LOGVB("PPM: %s :%ld.%ld ms :  %llu ms", temp_str, ( ppm.tv_sec /1000 ), ( ppm.tv_sec % 1000 ), absolute);
