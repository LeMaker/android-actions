...
{   
    ...
    else if(pBuffer->nOutputPortIndex == OMX_CAMPORT_INDEX_VF)
    {
        ...
        {
            surface_display_main_show(pBuffer->pBuffer+pBuffer->nOffset, DEFAULT_FRAME_WIDTH, DEFAULT_FRAME_HEIGHT,DEFAULT_CAMERA_COLOR_FORMAT);
        }
...
int allocBufTest()
{
    ...
    for (nPortIndex = OMX_CAMPORT_INDEX_VF; nPortIndex <= OMX_CAMPORT_INDEX_CP; nPortIndex++)
    {
        ...
        {
            if ((err = OMX_AllocateBuffer(appPriv->camerahandle,
                    &sCameraPortBufferList[nPortIndex].pBufHeaderList[i],
                    nPortIndex, NULL, nBufferSize)) != OMX_ErrorNone)
            {
                DEBUG(DEB_LEV_ERR,
