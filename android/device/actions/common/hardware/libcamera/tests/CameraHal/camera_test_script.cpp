...
extern const thumb_size thumbSize [];
...
extern const char *strawb_mode[];
extern const char *flashModes[];
extern const char *scene[];
extern const char *iso[];
extern const char *effects[];
extern const char *antibanding[];
extern const char *focus[];

extern pixel_format pixelformat[];
extern const char *codingformat[];
...
extern size_t length_cameras;
extern char script_name[];
extern int restartCount;
extern bool bLogSysLinkTrace;
extern int bufferStarvationTest;
extern size_t length_outputFormat;
extern size_t length_videoCodecs;
extern size_t length_audioCodecs;
extern size_t length_VbitRate;

extern int frameRateIDX;


extern camera_supported_t camera_supporteds[];
    ...
    while ( NULL != cmd && (stopScript == false)) {
        ...
        switch (id) {
            ...
            case '4':
            {
                ...
                for(i = 0; i < GET_SUPPORTED(length_previewSize) ; i++)
                {
                    if( strcmp((cmd + 1), GET_SUPPORTED(previewSize)[i].desc) == 0)
                    {
                        width = GET_SUPPORTED(previewSize)[i].width;
                        height = GET_SUPPORTED(previewSize)[i].height;
                        previewSizeIDX = i;
                        break;
                    }
                }

                if(i < GET_SUPPORTED(length_previewSize))
                {
                ...
                {
                    printf("not support preview resolution:%s\n", (cmd+1));
                }
            ...
            case '5':
                ...
                for (i = 0; i < GET_SUPPORTED(length_captureSize); i++) {
                    if ( strcmp((cmd + 1), GET_SUPPORTED(captureSize)[i].name) == 0)
                        break;
                }

                if (  i < GET_SUPPORTED(length_captureSize) ) {
                    params.setPictureSize(GET_SUPPORTED(captureSize)[i].width, GET_SUPPORTED(captureSize)[i].height);
                    captureSizeIDX = i;
                    if ( hardwareActive )
                        camera->setParameters(params.flatten());
                }
                ...
                {
                    printf("not support capture resolution:%s\n", (cmd+1));
                }
            ...
            case '7':
                ...
                compensation = atof(cmd + 1)*3;

                if(compensation>=GET_SUPPORTED(compensationMin) && compensation <= GET_SUPPORTED(compensationMax))
                {
                ...
                {
                    printf("not support EV:%s\n", (cmd+1));
                }
                break;

            case '8':
                for (i = 0; i < GET_SUPPORTED(length_strawb_mode); i++) {
                    if ( strcmp((cmd + 1), GET_SUPPORTED(strawb_mode)[i]) == 0)
                        break;
                }
                if(i < GET_SUPPORTED(length_strawb_mode))
                {
            ...
            case '9':
                for(i = 0; i < length_videoCodecs; i++)
                {
                ...
                if(i >= length_videoCodecs)
                {
                    printf("not support VideoCodecs:%s\n", (cmd+1));
                }
                break;

            case 'v':
                for(i = 0; i < length_outputFormat; i++)
                {
                ...
                if(i >= length_outputFormat)
                {
                    printf("not support outputFormat:%s\n", (cmd+1));
                }
            break;

            case '~':
                for(i = 0; i < GET_SUPPORTED(length_pixelformat); i++)
                {
                    if( strcmp((cmd + 1), GET_SUPPORTED(pixelformat)[i].pixformat) == 0)
                    {
                ...
                if(i >= GET_SUPPORTED(length_pixelformat))
                {
                    printf("not support previewformat:%s\n", (cmd+1));
                }
            ...
            case '$':
                for(i = 0; i < GET_SUPPORTED(length_codingformat); i++)
                {
                    if( strcmp((cmd + 1), GET_SUPPORTED(codingformat)[i]) == 0)
                    { 
                ...
                if(i >= GET_SUPPORTED(length_codingformat))
                {
                    printf("not support pictureformat:%s\n", (cmd+1));
                }

                break;
            case '-':
                for(i = 0; i < length_audioCodecs; i++)
                {
                ...
                if(i >= length_audioCodecs)
                {
                    printf("not support audioCodec:%s\n", (cmd+1));
                }
                break;

            case 'A':
                camera_index=atoi(cmd+1);
                if(camera_index < (int)length_cameras)
                {
                ...
                {
                    printf("not support camera index:%s\n", (cmd+1));
                }
            ...
            case 'l':
            case 'L':
                for(i = 0; i < GET_SUPPORTED(length_VcaptureSize); i++)
                {
                    if( strcmp((cmd + 1), GET_SUPPORTED(VcaptureSize)[i].desc) == 0)
                    {
                        VcaptureSizeIDX = i;
                        printf("Video Capture Size: %s\n", GET_SUPPORTED(VcaptureSize)[i].desc);
                        break;
                    }
                }
                if(i >= GET_SUPPORTED(length_VcaptureSize))
                {
                    printf("not support capture resolution:%s\n", (cmd+1));
                }
                break;
            case ']':
                for(i = 0; i < length_VbitRate; i++)
                {
                ...
                if(i >= length_VbitRate)
                {
                    printf("not support VbitRate:%s\n", (cmd+1));
                }
                break;
            case ':':
                int width, height;
                for(i = 0; i < GET_SUPPORTED(length_thumbSize) ; i++)
                {
                    if( strcmp((cmd + 1), GET_SUPPORTED(thumbSize)[i].desc) == 0)
                    {
                        width = GET_SUPPORTED(thumbSize)[i].width;
                        height = GET_SUPPORTED(thumbSize)[i].height;
                        thumbSizeIDX = i;
                        break;
                    }
                }
                ...
                if (i <  GET_SUPPORTED(length_thumbSize) )   //if the resolution is not in the supported ones
                {
                ...
                {
                    printf("not support thumbSize:%s\n", (cmd+1));
                }
            ...
            case 'J':
                ...
                for(i = 0; i < GET_SUPPORTED(length_flashModes) ; i++)
                {
                    if( strcmp((cmd + 1), GET_SUPPORTED(flashModes)[i]) == 0)
                    { 
                ...
                if(i >= GET_SUPPORTED(length_flashModes))
                {
                    printf("not support flashmodes:%s\n", (cmd+1));
                }

                break;

            case 'w':
                for(i = 0; i < GET_SUPPORTED(length_scene) ; i++)
                {
                    if( strcmp((cmd + 1), GET_SUPPORTED(scene)[i]) == 0)
                    { 
                        break;
                    }
                }
                if( i < GET_SUPPORTED(length_scene))
                {
            ...
            case 'i':
                for(i = 0; i < GET_SUPPORTED(length_iso) ; i++)
                {
                    if( strcmp((cmd + 1), GET_SUPPORTED(iso)[i]) == 0)
                    { 
                        break;
                    }
                }
                if(i < GET_SUPPORTED(length_iso))
                {
            ...
            case 'h':
                ...
                if(sharpness >= GET_SUPPORTED(sharpnessMin) && sharpness <= GET_SUPPORTED(sharpnessMax))
                {
                ...
                {
                    printf("not support sharpness:%s\n", (cmd+1));
                }
            ...
            case 'c':
                ...
                if(contrast >= GET_SUPPORTED(contrastMin) && contrast <= GET_SUPPORTED(contrastMax))
                {
            ...
            case 'Z':
                ...
                for(i = 0; i < GET_SUPPORTED(length_zoom); i++)
                {
                    if( strcmp((cmd + 1), GET_SUPPORTED(zoom)[i].zoom_description) == 0)
                    {
                        zoomIDX = i;
                        break;
                    }
                }
                if(i < GET_SUPPORTED(length_zoom))
                {
            ...
            case 'b':
                ...
                if(brightness >= GET_SUPPORTED(brightnessMin) && brightness <= GET_SUPPORTED(brightnessMax))
                {
                ...
                {
                    printf("not support brightness:%s\n", (cmd+1));
                }
            ...
            case 's':
                ...
                if(saturation >= GET_SUPPORTED(saturationMin) && saturation <= GET_SUPPORTED(saturationMax))
                {
                ...
                {
                    printf("not support saturation:%s\n", (cmd+1));
                }

                break;

            case 'e':
                for(i = 0; i < GET_SUPPORTED(length_effects); i++)
                {
                    if( strcmp((cmd + 1), GET_SUPPORTED(effects)[i]) == 0)
                    {
                        break;
                    }
                }
                if( i < GET_SUPPORTED(length_effects) )
                {
            ...
            case 'R':
                ...
                for (i = 0; i < GET_SUPPORTED(length_fpsConstRanges); i++) {
                    if (frameR == GET_SUPPORTED(fpsConstRanges)[i].constFramerate)
                    {
                        frameRateIndex = i;
                        isvalid = 1;
                        break;
                ...
                if(isvalid == 1)
                {
                    params.set(CameraParameters::KEY_PREVIEW_FPS_RANGE, GET_SUPPORTED(fpsConstRanges)[frameRateIndex].range);
                ...
                {
                    printf("not support framerate:%s\n", (cmd+1));
            ...
            case 'x':
                for(i = 0; i < GET_SUPPORTED(length_antibanding); i++)
                {
                    if( strcmp((cmd + 1), GET_SUPPORTED(antibanding)[i]) == 0)
                    {
                        break;
                    }
                }      
                
                if(i < GET_SUPPORTED(length_antibanding))
                {
                ...
                {
                    printf("not support atibanding:%s\n", (cmd+1));
                }

                break;

            case 'g':
                for(i = 0; i < GET_SUPPORTED(length_focus); i++)
                {
                    if( strcmp((cmd + 1), GET_SUPPORTED(focus)[i]) == 0)
                    {
                        break;
                    }
                }   
                if(i < GET_SUPPORTED(length_focus))
                {
