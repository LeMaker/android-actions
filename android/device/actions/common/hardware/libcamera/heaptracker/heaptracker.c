...
void print_backtrace(intptr_t *bt, int depth)
{
    const mapinfo *mi;
    int cnt, rel_pc;
    ...
    for (cnt = 0; cnt < depth && cnt < MAX_BACKTRACE_DEPTH; cnt++) {
        mi = pc_to_mapinfo(milist, bt[cnt], (unsigned *)&rel_pc);
...
void heaptracker_show_leaked_memory(void)
{
    struct hdr *show; int cnt;
    ...
    while (show) {
        malloc_log("+++ DELETING %d BYTES OF LEAKED MEMORY AT %p (%d REMAINING)\n",
            show->size, user(show), num);
        malloc_log("+++ ALLOCATION %p SIZE %d ALLOCATED HERE:\n",
            user(show), show->size);
        print_backtrace(show->bt, show->bt_depth);
        show = show->next;
    }
    pthread_rwlock_unlock(&lock);
