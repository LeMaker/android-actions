...
**  Externs
******************************************************************************/
static int hciVersion = HCI_VERSION_H5;
#ifdef HCI_USE_MCT
...
inline int get_num_hci_cmd_pkts(void) {
#ifdef HCI_USE_MCT
	return num_hci_cmd_pkts;
#else
	return ((HCI_VERSION_H5 == hciVersion)? h5_num_hci_cmd_pkts : h4_num_hci_cmd_pkts);
#endif
}
  ...
  while (p_next_msg && sending_msg_count < ARRAY_SIZE(sending_msg_que))
  {
    if ((p_next_msg->event & MSG_EVT_MASK)==MSG_STACK_TO_HC_HCI_CMD)
    {
      ...
      if (tx_cmd_pkts_pending ||
          (sending_hci_cmd_pkts_count >= get_num_hci_cmd_pkts()))
      {
...
static void event_rx(UNUSED_ATTR void *context) {
  ...
  if (tx_cmd_pkts_pending && (get_num_hci_cmd_pkts() > 0)) {
...
static int init(const bt_hc_callbacks_t* p_cb, unsigned char *local_bdaddr)
{
	...
	hciVersion = vendor_get_hci_version();
