...
// btla-specific --
			...
			//ALOGI("GKI_getbuf req size:%d, alloc size:%d", size, Q->size);
            return ((void *) ((UINT8 *)p_hdr + BUFFER_HDR_SIZE));
        }
		else
		{
			ALOGI("GKI_getbuf can't get exact buffer,size:%d,Q size:%d", size, Q->size);
		}
    }

    GKI_enable();

	ALOGI("GKI_getbuf can't get buffer,size:%d", size);
