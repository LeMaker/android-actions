...
#define		VENDOR_LIB_NUM		2
static const char *VENDOR_LIBRARY_NAME[] = {"libbt-vendor-rtl8723bu.so",	\
											"libbt-vendor-rtl8723bs.so"
											};
...
bool vendor_open(const uint8_t *local_bdaddr) {
  ...
  for (i=0; i<VENDOR_LIB_NUM; i++) {
  	 lib_handle = dlopen(VENDOR_LIBRARY_NAME[i], RTLD_NOW);
	 if (!lib_handle) {
    	ALOGE("%s unable to open %s: %s", __func__, VENDOR_LIBRARY_NAME[i], dlerror());
    	continue;
	 }

	 vendor_interface = (bt_vendor_interface_t *)dlsym(lib_handle, VENDOR_LIBRARY_SYMBOL_NAME);
	 if (!vendor_interface) {
	    ALOGE("%s unable to find symbol %s in %s: %s", __func__, VENDOR_LIBRARY_SYMBOL_NAME, VENDOR_LIBRARY_NAME[i], dlerror());
		dlclose(lib_handle);
		lib_handle = NULL;
		continue;
	 }

	 ret = vendor_interface->detect_bluetooth();
	 if (ret) {
	 	ALOGI("Find bluetooth: %s", VENDOR_LIBRARY_NAME[i]);
	 	break;
	 } else {
...
int vendor_get_hci_version(void) {
	return vendor_interface->get_hci_version();
}
