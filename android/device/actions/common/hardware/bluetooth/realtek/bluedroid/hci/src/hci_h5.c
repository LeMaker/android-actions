...
static void h5_timeout_handler(union sigval sv);
...
static timer_t OsAllocateTimer(int signo)
{
	...
	sigev.sigev_notify_function = h5_timeout_handler;
...
static void h5_timeout_handler(union sigval sv)
 {
    ...
    if (sv.sival_int == TIMER_H5_DATA_RETRANS)
    {
        h5_retransfer_signal_event(H5_EVENT_RX);
    }
    else
    if (sv.sival_int == TIMER_H5_SYNC_RETRANS)
    {
    ...
    if (sv.sival_int == TIMER_H5_CONF_RETRANS)
    {
    ...
    if (sv.sival_int == TIMER_H5_WAIT_CT_BAUDRATE_READY)
    {
    ...
    if (sv.sival_int == TIMER_H5_HW_INIT_READY)
    {
    ...
    {
        ALOGE("H5 timer rx unspported signo(%d)", sv.sival_int);
    }
}
