...
static timer_t OsAllocateTimer(int signo)
{
	...
	sigev.sigev_notify_function = timeout_handler;
...
static void timeout_handler(union sigval sv)
{
	...
	else if (sv.sival_int == TIMER_A2DP_PACKET_COUNT)
	{
	...
	else if (sv.sival_int == TIMER_HOGP_PACKET_COUNT)
	{
	...
	else if (sv.sival_int == TIMER_PAN_PACKET_COUNT)
	{
	...
	{
		ALOGE("rtk_parse_data timer unspported signo(%d)", sv.sival_int);
	}
}
