...
extern int recv_byte_total;
extern int send_byte_total;
...
static int get_recv_byte(void)
{
    if (interface_ready() == FALSE)
        return BT_STATUS_NOT_READY;    
    
    return recv_byte_total;
 
}

static int get_send_byte(void)
{
    if (interface_ready() == FALSE)
        return BT_STATUS_NOT_READY;    
    return send_byte_total;
}
