...
sensors_poll_context_t::sensors_poll_context_t() {
    ...
    for (i = 0; i < numSensorDrivers; i++) {
        memset(&(mPollFds[i]), 0, sizeof(pollfd));
        mPollFds[i].fd = -1;
    }
...
if(mCompassSensor->getFd() > 0){
    mPollFds[compass].fd = mCompassSensor->getFd();
    mPollFds[compass].events = POLLIN;
    mPollFds[compass].revents = 0;
}

if(((MPLSensor*) mSensor)->getDmpOrientFd() > 0){
    mPollFds[dmpOrient].fd = ((MPLSensor*) mSensor)->getDmpOrientFd();
    mPollFds[dmpOrient].events = POLLPRI;
    mPollFds[dmpOrient].revents = 0;
}

if(((MPLSensor*) mSensor)->getDmpSignificantMotionFd() > 0){
    mPollFds[dmpSign].fd = ((MPLSensor*) mSensor)->getDmpSignificantMotionFd();
    mPollFds[dmpSign].events = POLLPRI;
    mPollFds[dmpSign].revents = 0;
}

if(((MPLSensor*) mSensor)->getDmpPedometerFd() > 0){
    mPollFds[dmpPed].fd = ((MPLSensor*) mSensor)->getDmpPedometerFd();
    mPollFds[dmpPed].events = POLLPRI;
    mPollFds[dmpPed].revents = 0;
}
    int LightSenIdx = findIndexByHandle(SENSORS_LIGHT_HANDLE,sSensorList, sizeof(sSensorList));

    if(LightSenIdx < 0)
    {
        ALOGI("not found LightSensor in support list,LightSenIdx = %d",LightSenIdx);
    }
    else
    {
        ALOGD("cj+++ LightSenIdx = %d",LightSenIdx);
        ALOGD("cj+++LightSensor:: init a instance");
        //ALOGD("sSensorList[LightSen].name: %s",(char*)sSensorList[LightSenIdx].name);
        //ALOGD("sSensorList[LightSen].vendor: %s",(char*)sSensorList[LightSenIdx].vendor);
        //ALOGD("sSensorList[LightSen].reserved[0]: %s",(char*)sSensorList[LightSenIdx].reserved[0]);
        mLightSensor = new LightSensor((char*)sSensorList[LightSenIdx].reserved[0],sSensorList[LightSenIdx].resolution,sSensorList[LightSenIdx].minDelay);//cj
...
int sensors_poll_context_t::activate(int handle, int enabled) {
    ...
    err = mLightSensor->enable(handle, enabled);
	}else{
                                  ...
                                  int64_t timeout)
{
	...
	if(handle == ID_L)
		{
			ALOGD("cj+++batch=====");
			return mLightSensor->batch(handle, flags, period_ns, timeout);
		}
	else
		{
...
int sensors_poll_context_t::flush(int handle)
{
	...
	if(handle == ID_L)
		{
		ALOGD("cj+++flush=====");
			return mLightSensor->flush(handle);
		}
	else*/
		{
