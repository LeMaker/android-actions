...
struct sysfs_attrbs {
    char *name;
    char *buffer_enable;
    int buffer_enable_value;
    char *master_enable;
    int master_enable_value;
    char *power_state;
...
static bool clear_biases_immediately = false;
...
int main(int argc, char **argv)
{
    ...
    while ((c = getopt(argc, argv, "lwch")) != -1) {
        switch (c) {
        case 'l':
            printf("option:  -l \n");
        ...
        case 'w':
            ...
            break;
       case 'c':
            printf("option:  -c  -> clear the MLCAL_FILE \n");
            clear_biases_immediately = true;
        ...
        case 'h':
            printf("###############   menus   #############\n");
            printf("##### l:load MLCAL_FILE           #####\n");
            printf("##### w:write biases immediately  #####\n");
            printf("##### c:clear the MLCAL_FILE      #####\n");
            printf("##### h:help                      #####\n");
            printf("############### menus end #############\n");
            return -1;
        }
    ...
    /*
    if (save_n_write_sysfs_int(mpu.master_enable, 0, &mpu.master_enable_value) < 0) {
        printf("Self-Test:ERR-Failed to disable master_enable ...\n");
    }
    */
    if (save_n_write_sysfs_int(mpu.buffer_enable, 0, &mpu.buffer_enable_value) < 0) {
        printf("Self-Test:ERR-Failed to disable buffer/enable\n");
    }
    ...
    if (write_sysfs_int(mpu.buffer_enable, 1) < 0) {
        printf("Self-Test:ERR-Failed to enable buffer/enable\n");
    }

    /*
    if (write_sysfs_int(mpu.master_enable, 1) < 0) {
        printf("Self-Test:ERR-Failed to enable master_enable ...\n");
    }
    */
    if(clear_biases_immediately){
        clear_biases_immediately = false;
        if (write_sysfs_int(mpu.accel_x_offset, 0) < 0) {
            printf("Self-Test:ERR-Failed to write %s\n", mpu.accel_x_offset);
        }
        if (write_sysfs_int(mpu.accel_y_offset, 0) < 0) {
            printf("Self-Test:ERR-Failed to write %s\n", mpu.accel_y_offset);
        }
        if (write_sysfs_int(mpu.accel_z_offset, 0) < 0) {
            printf("Self-Test:ERR-Failed to write %s\n", mpu.accel_z_offset);
        }
        if (write_sysfs_int(mpu.gyro_x_offset, 0) < 0) {
            printf("Self-Test:ERR-Failed to write %s\n", mpu.gyro_x_offset);
        }
        if (write_sysfs_int(mpu.gyro_y_offset, 0) < 0) {
            printf("Self-Test:ERR-Failed to write %s\n", mpu.gyro_y_offset);
        }
        if (write_sysfs_int(mpu.gyro_z_offset, 0) < 0) {
            printf("Self-Test:ERR-Failed to write %s\n", mpu.gyro_z_offset);
        }
        goto restore_settings;
    }

    printf("####  fopen -> mpu.self_test \n");
    fptr = fopen(mpu.self_test, "r");
    if (!fptr) {
    ...
    */
    ...
    printf("Self-Test:factory_gyro_bias[0..2] = [%ld %ld %ld]\n", 
                   save_data.factory_gyro_bias[0], save_data.factory_gyro_bias[1], save_data.factory_gyro_bias[2]);
...
restore_settings:
    ...
    if (write_sysfs_int(mpu.buffer_enable, mpu.buffer_enable_value) < 0) {
        printf("Self-Test:ERR-Failed to restore buffer/enable\n");
    }

    /*
    if (write_sysfs_int(mpu.master_enable, mpu.master_enable_value) < 0) {

        printf("Self-Test:ERR-Failed to restore master_enable\n");
    }
    */
    if (write_sysfs_int(mpu.master_enable, 1) < 0) {
        printf("Self-Test:ERR-Failed to enable master_enable ...\n");
