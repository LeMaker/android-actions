...
int sensors_detect_devices_input(const struct sensor_t* slist, int ssize,
                                struct sensor_t* clist, int csize)
{
        ...
        //ALOGD("sensors_detect_devices_input...");
        ...
        *filename++ = '/';
        ...
        //ALOGD("devname:%s",devname);
        //ALOGD("filename:%s",filename);
        
        while((de = readdir(dir))) {
            //ALOGD("de->d_name[0]:%s",(char*)(de->d_name));
            if(de->d_name[0] == '.' &&
                    (de->d_name[1] == '\0' ||
            ...
            if (fd>=0) {
                ...
                //ALOGD("name:%s",name);
                //ALOGD("ssize:%d,csize:%d",ssize,csize);
                for (idx = 0; idx < ssize; idx++) {
                    //("slist[%d].name: %s",idx ,(char*)slist[idx].name);
                    if (!strcmp(name, (char*)(slist[idx].reserved[0]))) {
                        ALOGD("input detectd:%s",name);
...
/** read a sysfs entry that represents an string*/
int read_sysfs_str(char *filename, char str[])
{
    int res=0; 
    FILE *fp;
    ...
    if (fp != NULL) { 
        ...
        ALOGE("ERR open file to read:%s",filename);
        res= -1;
    }
    return res;
}

int sensors_detect_devices_iio(const struct sensor_t* slist, int ssize,
                                struct sensor_t* clist, int csize)
{
    ...
    char *devname = "/sys/bus/iio/devices/iio:device0";

    //ALOGD("sensors_detect_devices_iio...");
    filename1 = "/sys/bus/iio/devices/iio:device0/name";
    filename2 = "/sys/bus/iio/devices/iio:device0/secondary_name";

    //ALOGD("filename1:%s",filename1);
    //ALOGD("filename2:%s",filename2);

	read_sysfs_str(filename1,name);
    read_sysfs_str(filename2,secondary_name);
    ALOGD("iio detected name:%s",name);
    ALOGD("iio detected secondary_name:%s",secondary_name);

    //ALOGD("ssize:%d,csize:%d",ssize,csize);
    for (int idx = 0; idx < ssize; idx++) {
        //ALOGD("slist[%d].name: %s",idx ,(char*)slist[idx].name);
        if ((!strcmp(name, (char*)slist[idx].reserved[0])) || (!strcmp(secondary_name, (char*)slist[idx].reserved[0]))) {
            memcpy(&clist[count], &slist[idx], sizeof(struct sensor_t));
            count ++;
         }
    }
    
    //ALOGD("sensors_detect_devices_iio...finish");

    return count;
}

int sensors_detect_devices(const struct sensor_t* slist, int ssize,
                                struct sensor_t* clist, int csize)
{
    int numSensors_input = 0;
    int numSensors_iio = 0;
    int numsensors = 0;
    int i=0;
    struct sensor_t supportSensorList_input[LOCAL_SENSORS];
    struct sensor_t supportSensorList_iio[LOCAL_SENSORS];

    //ALOGD("sensors_detect_devices...");

    numSensors_input = sensors_detect_devices_input(slist,ssize,supportSensorList_input,ARRAY_SIZE(supportSensorList_input));
    numSensors_iio = sensors_detect_devices_iio(slist,ssize,supportSensorList_iio,ARRAY_SIZE(supportSensorList_iio));
    
    
    //ALOGD("numSensors_input:%d",numSensors_input);
    //ALOGD("numSensors_iio:%d",numSensors_iio);
    
    /*
    for(i=0;i<numSensors_input;i++)
    {
        ALOGD("supportSensorList_input[%d].name: %s",i,(char*)supportSensorList_input[i].name);
        ALOGD("supportSensorList_input[%d].vendor: %s",i,(char*)supportSensorList_input[i].vendor);
        ALOGD("supportSensorList_input[%d].reserved[0]: %s",i,(char*)supportSensorList_input[i].reserved[0]);
        
    }
    */
    /*
    for(i=0;i<numSensors_iio;i++)
    {
        ALOGD("supportSensorList_iio[%d].name: %s",i,(char*)supportSensorList_iio[i].name);
        ALOGD("supportSensorList_iio[%d].vendor: %s",i,(char*)supportSensorList_iio[i].vendor);
        ALOGD("supportSensorList_iio[%d].reserved[0]: %s",i,(char*)supportSensorList_iio[i].reserved[0]);
        
    }
    */

    numsensors = numSensors_input + numSensors_iio;
    //ALOGD("numsensors:%d",numsensors);
    
    memcpy(clist, supportSensorList_input,
           sizeof(struct sensor_t) * numSensors_input);
    
    memcpy(&clist[numSensors_input], supportSensorList_iio,
           sizeof(struct sensor_t) * numSensors_iio);
    
    return numsensors;
}


int findIndexByHandle(int handle,struct sensor_t *list,int len)
{
    int idx=-1;

    for(int i=0;i < len/ sizeof(sensor_t);i++)
    {
        if((int)list[i].handle == handle)
        {
            idx = i;  
            break;
        }
...
int MPLSensor::populateSensorList(struct sensor_t *list, int len)
{
    ...
    int numsensors = 0;
    struct sensor_t acturalSupportSensorList[LOCAL_SENSORS];

    //ALOGD("MPLSensor::populateSensorList...");

    if (numsensors <= 0) {
        numsensors = sensors_detect_devices(sBaseSensorList, ARRAY_SIZE(sBaseSensorList),
                                   acturalSupportSensorList, ARRAY_SIZE(acturalSupportSensorList));

    }
    /*
    for(int i=0;i<numsensors;i++)
    {
        ALOGD("acturalSupportSensorList[%d].name: %s",i,(char*)acturalSupportSensorList[i].name);
        ALOGD("acturalSupportSensorList[%d].vendor: %s",i,acturalSupportSensorList[i].vendor);
        ALOGD("acturalSupportSensorList[%d].reserved[0]: %s",i,(char*)acturalSupportSensorList[i].reserved[0]);
        
    }
    */

    if(len < 
        (int)(numsensors * sizeof(sensor_t))) {
        LOGE("HAL:sensor list too small, not populating.");
        return -numsensors;
    }

     memcpy(list, acturalSupportSensorList,
           sizeof (struct sensor_t) * numsensors);
    /*
    for(int i=0;i<numsensors;i++)
    {
        ALOGD("list[%d].name: %s",i,(char*)list[i].name);
        ALOGD("list[%d].vendor: %s",i,list[i].vendor);
        ALOGD("list[%d].handle: %d",i,list[i].handle);
        ALOGD("list[%d].maxRange: %f",i,list[i].maxRange);
        ALOGD("list[%d].resolution: %f",i,list[i].resolution);
        ALOGD("list[%d].power: %f",i,list[i].power);
        ALOGD("list[%d].minDelay: %d",i,list[i].minDelay);
        ALOGD("list[%d].reserved[0]: %s",i,(char*)list[i].reserved[0]);
    ...
    */
    ...
    fillGyro(chip_ID, list,len);
    fillAccel(chip_ID, list,len);

    // TODO: need fixes for unified HAL and 3rd-party solution

    int MagneticFieldIdx = findIndexByHandle(SENSORS_MAGNETIC_FIELD_HANDLE,list,len);
    int RawMagneticFieldIdx = findIndexByHandle(SENSORS_RAW_MAGNETIC_FIELD_HANDLE,list,len);
    if(MagneticFieldIdx < 0)
    {
         ALOGE("not found MagneticField,don't fill MagneticField) -> idx= %d",MagneticFieldIdx); 
    }
    else
    {
        mCompassSensor->fillList(&list[MagneticFieldIdx]);
    }
    if(RawMagneticFieldIdx < 0)
    {
         ALOGE("not found RawMagneticField,don't fill RawMagneticField -> idx= : %d",RawMagneticFieldIdx); 
    }
    else
    {
        mCompassSensor->fillList(&list[RawMagneticFieldIdx]);
    }
    

    if (mPressureSensor != NULL) {
        int PressureIdx = findIndexByHandle(SENSORS_PRESSURE_HANDLE,list,len);
        if(PressureIdx < 0)
        {
             ALOGI("not found Pressure,don't fill Pressure -> idx= %d",PressureIdx); 
        }
        else
        {
            mPressureSensor->fillList(&list[PressureIdx]);
        }
...
void MPLSensor::fillAccel(const char* accel, struct sensor_t *list,int len)
{
    VFUNC_LOG;
    int AccelerometerIdx;

    AccelerometerIdx = findIndexByHandle(SENSORS_ACCELERATION_HANDLE,list,len);

    if(AccelerometerIdx < 0)
    {
         ALOGE("not found Accelerometer,fillAccel failed -> idx= %d",AccelerometerIdx); 
         return;
    }
    ...
    if (accel) {
        if(accel != NULL && strcmp(accel, "BMA250") == 0) {
            list[AccelerometerIdx].maxRange = ACCEL_BMA250_RANGE;
            list[AccelerometerIdx].resolution = ACCEL_BMA250_RESOLUTION;
            list[AccelerometerIdx].power = ACCEL_BMA250_POWER;
            list[AccelerometerIdx].minDelay = ACCEL_BMA250_MINDELAY;
            return;
        } else if (accel != NULL && strcmp(accel, "MPU6050") == 0) {
            list[AccelerometerIdx].maxRange = ACCEL_MPU6050_RANGE;
            list[AccelerometerIdx].resolution = ACCEL_MPU6050_RESOLUTION;
            list[AccelerometerIdx].power = ACCEL_MPU6050_POWER;
            list[AccelerometerIdx].minDelay = ACCEL_MPU6050_MINDELAY;
            return;
        } else if (accel != NULL && strcmp(accel, "MPU6500") == 0) {
            list[AccelerometerIdx].maxRange = ACCEL_MPU6500_RANGE;
            list[AccelerometerIdx].resolution = ACCEL_MPU6500_RESOLUTION;
            list[AccelerometerIdx].power = ACCEL_MPU6500_POWER;
            list[AccelerometerIdx].minDelay = ACCEL_MPU6500_MINDELAY;
            return;
            ...
            list[AccelerometerIdx].maxRange = ACCEL_MPU9150_RANGE;
            list[AccelerometerIdx].resolution = ACCEL_MPU9150_RESOLUTION;
            list[AccelerometerIdx].power = ACCEL_MPU9150_POWER;
            list[AccelerometerIdx].minDelay = ACCEL_MPU9150_MINDELAY;
            return;
        } else if (accel != NULL && strcmp(accel, "MPU9250") == 0) {
            list[AccelerometerIdx].maxRange = ACCEL_MPU9250_RANGE;
            list[AccelerometerIdx].resolution = ACCEL_MPU9250_RESOLUTION;
            list[AccelerometerIdx].power = ACCEL_MPU9250_POWER;
            list[AccelerometerIdx].minDelay = ACCEL_MPU9250_MINDELAY;
            return;
        } else if (accel != NULL && strcmp(accel, "MPU9350") == 0) {
            list[AccelerometerIdx].maxRange = ACCEL_MPU9350_RANGE;
            list[AccelerometerIdx].resolution = ACCEL_MPU9350_RESOLUTION;
            list[AccelerometerIdx].power = ACCEL_MPU9350_POWER;
            list[AccelerometerIdx].minDelay = ACCEL_MPU9350_MINDELAY;
            return;
...
void MPLSensor::fillGyro(const char* gyro, struct sensor_t *list,int len)
{
    VFUNC_LOG;
    int gyroIdx;
    int rawGyroIdx;

    gyroIdx = findIndexByHandle(SENSORS_GYROSCOPE_HANDLE,list,len);
    rawGyroIdx = findIndexByHandle(SENSORS_RAW_GYROSCOPE_HANDLE,list,len);

    if(gyroIdx < 0)
    {
         ALOGE("not found Gyro,fillGyro failed -> idx= %d",gyroIdx); 
         return;
    }
    if(rawGyroIdx < 0)
    {
         ALOGE("not found RawGyro,,fillGyro failed -> idx= : %d",rawGyroIdx); 
         return;
    }

    if ( gyro != NULL && strcmp(gyro, "MPU3050") == 0) {
        list[gyroIdx].maxRange = GYRO_MPU3050_RANGE;
        list[gyroIdx].resolution = GYRO_MPU3050_RESOLUTION;
        list[gyroIdx].power = GYRO_MPU3050_POWER;
        list[gyroIdx].minDelay = GYRO_MPU3050_MINDELAY;
    } else if( gyro != NULL && strcmp(gyro, "MPU6050") == 0) {
        list[gyroIdx].maxRange = GYRO_MPU6050_RANGE;
        list[gyroIdx].resolution = GYRO_MPU6050_RESOLUTION;
        list[gyroIdx].power = GYRO_MPU6050_POWER;
        list[gyroIdx].minDelay = GYRO_MPU6050_MINDELAY;
    } else if( gyro != NULL && strcmp(gyro, "MPU6500") == 0) {
        list[gyroIdx].maxRange = GYRO_MPU6500_RANGE;
        list[gyroIdx].resolution = GYRO_MPU6500_RESOLUTION;
        list[gyroIdx].power = GYRO_MPU6500_POWER;
        list[gyroIdx].minDelay = GYRO_MPU6500_MINDELAY;
        ...
        list[gyroIdx].maxRange = GYRO_MPU9150_RANGE;
        list[gyroIdx].resolution = GYRO_MPU9150_RESOLUTION;
        list[gyroIdx].power = GYRO_MPU9150_POWER;
        list[gyroIdx].minDelay = GYRO_MPU9150_MINDELAY;
    } else if( gyro != NULL && strcmp(gyro, "MPU9250") == 0) {
        list[gyroIdx].maxRange = GYRO_MPU9250_RANGE;
        list[gyroIdx].resolution = GYRO_MPU9250_RESOLUTION;
        list[gyroIdx].power = GYRO_MPU9250_POWER;
        list[gyroIdx].minDelay = GYRO_MPU9250_MINDELAY;
    } else if( gyro != NULL && strcmp(gyro, "MPU9350") == 0) {
        list[gyroIdx].maxRange = GYRO_MPU9350_RANGE;
        list[gyroIdx].resolution = GYRO_MPU9350_RESOLUTION;
        list[gyroIdx].power = GYRO_MPU9350_POWER;
        list[gyroIdx].minDelay = GYRO_MPU9350_MINDELAY;
    } else {
    ...
    list[rawGyroIdx].maxRange = list[gyroIdx].maxRange;
    list[rawGyroIdx].resolution = list[gyroIdx].resolution;
    list[rawGyroIdx].power = list[gyroIdx].power;
    list[rawGyroIdx].minDelay = list[gyroIdx].minDelay;
...
void MPLSensor::fillRV(struct sensor_t *list,int len)
{
    VFUNC_LOG;

    int RotationVectorIdx = findIndexByHandle(SENSORS_ROTATION_VECTOR_HANDLE,list,len);
    int GyroIdx = findIndexByHandle(SENSORS_GYROSCOPE_HANDLE,list,len);
    int AccelerometerIdx = findIndexByHandle(SENSORS_ACCELERATION_HANDLE,list,len);
    int MagneticFieldIdx = findIndexByHandle(SENSORS_MAGNETIC_FIELD_HANDLE,list,len);

    if(RotationVectorIdx < 0)
    {
         ALOGE("not found RotationVector,fillRV failed -> idx= %d",RotationVectorIdx); 
         return;
    }
    if(GyroIdx < 0)
    {
         ALOGE("not found Gyro,fillRV failed -> idx= : %d",GyroIdx); 
         return;
    }
    if(AccelerometerIdx < 0)
    {
         ALOGE("not found Accelerometer,fillRV failed -> idx= %d",AccelerometerIdx); 
         return;
    }
    if(MagneticFieldIdx < 0)
    {
         ALOGE("not found MagneticField,,fillRV failed -> idx= : %d",MagneticFieldIdx); 
         return;
    }

    /* compute power on the fly */
    list[RotationVectorIdx].power = list[GyroIdx].power +
                                 list[AccelerometerIdx].power +
                                 list[MagneticFieldIdx].power;
    list[RotationVectorIdx].resolution = .00001;
    list[RotationVectorIdx].maxRange = 1.0;
    list[RotationVectorIdx].minDelay = 5000;

    return;
}

/* fillGMRV depends on values of accel and mag in the list */
void MPLSensor::fillGMRV(struct sensor_t *list,int len)
{
    VFUNC_LOG;

    int GeomagneticRotationVectorIdx = findIndexByHandle(SENSORS_GEOMAGNETIC_ROTATION_VECTOR_HANDLE,list,len);
    int AccelerometerIdx = findIndexByHandle(SENSORS_ACCELERATION_HANDLE,list,len);
    int MagneticFieldIdx = findIndexByHandle(SENSORS_MAGNETIC_FIELD_HANDLE,list,len);

    if(GeomagneticRotationVectorIdx < 0)
    {
         ALOGE("not found GeomagneticRotationVector,fillGMRV failed -> idx= %d",GeomagneticRotationVectorIdx); 
         return;
    }
    
    if(AccelerometerIdx < 0)
    {
         ALOGE("not found Accelerometer,fillGMRV failed -> idx= %d",AccelerometerIdx); 
         return;
    }
    if(MagneticFieldIdx < 0)
    {
         ALOGE("not found MagneticField,,fillGMRV failed -> idx= : %d",MagneticFieldIdx); 
         return;
    }

    
    /* compute power on the fly */
    list[GeomagneticRotationVectorIdx].power = list[AccelerometerIdx].power +
                                 list[MagneticFieldIdx].power;
    list[GeomagneticRotationVectorIdx].resolution = .00001;
    list[GeomagneticRotationVectorIdx].maxRange = 1.0;
    list[GeomagneticRotationVectorIdx].minDelay = 5000;

    return;
}

/* fillGRV depends on values of gyro and accel in the list */
void MPLSensor::fillGRV(struct sensor_t *list,int len)
{
    VFUNC_LOG;

    int GameRotationVectorIdx = findIndexByHandle(SENSORS_GAME_ROTATION_VECTOR_HANDLE,list,len);
    int AccelerometerIdx = findIndexByHandle(SENSORS_ACCELERATION_HANDLE,list,len);
    int GyroIdx = findIndexByHandle(SENSORS_GYROSCOPE_HANDLE,list,len);

    if(GameRotationVectorIdx < 0)
    {
         ALOGE("not found GameRotationVector,fillGRV failed -> idx= %d",GameRotationVectorIdx); 
         return;
    }
    
    if(AccelerometerIdx < 0)
    {
         ALOGE("not found Accelerometer,fillGRV failed -> idx= %d",AccelerometerIdx); 
         return;
    }
    if(GyroIdx < 0)
    {
         ALOGE("not found Gyro,,fillGRV failed -> idx= : %d",GyroIdx); 
         return;
    }

    /* compute power on the fly */
    list[GameRotationVectorIdx].power = list[GyroIdx].power +
                                 list[AccelerometerIdx].power;
    list[GameRotationVectorIdx].resolution = .00001;
    list[GameRotationVectorIdx].maxRange = 1.0;
    list[GameRotationVectorIdx].minDelay = 5000;

    return;
}

void MPLSensor::fillOrientation(struct sensor_t *list,int len)
{
    VFUNC_LOG;

    int OrientationIdx = findIndexByHandle(SENSORS_ORIENTATION_HANDLE,list,len);
    int GyroIdx = findIndexByHandle(SENSORS_GYROSCOPE_HANDLE,list,len);
    int AccelerometerIdx = findIndexByHandle(SENSORS_ACCELERATION_HANDLE,list,len);
    int MagneticFieldIdx = findIndexByHandle(SENSORS_MAGNETIC_FIELD_HANDLE,list,len);
    ...
    if(OrientationIdx < 0)
    {
         ALOGE("not found Orientation,fillOrientation failed -> idx= : %d",OrientationIdx); 
         return;
    }
    if(GyroIdx < 0)
    {
         ALOGE("not found Gyro,fillOrientation failed -> idx= : %d",GyroIdx); 
         return;
    }
    if(AccelerometerIdx < 0)
    {
         ALOGE("not found Accelerometer,fillOrientation failed -> idx= %d",AccelerometerIdx); 
         return;
    }
    if(MagneticFieldIdx < 0)
    {
         ALOGE("not found MagneticField,,fillOrientation failed -> idx= : %d",MagneticFieldIdx); 
         return;
    }

    list[OrientationIdx].power = list[GyroIdx].power +
                              list[AccelerometerIdx].power +
                              list[MagneticFieldIdx].power;
    list[OrientationIdx].resolution = .00001;
    list[OrientationIdx].maxRange = 360.0;
    list[OrientationIdx].minDelay = 5000;

    return;
}

void MPLSensor::fillGravity( struct sensor_t *list,int len)
{
    VFUNC_LOG;

    int GravityIdx = findIndexByHandle(SENSORS_GRAVITY_HANDLE,list,len);
    int GyroIdx = findIndexByHandle(SENSORS_GYROSCOPE_HANDLE,list,len);
    int AccelerometerIdx = findIndexByHandle(SENSORS_ACCELERATION_HANDLE,list,len);
    int MagneticFieldIdx = findIndexByHandle(SENSORS_MAGNETIC_FIELD_HANDLE,list,len);
    ...
    if(GravityIdx < 0)
    {
         ALOGE("not found Gravity,fillGravity failed -> idx= : %d",GravityIdx); 
         return;
    }
    if(GyroIdx < 0)
    {
         ALOGE("not found Gyro,fillGravity failed -> idx= : %d",GyroIdx); 
         return;
    }
    if(AccelerometerIdx < 0)
    {
         ALOGE("not found Accelerometer,fillGravity failed -> idx= %d",AccelerometerIdx); 
         return;
    }
    if(MagneticFieldIdx < 0)
    {
         ALOGE("not found MagneticField,,fillGravity failed -> idx= : %d",MagneticFieldIdx); 
         return;
    }

    list[GravityIdx].power = list[GyroIdx].power +
                          list[AccelerometerIdx].power +
                          list[MagneticFieldIdx].power;
    list[GravityIdx].resolution = .00001;
    list[GravityIdx].maxRange = 9.81;
    list[GravityIdx].minDelay = 5000;

    return;
}

void MPLSensor::fillLinearAccel(struct sensor_t *list,int len)
{
    VFUNC_LOG;

    int LinearAccelIdx = findIndexByHandle(SENSORS_LINEAR_ACCEL_HANDLE,list,len);
    int GyroIdx = findIndexByHandle(SENSORS_GYROSCOPE_HANDLE,list,len);
    int AccelerometerIdx = findIndexByHandle(SENSORS_ACCELERATION_HANDLE,list,len);
    int MagneticFieldIdx = findIndexByHandle(SENSORS_MAGNETIC_FIELD_HANDLE,list,len);
    ...
    if(LinearAccelIdx < 0)
    {
         ALOGE("not found LinearAccel,fillLinearAccel failed -> idx= : %d",LinearAccelIdx); 
         return;
    }
    if(GyroIdx < 0)
    {
         ALOGE("not found Gyro,fillLinearAccel failed -> idx= : %d",GyroIdx); 
         return;
    }
    if(AccelerometerIdx < 0)
    {
         ALOGE("not found Accelerometer,fillLinearAccel failed -> idx= %d",AccelerometerIdx); 
         return;
    }
    if(MagneticFieldIdx < 0)
    {
         ALOGE("not found MagneticField,,fillLinearAccel failed -> idx= : %d",MagneticFieldIdx); 
         return;
    }

    list[LinearAccelIdx].power = list[GyroIdx].power +
                          list[AccelerometerIdx].power +
                          list[MagneticFieldIdx].power;
    list[LinearAccelIdx].resolution = list[AccelerometerIdx].resolution;
    list[LinearAccelIdx].maxRange = list[AccelerometerIdx].maxRange;
    list[LinearAccelIdx].minDelay = 5000;

    return;
}

void MPLSensor::fillSignificantMotion(struct sensor_t *list,int len)
{
    VFUNC_LOG;

    int SignificantMotionIdx = findIndexByHandle(SENSORS_SIGNIFICANT_MOTION_HANDLE,list,len);
    int AccelerometerIdx = findIndexByHandle(SENSORS_ACCELERATION_HANDLE,list,len);
    


    if(SignificantMotionIdx < 0)
    {
         ALOGE("not found SignificantMotion,fillSignificantMotion failed -> idx= : %d",SignificantMotionIdx); 
         return;
    }
    
    if(AccelerometerIdx < 0)
    {
         ALOGE("not found Accelerometer,fillSignificantMotion failed -> idx= %d",AccelerometerIdx); 
         return;
    }
    


    list[SignificantMotionIdx].power = list[AccelerometerIdx].power;
    list[SignificantMotionIdx].resolution = 1;
    list[SignificantMotionIdx].maxRange = 1;
    list[SignificantMotionIdx].minDelay = -1;
}

#ifdef ENABLE_DMP_SCREEN_AUTO_ROTATION
void MPLSensor::fillScreenOrientation(struct sensor_t *list,int len)
{
    VFUNC_LOG;

    int ScreenOrientationIdx = findIndexByHandle(SENSORS_SCREEN_ORIENTATION_HANDLE,list,len);
    int AccelerometerIdx = findIndexByHandle(SENSORS_ACCELERATION_HANDLE,list,len);

    if(ScreenOrientationIdx < 0)
    {
         ALOGE("not found ScreenOrientation,fillScreenOrientation failed -> idx= : %d",ScreenOrientationIdx); 
         return;
    }
    
    if(AccelerometerIdx < 0)
    {
         ALOGE("not found Accelerometer,fillScreenOrientation failed -> idx= %d",AccelerometerIdx); 
         return;
    }

    list[ScreenOrientationIdx].power = list[AccelerometerIdx].power;
    list[ScreenOrientationIdx].resolution = 1;
    list[ScreenOrientationIdx].maxRange = 3;
    list[ScreenOrientationIdx].minDelay = 0;
}
#endif
/*
void MPLSensor::fillLight(struct sensor_t *list)
{
    VFUNC_LOG;

    list[LightSen].power = list[LightSensor].power;
    list[LightSen].resolution = list[LightSensor].resolution;
    list[LightSen].maxRange = list[LightSensor].maxRange;
    list[LightSen].minDelay = list[LightSensor].minDelay;
