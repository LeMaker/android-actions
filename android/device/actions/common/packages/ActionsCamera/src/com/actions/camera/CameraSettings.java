...
public class CameraSettings {
    ...
    public static final String KEY_CAMERA_HFLIP = "pref_camera_hflip_key";
    public static final String KEY_CAMERA_VFLIP = "pref_camera_vflip_key";    
    ...
    private void initPreference(PreferenceGroup group) {
        ...
        IconListPreference cameraHflipPref =
                (IconListPreference) group.findPreference(KEY_CAMERA_HFLIP);
        IconListPreference cameraVflipPref =
                (IconListPreference) group.findPreference(KEY_CAMERA_VFLIP);                                
        ...
        if (cameraHflipPref != null) buildCameraFlip(group, cameraHflipPref,CameraUtil.HFLIP_MODE);
        if (cameraVflipPref != null) buildCameraFlip(group, cameraVflipPref,CameraUtil.VFLIP_MODE);
    ...
    private void buildCameraFlip(
            PreferenceGroup group, IconListPreference preference,int mode) {
				int flipmode = 2;		//0: hflip   1:vflip
        int mFlip;
        CharSequence[] entryValues = new CharSequence[flipmode];
        for (int i = 0; i < flipmode; ++i) {
            entryValues[i] = "" + i;
        }
        preference.setEntryValues(entryValues);
        
        mFlip=preference.findIndexOfValue(preference.getValue());
        String strMode;
        if(mode==CameraUtil.HFLIP_MODE){
        		if(mCameraId==0){
        			  strMode=CameraUtil.BACK_CAMERA_HFLIP;
        		}
        		else{		
        			  strMode=CameraUtil.FRONT_CAMERA_HFLIP; 
						}
				}
				else{
        		if(mCameraId==0){
        			  strMode=CameraUtil.BACK_CAMERA_VFLIP;
        		}
        		else{		
        			  strMode=CameraUtil.FRONT_CAMERA_VFLIP;        				
						}					
				}
				int mval=CameraUtil.systemproperties_getInt(strMode,0);					
				if(mFlip!=mval){
						preference.setValueIndex(mval);	
				}
