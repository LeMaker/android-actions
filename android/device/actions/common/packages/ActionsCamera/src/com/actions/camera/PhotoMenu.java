...
import com.actions.camera.ui.AbstractSettingPopup;
import com.actions.camera.ui.CountdownTimerPopup;
import com.actions.camera.ui.ListPrefSettingPopup;
import com.actions.camera.ui.PieItem;
import com.actions.camera.ui.PieItem.OnClickListener;
import com.actions.camera.ui.PieRenderer;
...
public class PhotoMenu extends PieController
    ...
    public void initialize(PreferenceGroup group) {
        ...
        //HFlip switch.
        if (group.findPreference(CameraSettings.KEY_CAMERA_HFLIP) != null) {
            item = makeSwitchItem(CameraSettings.KEY_CAMERA_HFLIP, false);
            final PieItem fitem = item;
            item.setOnClickListener(new OnClickListener() {
                @Override
                public void onClick(PieItem item) {
                    // change the hflip.
                    ListPreference pref = mPreferenceGroup
                            .findPreference(CameraSettings.KEY_CAMERA_HFLIP);
                    if (pref != null) {
                        int index = pref.findIndexOfValue(pref.getValue());
                        CharSequence[] values = pref.getEntryValues();
                        index = (index + 1) % values.length;
                        pref.setValueIndex(index);
                        mListener.onCameraHflipClicked(index);
                    }
                    updateItem(fitem, CameraSettings.KEY_CAMERA_HFLIP);
                }
            });
            mRenderer.addItem(item);
        }
        ...
        //VFlip switch.
        if (group.findPreference(CameraSettings.KEY_CAMERA_VFLIP) != null) {
            item = makeSwitchItem(CameraSettings.KEY_CAMERA_VFLIP, false);
            final PieItem fitem = item;
            item.setOnClickListener(new OnClickListener() {
                @Override
                public void onClick(PieItem item) {
                    // change the vflip.
                    ListPreference pref = mPreferenceGroup
                            .findPreference(CameraSettings.KEY_CAMERA_VFLIP);
                    if (pref != null) {
                        int index = pref.findIndexOfValue(pref.getValue());
                        CharSequence[] values = pref.getEntryValues();
                        index = (index + 1) % values.length;
                        pref.setValueIndex(index);
                        mListener.onCameraVflipClicked(index);
                    }
                    updateItem(fitem, CameraSettings.KEY_CAMERA_VFLIP);
                }
            });
            mRenderer.addItem(item);
        }
