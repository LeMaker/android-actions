...
static int g_max_size = 0x200000; //default to 2M Bytes
static int g_write_size = 0;

static char inputfile[512];
bool useService = true;
bool loopRecord = true;
...
const char KERNEL_TAG[]={'K','e','r','n','e','l','\0'};
static int g_outkernelFD = -1;
...
static int open_kernel_logfile(const char *output_file)
{
	const char *p;
	char *s, *s1, *k;
	char kernel_file[255]={0};
	int l,len;
	char tag[]={'_','k','e','r','n','e','l'}; 

	fprintf(stderr, "open_kernel_logfile\n");
	
	if(!output_file)
	return -1;

	len = sizeof(kernel_file);
	if((sizeof(output_file)+sizeof(tag)-1)>sizeof(kernel_file))
	return -1;
	...
	return open(kernel_file, O_WRONLY | O_APPEND | O_CREAT, S_IRUSR | S_IWUSR);
...
static void rotateLogs()
{
    ...
    g_outkernelFD = open_kernel_logfile(g_outputFileName);
	if (g_outkernelFD < 0) {
        exit(-1);
    }
...
static void processBuffer(log_device_t* dev, struct log_msg *buf)
{
    ...
    if (android_log_shouldPrintLine(g_logformat, entry.tag, entry.priority)) {
		...
		g_write_size+=bytesWritten;
	
		if(g_write_size>=g_max_size)
		{
			close(g_outFD);
			close(g_outkernelFD);
			exit(-1);
...
static void show_help(const char *cmd)
{
    ...
    fprintf(stderr, "options include:\n"
			...
			"  -m <count>      set max size for log file\n"
...
static void readLogLines(void)  
{
	...
	while (1) 
	{
		ret = klogctl(9, buffer, sizeof(buffer));	
		if(ret > 0) {
			ret = klogctl(2, buffer, sizeof(buffer));
			if(ret > 0) {
				entry.tid = 0;
				entry.pid = getpid();
				entry.message = buffer;
				entry.messageLen =ret;
				entry.tag = KERNEL_TAG;
				bytesWritten = android_log_printLogLine(g_logformat, g_outkernelFD, &entry);
...
useService = true;
	...
	fprintf(stderr,"useService:%d\n",useService);

	if(useService)
	{
		inputfile[0]=0;
		property_get("sys.actionlogcat.fullfilename",inputfile, NULL);
		android::g_outputFileName = inputfile;
		fprintf(stderr,"buf1:%s %s\n",inputfile, android::g_outputFileName);
		buf[0]=0;
		property_get("sys.actionlogcat.timeformat",buf, NULL);
		fprintf(stderr,"t2:%s\n",buf);
		if(buf[0]!=0)
		{
			err = setLogFormat (buf);
			if (err < 0) {
				fprintf(stderr,"Invalid timeformat prop\n");
				exit(-1);
			}
			hasSetLogFormat = 1;
		}
		buf[0]=0;
		property_get("sys.actionlogcat.maxsize",buf, NULL);
		fprintf(stderr,"t3:%s\n",buf);
		if(buf[0]!=0)
		{
			if (!isdigit(buf[0])) {
				fprintf(stderr,"Invalid maxsize prop\n",buf);
				exit(-1);
			}
			g_max_size = atoi(buf);
			g_max_size <<=20; //convert from Mbytes to bytes
			fprintf(stderr,"set max size=%d bytes\n",g_max_size);
		}
	}
	else
	{
    ...
    for (;;) {
        ...
        ret = getopt(argc, argv, "cdt:T:gG:sQf:r:n:v:m:b:BSpP:");
        ...
        switch(ret) {
			...
			case 'm':
				if (!isdigit(optarg[0])) {
					fprintf(stderr,"Invalid parameter to -m %s\n",optarg[0]);
					android::show_help(argv[0]);
					exit(-1);
				}
				g_max_size = atoi(optarg);
				g_max_size <<=20; //convert from Mbytes to bytes
				break;
