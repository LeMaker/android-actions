...
enum {
    ...
    /* Actions camera usage flag */
    GRALLOC_USAGE_HW_CAMERA             = 0x00200000,

    /* Actions video usage flag */
    GRALLOC_USAGE_HW_VIDEO              = 0x00100000,
    GRALLOC_USAGE_HW_VIDEO_PP           = 0x00400000,
    GRALLOC_USAGE_HW_VIDEO_H265         = 0x00800000,
    GRALLOC_USAGE_HW_VIDEO_MASK         = 0x00D00000,
};

/*
* buffer_info_t records additional infos of buffer allocated from gralloc module. 
* Note that the size of buffer_info_t's each member must be integer multiple of int.
*/
enum{ BUFFER_INFO_PHY_CONTIG = 0x1,};
...
typedef struct gralloc_module_t {
	...
	/* the  (*getPhysAddr) to get physical address of a  buffer
	*/
	 int (*getPhysAddr)
	 	(struct gralloc_module_t const* module, buffer_handle_t handle,
					    void **paddr);
    /*the (*stretchBlit) copy the source buffer to dest,both source and dest buffer need allocate from gralloc
    */
    int (*stretchBlit)(struct gralloc_module_t const* module,
							  buffer_handle_t src, buffer_handle_t dest,
							  android_native_rect_t* psSrcRect, 
							  android_native_rect_t* psDestRect,
							  int eRotation);
	  /*the (*stretchBlit) copy the source buffer to dest
	  source buffer need allocate from gralloc, dest not need
    */
    int (*stretchBlit2)(struct gralloc_module_t const* module,
								   buffer_handle_t src, 
								   void *dest, int format, int w, int h,
								   android_native_rect_t *psSrcRect, 
								   android_native_rect_t *psDestRect,
								   int eRotation);
	/*the (*getBufferInfo) get buffer info like width,height,format,size
	*/
	int (*getBufferInfo)(struct gralloc_module_t const* module,	buffer_handle_t handle, 
					   int *width, int *height, int *format, int *size, int *usage);
    /*the (*getExtraBufferInfo) get extra buffer info
    */
    int (*getExtraBufferInfo)(struct gralloc_module_t const* module, buffer_handle_t handle, 
                         buffer_info_t* info);
