...
//ActionsCode(fanguoyong,  change_code)
...
public class MediaPlaybackService extends Service {
    ...
    //ActionsCode(fanguoyong,  add some variables)
    private String currentFilePath;
		private String mFileId;
		private int mFileTtmpPos = -1;
    ...
    public void onCreate() {
		...
		//ActionsCode(author:fanguoyong,sync from 4.4.4,disable remotecontrol)
 /*     
...
*/       
		...
		reloadQueue();
        //ActionsCode(author:fanguoyong,bugfix:BUG00234895)
        hasReload = true;
		...
		//ActionsCode(author:fanguoyong,sync from 4.4.4)
		IntentFilter intentFilter7 = new IntentFilter(Intent.ACTION_SHUTDOWN);
		registerReceiver(shundownReceiver, intentFilter7);
     ...
     */
		...
		/**
		  *BUGFIX :BUG00205909 
		  *do not stop playing music in flash when pull out the external sdcard
		  ************************************
		  *      
            ...
            mUnmountReceiver = new BroadcastReceiver() {
                ...
                public void onReceive(Context context, Intent intent) {
                    ...
                    if (action.equals(Intent.ACTION_MEDIA_EJECT)) {
                        ...
                        StorageVolume volume = (StorageVolume) intent
													.getExtras().get("storage_volume");
												if ( currentFilePath != null && currentFilePath.contains(volume.getPath()) ) {
                    			saveQueue(true);
                    			mQueueIsSaveable = false;
                    			//ActionsCode(author:fanguoyong,bugfix:BUG00234895)
													hasReload = false;
                        ...
                        hasReload = true;
										...
										if (action.equals(Intent.ACTION_MEDIA_SCANNER_FINISHED)) {
											Log.e(LOGTAG, "dcx ----ACTION_MEDIA_SCANNER_FINISHED");
											updatePlayList();
					            notifyChange(QUEUE_CHANGED);
					            notifyChange(META_CHANGED);
					          }
	...
	void updatePlayList() {
		...
		mCursor = getContentResolver().query(
				...
				null, mSortOrder);
		if (mCursor != null) {
			len = mCursor.getCount();
		}
		Log.v(LOGTAG, "len is " + len + ",mPlayList:" + mPlayList
				+ ",mPlayPos:" + mPlayPos);
		if (mCursor == null || len <= 0) {
			stop(true);
			mPlayList = null;
			mPlayListLen = 0;
			if (mCursor != null) {
		...
		try {
			...
			colidx = mCursor.getColumnIndexOrThrow(MediaStore.Audio.Media._ID);
		}
		for (int i = 0; i < len; i++) {
			list[i] = mCursor.getLong(colidx);
			mCursor.moveToNext();
		}
		if (mCursor != null) {
			mCursor.close();
		...
		if (debug) {
			...
			for (int i = 0; i < mPlayListLen; i++) {
				Log.d(LOGTAG, "i :" + i + " , " + mPlayList[i]);
			}
			for (int i = 0; i < len; i++) {
				mCursor = getContentResolver().query(
						MediaStore.Audio.Media.EXTERNAL_CONTENT_URI,
						mCursorCols, "_id=" + String.valueOf(list[i]), null,
						null);
				if (mCursor == null && mCursor.getCount() == 0) {
					continue;
				}
				mCursor.moveToFirst();
				trackName = mCursor.getString(mCursor
						.getColumnIndexOrThrow(MediaStore.Audio.Media.TITLE));
				Log.d(LOGTAG, "i :" + i + " , " + list[i] + " , " + trackName);
				if (mCursor != null) {
					mCursor.close();
					mCursor = null;
				}
		...
		if (mPlayList != null && mPlayPos != -1) {
			...
			if (mCursor != null && mCursor.getCount() != 0) {
				Log.v(LOGTAG, "current song  exist " + mCursor.getCount());
				Log.v(LOGTAG, "len : " + len + " ,mPlayListLen : "
						+ mPlayListLen);
				for (int i = 0; i < len; i++) {
					if (list[i] == mPlayList[mPlayPos]) {
						newPos = i;
						break;
					}
				}
			...
			/*
			 *BUGFIX:BUG00234877
			 *do not manually stop current in case the current song was renamed
			 ***************************
			 *ActionsCode(autho:fanguoyong,change_code)
			*/
			else{
				newPos = mPlayPos;
			}
			/*
				...
				Log.v(LOGTAG, "no current song " + mCursor.getCount());
				Log.v(LOGTAG, "len : " + len + " ,mPlayListLen : "
						+ mPlayListLen);
				if (isPlaying()) {
				...
				//stop(true);
				for (int j = mPlayPos; j < mPlayListLen; j++) {
					for (int i = 0; i < len; i++) {
						if (list[i] == mPlayList[j]) {
							newPos = i;
							break;
						}
					}
		...
		/*
		 * if(list.length == 0||mPlayList==null){ Log.v(LOGTAG,"list.length:"
		 * +list.length+" , mPlayList:"+mPlayList); return; }
		 */
		addToPlayList(list, -1);
		mPlayPos = newPos;
		Log.d(LOGTAG, "mPlayPos:" + mPlayPos);
		if (currentSongRemoved) {
			//mPlayPos = (--mPlayPos <= -1) ? 0 : mPlayPos;
			//Log.d(LOGTAG, "select song postion " + mPlayPos);
			//open(list, mPlayPos);
			//play();
 ...
 */     
    ...
    private void openCurrentAndNext() {
        synchronized (this) {
			...
			//ActionsCode(author:fanguoyong,sync from 4.4.4)
			currentFilePath = mCursor.getString(mCursor
    ...
    private void setNextTrack() {
		...
		//ActionsCode(author:fanguoyong,sync from 4.4.4)
		mFileTtmpPos = mNextPlayPos;
		Log.v(LOGTAG, "mNextPlayPos:" + mNextPlayPos);
		Cursor mTmpCursor = null;
		if (mFileTtmpPos >= 0 && mFileTtmpPos < mPlayList.length) {
			String id = String.valueOf(mPlayList[mFileTtmpPos]);
			mTmpCursor = getContentResolver().query(
					MediaStore.Audio.Media.EXTERNAL_CONTENT_URI, mCursorCols,
					"_id=" + id, null, null);
			if (mTmpCursor != null) {
				mTmpCursor.moveToFirst();
				if (mTmpCursor.getCount() > 0) {
					mFileId = mTmpCursor.getString(0);
					Log.v(id, "mFileId:" + mFileId);
				}
			}
		}
		if (mTmpCursor != null) {
    ...
    public void play() {
        ...
        if (mPlayer.isInitialized()) {
			...
			//ActionsCode(author:fanguoyong,sync from 4.4.4)
			/*
			 * 2013-03-14: fix:play the last file again.it show play the first
			 * file.
			 */
			/*
			 * if (mRepeatMode != REPEAT_CURRENT && duration > 2000 &&
			 * mPlayer.position() >= duration - 2000) { gotoNext(true); }
			 */
	...
	private void gotoTmpNext() {
		synchronized (this) {
			...
			// Log.v(LOGTAG, "next position :"+pos);
			// Log.v(LOGTAG, " mPlayPos :"+mPlayPos);
			mPlayPos = pos;
			saveBookmarkIfNeeded();
			stop(false);
			openCurrentAndNext();
    ...
    private class MultiPlayer {
        ...
        public void setNextDataSource(String path) {
			...
			//ActionsCode(author:fanguoyong,sync from 4.4.4)
            //if (path == null) {
			if (path == null || mNextPlayPos < mPlayList.length) {
                return;
            }
        ...
        public void stop() {
            ...
            //ActionsCode(author:fanguoyong,stop)
			if (mNextMediaPlayer != null) {
            ...
            public void onCompletion(MediaPlayer mp) {
                if (mp == mCurrentMediaPlayer && mNextMediaPlayer != null) {
					...
					if (mNextPlayPos >= 0 && mNextPlayPos < mPlayList.length) {
						...
						// Log.v(LOGTAG, "id:"+id);
						// Log.v(LOGTAG, "mFileId:"+mFileId);
						if (!mFileId.equals(id)) {
							// Log.e(LOGTAG, "next play song is delete .");
        ...
        MediaPlayer.OnErrorListener errorListener = new MediaPlayer.OnErrorListener() {
            public boolean onError(MediaPlayer mp, int what, int extra) {
                ...
                default:
					...
					//ActionsCode(author:fanguoyong,BUGFIX:BUG00243436):stop playing when MediaPlayer report error in REPEAT_CURRENT mode.
					if (mRepeatMode == REPEAT_CURRENT) {
						stop();
						if (mCursor != null) {
	                    	mCursor.close();
