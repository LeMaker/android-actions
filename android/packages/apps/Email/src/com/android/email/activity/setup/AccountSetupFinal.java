...
public class AccountSetupFinal extends AccountSetupActivity
    ...
    */
    private void populateHostAuthsFromSetupData() {
        ...
        address = domain;
        //ActionsCode(liuxinxu, BUGFIX: BUG00240062: recreate receive server address.)
        if(mSetupData.getIncomingProtocol().equals(HostAuth.SCHEME_POP3) || 
        		mSetupData.getIncomingProtocol().equals(HostAuth.SCHEME_IMAP)) {
        	address = AccountSettingsUtils.inferServerName(this, domain,
        			mSetupData.getIncomingProtocol(), null);
        }
        
        Log.e(TAG, "recreate incoming address " + address);
        
        recvAuth.setConnection(mSetupData.getIncomingProtocol(), address,
        ...
        if (info.usesSmtp) {
            ...
            //ActionsCode(liuxinxu, BUGFIX: BUG00240062: recreate send server address.)
            address = AccountSettingsUtils.inferServerName(this, domain,
        			null, HostAuth.LEGACY_SCHEME_SMTP);
            Log.e(TAG, "recreate sending address " + address);
            sendAuth.setConnection(HostAuth.LEGACY_SCHEME_SMTP, address,
