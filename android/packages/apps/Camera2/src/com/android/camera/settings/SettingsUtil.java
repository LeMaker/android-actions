...
public class SettingsUtil {
     ...
     */
    /**
    * BUGFIX: fix for app params sync with hotplug uvc modules .
    *ActionsCode(author:liyuan, change_code)
    */
    public static Size getPhotoSize(String sizeSetting, List<Size> supported, int cameraId) {
        return getPhotoSize(sizeSetting, supported, cameraId, false);
    } 
    public static Size getPhotoSize(String sizeSetting, List<Size> supported, int cameraId, boolean refresh) {
        if (ResolutionUtil.NEXUS_5_LARGE_16_BY_9.equals(sizeSetting)) {
            return ResolutionUtil.NEXUS_5_LARGE_16_BY_9_SIZE;
        }
        Size selectedSize = getCameraPictureSize(sizeSetting, supported, cameraId, refresh);
        return selectedSize;
    }
    ...
    */
    private static Size getCameraPictureSize(String sizeSetting, List<Size> supported,
            int cameraId) {
        return getCameraPictureSize(sizeSetting, supported, cameraId, false);
    }
    private static Size getCameraPictureSize(String sizeSetting, List<Size> supported,
            int cameraId, boolean refresh) {
        return getSelectedCameraPictureSizes(supported, cameraId, refresh).getFromSetting(sizeSetting,
                supported);
    }
    ...
    */
	...
	return getSelectedCameraPictureSizes(supported, cameraId, false);
    }
    static SelectedPictureSizes getSelectedCameraPictureSizes(List<Size> supported, int cameraId, boolean refresh) {
        List<Size> supportedCopy = new LinkedList<Size>(supported);
        if (sCachedSelectedPictureSizes.get(cameraId) != null && !refresh) {
            return sCachedSelectedPictureSizes.get(cameraId);
        }else if(refresh){
            sCachedSelectedPictureSizes.delete(cameraId);
        }
    ...
    */
    static SelectedVideoQualities getSelectedVideoQualities(int cameraId) {
		return getSelectedVideoQualities(cameraId, false);
	}
    static SelectedVideoQualities getSelectedVideoQualities(int cameraId, boolean refresh) {
        if (sCachedSelectedVideoQualities.get(cameraId) != null && !refresh) {
