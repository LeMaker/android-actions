...
public class VideoModule extends CameraModule
    ...
    private class MyBroadcastReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(Intent.ACTION_MEDIA_EJECT)) {
				// ActionsCode(liyuan, change_code): fix record ends while pull out sd-card
				Uri uri = intent.getData();
                if(uri != null){
                    String storagePath = Storage.DIRECTORY;
                    int lastIndex = storagePath.indexOf("DCIM");
                    storagePath = storagePath.substring(0, lastIndex-1);
                    if(uri.toString().startsWith(("file://"+storagePath))) { 
                        stopVideoRecording();
                    }
                }else{
                    stopVideoRecording();
    ...
    private static Point getDesiredPreviewSize(Context context, CameraSettings settings,
        ...
        List<Size> sizes = capabilities.getSupportedPreviewSizes();
  		...
  		//ActionsCode(author:liyuan, change_code, BUGFIX:BUG00252358)
		if(previewScreenShortSide>0)
		{
			...
			Log.w(TAG, "for view unMeasured, do not check preferred");
		}
    ...
    private void startVideoRecording() {
		...
		//ActionsCode(liyuan, change_code),avoid click fluent confusion for recond click
        mAppController.getCameraAppUI().showModeOptions(false);
	...
	*/
	mActivity.setStorageSpaceListener(new CameraActivity.OnStorageUpdateDoneListener() {
	    @Override
	    public void onStorageUpdateDone(long bytes) {
	        if (bytes <= Storage.LOW_STORAGE_THRESHOLD_BYTES && mMediaRecorderRecording ==true) {
	            Log.w(TAG, "Storage is not enough..stop record");
	            stopVideoRecording();
	        } 
	    }
	});
    ...
    private boolean stopVideoRecording() {
        ...
        if (mSnapshotInProgress) {
			//ActionsCode(liyuan, change_code, bugfix:BUG00258134)
			if(mPaused)
			{
				mSnapshotInProgress = false;
            	showVideoSnapshotUI(false);
				Log.v(TAG, "snapshot canceled since pause accur!");
	...
	*ActionsCode(author:liyuan, change_code)
	*/
	mActivity.setStorageSpaceListener(null);
    ...
    private final class JpegPictureCallback implements CameraPictureCallback {
        ...
        public void onPictureTaken(byte [] jpegData, CameraProxy camera) {
			...
			if(!mPaused)
			{
				...
				Log.w(TAG, "pause accured,Video snapshot canceled.");
			}
