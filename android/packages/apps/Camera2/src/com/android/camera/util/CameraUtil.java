...
public class CameraUtil {
    ...
    /**
     *
     * Fix for the calculation of best preview resolution, choose the perfect size which nearest and bigger then the targetScreen.
     * ***********************************
     *
    ...
    public static int getOptimalPreviewSizeIndex(Context context,
        ...
        for (int i = 0; i < sizes.size(); i++) {
            ...
            if (heightDiff < minDiff) {
                //ActionsCode(liuyiguang, get the optimalPreview size, choose the perfect size which nearest and bigger then the targetScreen.)
                if (optimalSizeIndex == -1 || size.getHeight() >= targetHeight) {
                    optimalSizeIndex = i;
                    minDiff = heightDiff;
                }
