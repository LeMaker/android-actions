...
public class Memory extends SettingsPreferenceFragment implements Indexable {
    ...
    private static String mUhostPath;
    ...
    public void onCreate(Bundle icicle) {
        ...
        //ActionsCode(phchen, new feature uhost), 
        mUhostPath = Environment.getUhostStorageDirectory().getPath();
        for (StorageVolume volume : storageVolumes) {
            if ((!volume.isEmulated()) 
            && (!((volume.getPath().startsWith(mUhostPath)) 
            && (!volume.getState().equals("mounted"))))) {
...
*/
    ...
    StorageEventListener mStorageListener = new StorageEventListener() {
        ...
        public void onStorageStateChanged(String path, String oldState, String newState) {
            ...
            //ActionsCode(phchen, new feature uhost),
            if(path != null && path.startsWith(mUhostPath)){
                if (newState.equals("unmounted")) {
                ...
                }else if(newState.equals("mounted")){
                    Log.d(TAG, "ACTION_MEDIA_MOUNTED" );
                    final StorageVolume[] storageVolumes = mStorageManager.getVolumeList();
                    for (StorageVolume volume : storageVolumes) {
                        Log.d(TAG, "volume=" + volume);
                        if ((!volume.isEmulated()) && path.equals(volume.getPath())) {
                    ...
                    for (StorageVolumePreferenceCategory category : mCategories) {
                        category.onResume();
                        Log.d(TAG, "mounted category=" + category );	
                    }
                }
            }
