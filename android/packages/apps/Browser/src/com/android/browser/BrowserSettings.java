...
public class BrowserSettings implements OnSharedPreferenceChangeListener,
    ...
    //ActionsCode(author:hexibin, set default search engin from property)
    private String mDefaultSearchEngin = "";
    ...
    /**
     **********************************************
     * ActionsCode(author:hexibin, new_method)
     * ActionsCode(author:hexibin, set default search engin from property)
     */
    public void checkDefaultSearchName() {
    	String currentName = getSearchEngineName();
    	if(!mPrefs.contains(PREF_SEARCH_ENGINE)){
    		mPrefs.edit().putString(PREF_SEARCH_ENGINE, currentName).apply();
    	}
        ...
        for (int i = 0; i < searchEngines.length; i++) {
            if(searchEngines[i].equals(currentName)){
            	found = true;
            }
        }
        
        if(!found){
            mPrefs.edit().putString(PREF_SEARCH_ENGINE, searchEngines[0]).apply();
        }
    }
    ...
    private void syncStaticSettings(WebSettings settings) {
        ...
        //ActionsCode(hexibin, bugfix BUG00236220)
    ...
    public String getSearchEngineName() {
    	//ActionsCode(author:hexibin, set default search engin from property)
    	if(mDefaultSearchEngin.equals("")){
    		mDefaultSearchEngin = SystemProperties.get("ro.browser.search_engin", SearchEngine.GOOGLE);
    	}
    	
        return mPrefs.getString(PREF_SEARCH_ENGINE, mDefaultSearchEngin);
    }
