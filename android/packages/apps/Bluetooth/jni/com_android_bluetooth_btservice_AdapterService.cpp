...
static jint getSendByteNative(JNIEnv* env, jobject obj) {
    ALOGV("%s:",__FUNCTION__);

    //jboolean result = JNI_FALSE;
    if (!sBluetoothInterface) return -1;

    int ret = sBluetoothInterface->get_send_byte();
    
    return (jint)ret;
}

static jint getRecvByteNative(JNIEnv* env, jobject obj) {
    ALOGV("%s:",__FUNCTION__);

    //jboolean result = JNI_FALSE;
    if (!sBluetoothInterface) return -1;

    int ret = sBluetoothInterface->get_recv_byte();
