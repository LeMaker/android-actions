...
public final class Launcher extends Activity
    ...
    //ActionsCode(authro:lizihao, comment:hide Shadow)
    ...
    // it from the context.
    private static final Object mSharedPrefsLock = new Object();
    ...
    private Runnable mUpdateWallpaperRunnable = new Runnable() {
        public void run() {
            disableWallpaperIfInAllApps();
        }
    };
    
    private Runnable mUpdateWallpaperVisibilityRunnable = new Runnable() {
        public void run() {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //ActionsCode(phchen, NEW FEATURE: hold still wallpaper, wallpaper as launcher background),
        wm = (WallpaperManager)
                getSystemService(Context.WALLPAPER_SERVICE);
        
        LauncherApplication app = ((LauncherApplication)getApplication());
        synchronized(mSharedPrefsLock) {
        ...
        //ActionsCode(phchen, NEW FEATURE: hold still wallpaper, wallpaper in launcher ),
        mWallpaperChange = Settings.Global.getInt(getContentResolver(),"launcher_wallpaper_status", 0);
    ...
    private void updateGlobalIcons() {
        ...
        //ActionsCode(phchen, BUGFIX: BUG00323082 ), 
        if ((sGlobalSearchIcon[coi] != null) && SystemProperties.get("persist.sys.no_bg", "0").equals("0")) {
             updateGlobalSearchIcon(sGlobalSearchIcon[coi]);
             searchVisible = true;
        }
        //ActionsCode(phchen, BUGFIX: BUG00323082 ), 
        if ((sVoiceSearchIcon[coi] != null) && SystemProperties.get("persist.sys.no_bg", "0").equals("0")){
    ...
    protected void onActivityResult(
        ...
        if (requestCode == REQUEST_PICK_WALLPAPER) {
            if (LauncherApplication.isStillWallpaper() == true 
                && resultCode == RESULT_OK 
                && (mDragLayer.getMeasuredWidth() > 0)) {
                if (mWallpaperChange == NO_CHANGE) {
                    mWallpaperChange = UNKNOWN;
                }
            }
        }
    ...
    protected void onStart() {
        mWorkspace.removeCallbacks(mUpdateWallpaperVisibilityRunnable);
        mstart = true;
        super.onStart();
    ...
    protected void onResume() {
        ...
        //ActionsCode(phchen, NEW FEATURE: hold still wallpaper, wallpaper in launcher ),
        if ((usedStaticWallpaper() == true) 
            && ( !isAllAppsVisible()) 
            && (mFirstCreate == false)){
            mDragLayer.mHandler.removeMessages(mDragLayer.WALLPAPER_MSG);
            Message msg = mDragLayer.mHandler.obtainMessage(mDragLayer.WALLPAPER_MSG);
            mDragLayer.mHandler.sendMessageDelayed(msg, 1000);
            
            mWorkspace.removeCallbacks(mUpdateWallpaperVisibilityRunnable);
            mWorkspace.removeCallbacks(mUpdateWallpaperRunnable);
            mWorkspace.postDelayed(mUpdateWallpaperRunnable, 2000);
        }
    }
    ...
    protected void onPause() {
        ...
        mWorkspace.removeCallbacks(mUpdateWallpaperRunnable);
        mWorkspace.postDelayed(mUpdateWallpaperVisibilityRunnable, 1000);
    ...
    */
    ...
    private void setupViews() {
        ...
        mQsbDivider = null;//findViewById(R.id.qsb_divider);
    ...
    public void startGlobalSearch(String initialQuery,
        ...
        //ActionsCode(phchen, BUGFIX:BUG00240631),
...
//        }
    ...
    void disableWallpaperIfInAllApps() {
        ...
        //ActionsCode(phchen, NEW FEATURE: hold still wallpaper, wallpaper as launcher background),
        } else  if (usedStaticWallpaper() == true) {
            Log.w(TAG, "disableWallpaperIfInAllApps");
            updateWallpaperVisibility(false);
        }
    }
    ...
    void showWorkspace(boolean animated, Runnable onCompleteRunnable) {
        if (mState != State.WORKSPACE) {
            ...
            // loaded mode)
            //ActionsCode(phchen, BUGFIX: BUG00323082 ), 
            if ((mSearchDropTargetBar != null) && SystemProperties.get("persist.sys.no_bg", "0").equals("0")){
    ...
    public void bindItems(final ArrayList<ItemInfo> shortcuts, final int start, final int end) {
        ...
        //ActionsCode(phchen, BUGFIX: BUG00314125 ),
        synchronized(mSharedPrefsLock) {
    ...
    public void bindComponentsRemoved(final ArrayList<String> packageNames,
        ...
        //ActionsCode(phchen, sync gs702c, bugfix BUG00129018), 
        /*if (waitUntilResume(new Runnable() {
            public void run() {
    ...
    private boolean isClingsEnabled() {
        ...
        //ActionsCode(phchen, sync gs702c, bugfix bug00130019)
        return Utilities.mClingsEnable;
    ...
    public static final int STATIC_WALLPAPER = 1;
    public static final int LIVE_WALLPAPER = 2;
    public static final int UNKNOWN = 3;
    public static int mWallpaperChange = NO_CHANGE; // 0:no change, 1:picture wallpaper, 2:live wallpaper, 3:unknown
    public static WallpaperManager wm;
    public Bitmap mScreenBitmap = null;
    private Canvas mCanvas = null;
    private static boolean mstart = false;
    public static boolean enterAllApps = false;
    public static boolean mFirstCreate = true;
    
    public static int mStatusBarHeight = 0;
    public static int mNavigationBarHeight = 0;

    public static boolean usedStaticWallpaper() {
        //ActionsCode(phchen, NEW FEATURE: hold still wallpaper, wallpaper in launcher ),
        if (LauncherApplication.isStillWallpaper() 
            && (mWallpaperChange != LIVE_WALLPAPER) && (mstart)) {
            return true;
        }
        return false;
    }
    public boolean inWorkSapceState(){
	if(mState == State.WORKSPACE)
            return true;
	return false;
    }
