...
class AllAppsList {
    ...
    */
    public void add(ApplicationInfo info) {
	    //ActionsCode(authro:lizihao, comment:get the hide activity)
        if(Utilities.mHideActivityEnable){
		
            for (Iterator iterator = Utilities.mActivityName.iterator(); iterator
                    .hasNext();) {
                String type = (String) iterator.next();
                if(info.componentName.getClassName().equals(type)){
                    return;
                }
            }
        }
