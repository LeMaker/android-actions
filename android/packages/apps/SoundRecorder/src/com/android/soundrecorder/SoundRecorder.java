...
class RemainingTimeCalculator {
    ...
    public long timeRemaining() {
		...
		long result = (mLastBlocks*blockSize-(2<<19))/mBytesPerSecond;
...
public class SoundRecorder extends Activity 
    ...
    static final String AUDIO_AAC = "audio/aac"; // audio/mp4a-latm
    static final String AUDIO_AMR = "audio/amr";
    static final String AUDIO_ANY = "audio/*";
    static final String ANY_ANY = "*/*";
    
    static final int BITRATE_AMR =  5900; // bits/sec
    static final int BITRATE_AAC = 64000;
    static final int BITRATE_3GPP = 5900;

    static final int SAMPLINGRATE_AAC = 44100;
    static final int AAC_TIME_LIMIT = 6600;
    
    WakeLock mWakeLock;
    String mRequestedType = AUDIO_AAC;
    Recorder mRecorder;
    ...
    public void onCreate(Bundle icycle) {
        ...
        if (i != null) {
            String s = i.getType();
            if (AUDIO_AMR.equals(s) || AUDIO_AAC.equals(s) || AUDIO_3GPP.equals(s) || AUDIO_ANY.equals(s)
                    || ANY_ANY.equals(s)) {
                mRequestedType = s;
            } else if (s != null) {
        ...
        String default_format = android.os.SystemProperties.get("ro.soundrecorder.format");
        Log.v(TAG, "default format: " + default_format);
        if (default_format.equals("aac")) {
            isChecked = true;
            mRequestedType = AUDIO_AAC;
        } else if (default_format.equals("amr")) {
            isChecked = false;
            mRequestedType = AUDIO_AMR;
        } else {
            Log.v(TAG, "can not match the format:" + default_format);
            isChecked = false;
        }
        mFormatButton.setChecked(isChecked);
    ...
    private void initResourceRefs() {
        ...
        mFormatButton = (ToggleButton) findViewById(R.id.formatButton);
//        if (mRequestedType == AUDIO_AAC) {
//            isChecked = true;
//        } else {
//            isChecked = false;
//        }
        ...
        mFormatButton.setOnCheckedChangeListener(new OnCheckedChangeListener() {
            ...
            case R.id.recordButton:
                ...
                if (!Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
                    ...
                    if (AUDIO_AMR.equals(mRequestedType)) {
						...
						// aac encoding use 3gpp format for BUG00218006
						//ActionsCode(daichangxin, bugfix BUG00218006:use aac encode and file use .aac end) . 
            ...
            mSDCardMountEventReceiver = new BroadcastReceiver() {
                ...
                public void onReceive(Context context, Intent intent) {
                    ...
                    if (action.equals(Intent.ACTION_MEDIA_EJECT)) {
						//ActionsCode (daichangxin,modify media_eject ,not stop recording)
//                        mRecorder.delete();
    ...
    private void updateTimerView() {
        ...
        if (state == Recorder.PLAYING_STATE) {
            ...
            updateTimeRemaining();
            if (mRequestedType == AUDIO_AAC && time >= AAC_TIME_LIMIT) {
                mRecorder.stopForResume();
                saveSample();
                mRecorder.resume();
                mRemainingTimeCalculator.reset();
                if (!Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
                    ...
                    stopAudioPlayback();
                    mRemainingTimeCalculator.setBitRate(BITRATE_AAC);
					//ActionsCode (daichangxin,modify aac 3gp output format and aac encode ,also use 3gp/aac record)
//                        mRecorder.delete();
                    mRecorder.startRecording(MediaRecorder.OutputFormat.THREE_GPP, ".aac", this);                
