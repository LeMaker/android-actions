...
public class Recorder implements OnCompletionListener, OnErrorListener {
    ...
    public void startRecording(int outputfileformat, String extension, Context context) {
        ...
        if (outputfileformat == MediaRecorder.OutputFormat.THREE_GPP && (extension.equalsIgnoreCase(".aac"))) {  // ActionsCode(daichangxin, add aac encode format)
            mRecorder.setAudioSamplingRate(SoundRecorder.SAMPLINGRATE_AAC);
        ...
        try {
            mRecorder.start();
			//ActionsCode(daichangxin,avoid click aac button so fast, result in aac anr),
            if (startAAC) {
            	try {
					Thread.sleep(181);
    ...
    public void stopRecordingForResume() {
        if (mRecorder == null)
            return;
    ...
    public void stopPlaybackForResume() {
        if (mPlayer == null) // we were not in playback
            return;
    ...
    public void stopForResume(){
        stopRecordingForResume();
        stopPlaybackForResume();
    }
