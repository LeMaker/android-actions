                ...
                if (mInfoDelta.mTotalBytes > 0) {
                    ...
                    try {
                        ...
                        if (e.errno == OsConstants.ENOSYS || e.errno == OsConstants.ENOTSUP) {
                            ...
                            // ActionsCode(authro:songzhining, comment: fix BUG00273147, anr when cancel big apk download in google play)
                            //Os.ftruncate(outFd, mInfoDelta.mTotalBytes);
			final long bigSize = (curSize > 0) ? curSize : 150*1024*1024; // first time: 150M
                            long unit = 10*1024*1024; // 10M
                            
                            if ( mInfoDelta.mTotalBytes > bigSize) {
                                Log.w(TAG, "total size:" + mInfoDelta.mTotalBytes);
                                long size = bigSize;
                                while (size < mInfoDelta.mTotalBytes) {
                                    Log.w(TAG, "truncate size:" + size);
                                    Os.ftruncate(outFd, size);
                                    size += unit;
                                }
                            }
                            Os.ftruncate(outFd, mInfoDelta.mTotalBytes);
