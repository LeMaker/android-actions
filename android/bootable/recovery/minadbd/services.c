...
#else
/*
 *change write fuse to write tmp
 *ActionsCode(author:liaotianyang, type:new_method)
 */
static void sideload_service(int s, void *cookie)
{
    unsigned char buf[4096];
    unsigned long count = (unsigned long) cookie;
    int fd;

    fprintf(stderr, "sideload_service invoked,len=%d\n",count);
    ...
    while(count > 0) {
        unsigned long xfer = (count > 4096) ? 4096 : count;
...
int service_to_fd(const char *name)
{
    ...
    if (!strncmp(name, "sideload:", 9)) {
        ...
        // sideload-host).
		//ActionsCode(author:liaotianyang, type:changecode)
		 ret = create_service_thread(sideload_service, (void*)(unsigned long)atoi(name + 9));
        //exit(3);
