 ...
 *ActionsCode(author:liaotianyang, type:change_code)
 */
static char tp_rotate; // 0 不转，1转90度，2转180， 3转270
...
{    
    if (!show_text) {
	    ...
	    if (show_menu) {
		    ...
		    i = menu_top + (MENU_CHAR_H*showline+char_height-1)/char_height ; /*重新计算行数*/		
 ...
 int ScreenRecoveryUI::Get_Selected_Item()
{
	...
	case 0: 
		if ( abs_scale[1] ) {
			y =   tp_y*gr_fb_height()/abs_scale[1] ; //坐标缩放
		} else {
			y = tp_y  ; /*坐标体系不一样*/
		}		
		break;
	case 1:
		if ( abs_scale[0] ) {
			y =   gr_fb_height() -  tp_x*gr_fb_height()/abs_scale[0] ; //坐标缩放
		} else {
			y = gr_fb_height() - tp_x  ; //坐标体系不一样
		}
		break;
	case 2:
		if ( abs_scale[1] ) {
			y =   gr_fb_height() -  tp_y*gr_fb_height()/abs_scale[1] ; //坐标缩放
		} else {
			y = gr_fb_height() - tp_y  ; /*坐标体系不一样*/
		}	
		break;
	default:
		if ( abs_scale[0] ) {
			y =  tp_x*gr_fb_height()/abs_scale[0] ; //坐标缩放
		} else {
			y =  tp_x  ; //坐标体系不一样
		}	
		break;
