...
class RecoveryUI {
  public:
	...
	//ActionsCode(author:liaotianyang, type:newcode, comment:support cardburn show text)
	virtual void print_text(const char* msg) = 0;
    // --- key handling ---
    ...
    virtual int Get_Selected_Item() = 0;
    int abs_scale[2]; // tp max scale	
    int tp_x, tp_y; // touch for handle
...
private:
    ...
    //ActionsCode(author:liaotianyang, type:newcode, comment:support tp)
    int abs_x, abs_y; // touch pad
    char abs_flag; // bit0=abs_x, bit1=abs_y	
	...
	static int rec_uevent_init(void);
    static void parse_rec_uevent(const char *msg, rec_uevent *uevent);
    static void process_rec_uevent(rec_uevent *uevent);
    static int handle_rec_uevent_fd(int fd);
    static int rec_uevent_callback(int fd, uint32_t revents, void *data); 
    static void process_ps_rec_uevent(rec_uevent *uevent);
