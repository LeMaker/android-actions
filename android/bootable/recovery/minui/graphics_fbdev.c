...
static gr_surface fbdev_init(minui_backend* backend) {
    ...
    if (vi.yres * fi.line_length * 2 <= fi.smem_len) {
		...
		printf("==double framebuf==\n");

    } else {
        double_buffered = false;
		printf("==sigle framebuf==\n"); 
	...
	/*
	*support douple framebuf  rotate of framebuffer, 
	*ActionsCode(author:liaotianyang, type:change_code)
	*/
    gr_draw = (GRSurface*) malloc(sizeof(GRSurface));
    memcpy(gr_draw, gr_framebuffer, sizeof(GRSurface));
    ...
    printf("framebuffer: %d (%d x %d), %d\n", fb_fd, gr_draw->width, gr_draw->height, gr_draw->row_bytes);
...
static gr_surface fbdev_flip(minui_backend* backend __unused) {
    if (double_buffered) {
		...
		*/
		...
		#else
		roate_framebuf((char*)(gr_framebuffer[1-displayed_buffer].data),(char*)(gr_draw->data));
		set_displayed_framebuffer(1-displayed_buffer);
		#endif
    } else {
		...
		*/
		...
		#else
        roate_framebuf((char*)(gr_framebuffer[0].data),(char*)(gr_draw->data));
		#endif
#endif
    }
    return gr_draw;
