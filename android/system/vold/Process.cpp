...
*
*/ 
#define DATA_ANR_PATH "/data/anr"
#define SYSTEM_SERVER_NAME "system_server"

int Process::dumpStackTraces(int pid, const char *name) {
    char buffer[PATH_MAX];
    char *proc_name = (char*)name;

    // get process name
    if (proc_name == NULL) {
        getProcessName(pid, buffer, sizeof(buffer));
        proc_name = buffer;
    }

    // only dump system_server
    if (!strcmp(proc_name, SYSTEM_SERVER_NAME)) {
        // create dir: /data/anr
        mkdir(DATA_ANR_PATH, 0775);
        chmod(DATA_ANR_PATH, 0775);
        chown(DATA_ANR_PATH, AID_SYSTEM, AID_SYSTEM);

        // kill -3 to dump stack trace
        SLOGW("Sending SIGQUIT to process %d", pid);
        kill(pid, SIGQUIT);
    }
