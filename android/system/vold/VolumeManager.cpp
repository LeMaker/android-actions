...
//ActionsCode(huoysh, add %d to satisfy the format string) 
...
static int adjustSectorNumExt4(unsigned numSectors) {
    ...
    unsigned reservedSectors = (numSectors * 4)/100 + (numSectors % 25 > 0);
    numSectors += reservedSectors > (4096 * clusterSectors) ? (4096 * clusterSectors) : reservedSectors;
    return ROUND_UP_POWER_OF_2(numSectors, 3)+2048;
}
...
VolumeManager::VolumeManager() {
    //ActionsCode(authro:huoysh, comment: add dirty bytes config to prop)
    char dirtyBytesStr[PROPERTY_VALUE_MAX];
    property_get("ro.udisk.dirty_bytes", dirtyBytesStr, "2097152"); // default: 2M fix burnintest vfs_sync time out,usb bus reset;
	...
	//ActionsCode(huoysh, set dirty ratio to 5 when UMS is active) 
    mUmsDirtyRatio = 5;
    mUmsDirtyExpire = 1;
    ...
    mFilterKdev = -1;
    mFilterCount = 0;
	//ActionsCode(huoysh, add dirty byte) 
    mDirtyBytes=atoi(dirtyBytesStr); //2M
    mDirtyBackgroundBytes=2097152; //2M
    mSavedDirtyBackgroundRatio=-1;
    mSavedDirtyBytes=-1;
    mSavedDirtyBackgroundBytes=-1;

	//ActionsCode(huoysh, bugfix for usb disconnected when copying files) 
    mFifoBach = 1;
    mSavedFifoBach = -1;
}
...
void VolumeManager::handleBlockEvent(NetlinkEvent *evt) {
    ...
    if ( mFilterKdev == (dev_t)MKDEV(major, minor) ) {
...
int VolumeManager::mountVolume(const char *label) {
    ...
    if (!v) {
        errno = ENOENT;
        SLOGE("mountVolume fail, can't look up volume(%s)\n", label);
        return -1;
    }
...
int VolumeManager::shareVolume(const char *label, const char *method) {
	...
	//ActionsCode(huoysh, /mnt/sdcard to lun0 and anything else to lun1. Fix this.) 
    int lun_number;
    ...
    if (mUmsSharingCount++ == 0) {
        FILE* fp;
	//ActionsCode(huoysh, save four vm configurations that restored in unShareVolume) 		
	...
	if ((fp = fopen("/proc/sys/vm/dirty_background_bytes", "r+"))) {
        ...
        mSavedFifoBach = -1; // in case we fail
        if ((fp = fopen("/sys/block/actj/queue/iosched/fifo_batch", "r+"))) {
            char line[16];
            if (fgets(line, sizeof(line), fp) && sscanf(line, "%d", &mSavedFifoBach)) {
                fprintf(fp, "%d\n", mFifoBach);
            } else {
                SLOGE("Failed to read fifo_bach (%s)", strerror(errno));
            }
            fclose(fp);
        } else {
            SLOGE("Failed to open /sys/block/actj/queue/iosched/fifo_batch (%s)", strerror(errno));
        }
    }
    return 0;
}
...
int VolumeManager::unshareVolume(const char *label, const char *method) {
	...
	if (mUmsSharingCount == 0 && mSavedFifoBach != -1) {
		FILE* fp;
		if ((fp = fopen("/sys/block/actj/queue/iosched/fifo_batch", "r+"))) {
		  fprintf(fp, "%d\n", mSavedFifoBach);
		  fclose(fp);
...
int VolumeManager::cleanupAsec(Volume *v, bool force) {
	//ActionsCode(huoysh, Only primary storage needs ASEC cleanup.) 
    if (!v->isPrimaryStorage())
        return 0;
    int rc = 0;
