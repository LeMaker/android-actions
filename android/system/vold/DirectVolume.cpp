...
static bool is_mountpoint_used(const char *mp)
{
    char device[256];
    char mount_path[256];
    char rest[256];
    ...
    while(fgets(line, sizeof(line), fp)) {
        ...
        if (!strcmp(mount_path, mp)) {
            fclose(fp);
		SLOGE("isMountpointMounted2 (mount_path=%s),(mp=%s)",mount_path, mp);	
            return true;
        }
    }
    ...
    for (it = mPaths->begin(); it != mPaths->end(); ++it) {
        if ((*it)->match(dp)) {
            ...
            if (action == NetlinkEvent::NlActionAdd) {
                ...
                if (is_mountpoint_used(getMountpoint())||is_mountpoint_used(getFuseMountpoint())) {
                    SLOGW("Volume is  mounted , can't handle another disk !");
                    return 0;                
                }
                snprintf(nodepath,
                ...
                if (!strcmp(devtype, "disk")) {
                    if ((strstr(dp, "usb"))&&(mDiskMinor!=minor)){
                        SLOGE("disk_type is not right, return!mDiskMinor= (%d)", mDiskMinor);
                        goto OUT;                 
                    }		
                    ...
                    if (strstr(dp, "usb")){
                        for(i=0;i<MAX_PARTITIONS ; i++)
                            if(mPartMinors[i]==minor)
                                break;
                        if(i >= MAX_PARTITIONS){
                            SLOGE("Partition is not right, return!mDiskMinor= (%d),minor=%d", mDiskMinor,minor);
                            goto OUT; 
                        }
                    }
...
void DirectVolume::handleDiskAdded(const char * /*devpath*/,
    ...
    if (tmp) {
        ...
        SLOGI("hDA.PARTN=%d", mDiskNumParts);
    } else {
    ...
    if (mDiskNumParts == 0) {
        ...
        //  ActionsCode(huoysh, if ums enabled, share volume) 
...
void DirectVolume::handlePartitionAdded(const char *devpath, NetlinkEvent *evt) {
    ...
    if (part_num >= MAX_PARTITIONS) {
        ...
        if ((mPartMinors[part_num - 1] == -1) && mPendingPartCount)
            mPendingPartCount--;
        if(part_num>1){
            if(mPartMajors[part_num-2]!=major)
                SLOGI("hPA. new major for same disk, pre=%d, now=%d",
                      mPartMajors[part_num-1], major);
        }
        mPartMajors[part_num -1] = major;
        mPartMinors[part_num -1] = minor;
        SLOGD("Dv:partAdd:part_num=%d, (%d:%d)\n",part_num, mPartMajors[part_num -1], mPartMinors[part_num -1]);
    }
...
void DirectVolume::handleDiskChanged(const char * /*devpath*/,
    ...
    if (getState() != Volume::State_Formatting) {
        if (mDiskNumParts == 0) {
            SLOGD("umount vol if diskchange no parts\n");
...
void DirectVolume::handleDiskRemoved(const char * /*devpath*/,
    ...
    Volume::mIsDiskPlugOut	= 1;
	SLOGD("Volume %s %s disk %d:%d removed\n", getLabel(), getMountpoint(), major, minor);
	
	//ActionsCode(huoysh, change_code,uhost/udisk plugin/plugout unmount)
    ...
    if (state != Volume::State_Mounted && state != Volume::State_Shared) {
        SLOGE("Disk removed: state != Volume::State_Mounted && state != Volume::State_Shared,delay 500ms");	
        usleep(500000);
       // return;
    }
...
void DirectVolume::handlePartitionRemoved(const char * /*devpath*/,
    ...
    if ((dev_t) MKDEV(major, mDiskMinor) == mCurrentlyMountedKdev) {
        /*
...
#ifdef PARTITION_DEBUG
            SLOGD("mRelPartMinors[%d]=%d\n",i - rel,mPartMinors[i]);
#endif
            devs[i - rel] = MKDEV(mDiskMajor, mPartMinors[i]);
        }
        return mDiskNumParts -rel ;
    }
