...
static int do_recv(int s, const char *path, char *buffer)
{
    ...
    // ActionsCode(authro:songzhining, comment: fix cts selinux bug)
    if (!strcmp(path, "/sys/fs/selinux/policy") && (access("/system/etc/test_policy", R_OK) == 0)) {
        path = "/system/etc/test_policy";
    }
