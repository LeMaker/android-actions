                             ...
                             unsigned *uid, unsigned *gid, unsigned *mode, uint64_t *capabilities)
{
    ...
    for(; pc->prefix; pc++){
        ...
        /* If name ends in * or start with * then allow partial matches. */
        if (pc->prefix[len -1] == '*') {
            if(!strncmp(pc->prefix, path, len - 1)) break;
        } else if(pc->prefix[0] == '*' && plen >= len) {
            if(!strchr(path, '/') && !strncmp(&pc->prefix[1], &path[plen-len+1], len - 1)) break;
        } else if (plen == len){
            if(!strncmp(pc->prefix, path, len)) break;
        }
    }
