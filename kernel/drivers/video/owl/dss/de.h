...
struct owl_de_hwops {
	...
	/* path operations */
	void (*path_enable)(enum owl_channel path, bool enbale);
	bool (*path_is_enabled)(enum owl_channel path);

	void (*path_size_set)(enum owl_channel path, u32 width, u32 height);
	
	void (*display_type_set)(enum owl_channel path, enum owl_display_type type);

	void (*video_enable)(enum owl_channel path,
			     enum owl_plane video, bool enbale);
	bool (*video_is_enabled)(enum owl_channel path, enum owl_plane video);

	void (*fcr_set)(enum owl_channel path);
	bool (*fcr_get)(enum owl_channel path);

	void (*set_gamma_table)(enum owl_channel path, u32 *gamma);
	void (*get_gamma_table)(enum owl_channel path, u32 *gamma);
	void (*enable_gamma_table)(enum owl_channel path, bool enable);

	void (*dither_set)(enum owl_channel path, enum owl_dither_mode mode);
	void (*dither_enable)(enum owl_channel path, bool enable);
	...
	void (*position_set)(enum owl_channel path, enum owl_plane video,
				u32 x_pos, u32 u_pos);
	...
	void (*alpha_set)(enum owl_channel path, enum owl_plane video,
				u8 alpha_value, bool alpha_en,  bool pre_mult_en);

	void (*critical_set)(enum owl_channel path,enum owl_plane video);
...
int de_wait_for_irq_interruptible_timeout(enum de_irq_type, unsigned long timeout);



int de_ovl_setup(enum owl_channel channel ,enum owl_plane plane, struct owl_overlay_info *oi, bool ilace);
...
void de_set_gamma_table(enum owl_channel path, u32 *gamma);
void de_get_gamma_table(enum owl_channel path, u32 *gamma);
void de_enable_gamma_table(enum owl_channel path, bool enable);
