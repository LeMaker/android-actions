...
void report_ampdu_sz_event(_adapter *padapter, PAMPDU_PARM_EVT parm)
{
	struct cmd_priv *pcmdpriv = NULL ;
	struct mlme_ext_priv *pmlmeext = NULL ;
	struct C2HEvent_Header *pc2h_evt_hdr = NULL ;
	struct ampdu_parm_event *pampdu_parm_evt = NULL ;
	...
	if ((pcmd_obj = (struct cmd_obj* )rtw_zmalloc(sizeof(struct cmd_obj))) == NULL) {
		DBG_871X("%s: alloc cmd_obj fail!\n", __func__) ;
		return ;	
	}

	cmd_sz = (sizeof(struct ampdu_parm_event) + sizeof(struct C2HEvent_Header)); 
	if((pevt_cmd = (u8* )rtw_zmalloc(cmd_sz)) == NULL) {
		DBG_871X("%s: alloc evt_cmd fail!\n", __func__) ;
		return ;
	}	

	_rtw_init_listhead(&pcmd_obj->list) ;
	pcmd_obj->cmdcode = GEN_CMD_CODE(_Set_MLME_EVT);	
	pcmd_obj->cmdsz = cmd_sz ;
	pcmd_obj->parmbuf = pevt_cmd ;

	pcmd_obj->rsp = NULL ;
	pcmd_obj->rspsz  = 0 ;

	pc2h_evt_hdr = (struct C2HEvent_Header* )(pevt_cmd) ;
	pc2h_evt_hdr->len = sizeof(struct ampdu_parm_event);
	pc2h_evt_hdr->ID = GEN_EVT_CODE(_AmpduSzSet);
	pc2h_evt_hdr->seq = ATOMIC_INC_RETURN(&pmlmeext->event_seq);

	pampdu_parm_evt = (struct ampdu_parm_event* )(pevt_cmd + sizeof(struct C2HEvent_Header)) ;
	pampdu_parm_evt->max_AMPDU_len = parm->max_AMPDU_len ;
	pampdu_parm_evt->max_AMPDU_num = parm->max_AMPDU_num ;
	rtw_enqueue_cmd(pcmdpriv, pcmd_obj) ;
}

void rtw_ampdu_sz_set_event_callback(_adapter *padapter, u8 *pbuf)
{
	PAMPDU_PARM_EVT pampdu_parm_evt = (PAMPDU_PARM_EVT)pbuf ;
	DBG_871X("%s@%d: update  max_AMPDU_len=%d\n", __func__, __LINE__, pampdu_parm_evt->max_AMPDU_len) ;
//rtw_hal_set_hwreg(padapter, HW_VAR_AMPDU_FACTOR, (u8 *)(&pampdu_parm_evt->max_AMPDU_len));
   rtw_write32(padapter, 0x0458, 0x1800);
	//update REG_MAX_AGGR_NUM_8723B
	//rtw_write8(padapter, 0x4ca, pampdu_parm_evt->max_AMPDU_num);	
}
