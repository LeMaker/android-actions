...
static bool get_power_ctrl_policy(void)
{
	...
	if (of_find_property(np, "no_bind_usb_port", NULL)) {
		return true;
	}
	return false;
}
...
{
	if(get_power_ctrl_policy()) return;
...
static int __init rtw_drv_entry(void)
{
	...
	wifi_power_on();
	DBG_871X_LEVEL(_drv_always_, "module init start,usb id:%d\n", aotg_id);
