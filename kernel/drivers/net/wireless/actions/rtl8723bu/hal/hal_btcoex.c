...
static u8 _is_btfwver_valid(PBTC_COEXIST pBtCoexist)
{
	if (IS_HARDWARE_TYPE_8723B(pBtCoexist->Adapter)) {
		if (pBtCoexist->boardInfo.btdmAntNum == 1) {
			BOOLEAN valid;
			valid = EXhalbtc8723b1ant_is_btfw_valid(pBtCoexist);
			if (valid == FALSE)
				return _FALSE;
			return _TRUE;
		}
	...
	if (!pBtCoexist->btInfo.btRealFwVer)
		return _FALSE;

	return _TRUE;
}

static void _query_btfwver(PBTC_COEXIST pBtCoexist)
{
	u8 buf[5] = {0};


	GLBtcQueryBtFwSeq = GLBtcBtMpInfoSeq++;
	GLBtcQueryBtFwSeq &= 0xF;

	buf[0] = (GLBtcQueryBtFwSeq << 4); /* ver & reqnum*/
	buf[1] = 0; /* BT_OP_GET_BT_VERSION = 0 */

	rtw_btcoex_btmpoper_cmd(pBtCoexist->Adapter, buf);
	pBtCoexist->btInfo.getBtFwVerCnt++;
}

u32 halbtcoutsrc_GetBtPatchVer(PBTC_COEXIST pBtCoexist)
{
	if (_is_btfwver_valid(pBtCoexist) == _FALSE) {
		if (halbtcoutsrc_IsHwMailboxExist(pBtCoexist) == _TRUE) {
			_query_btfwver(pBtCoexist);
		} else {
...
void hal_btcoex_InitHwConfig(PADAPTER padapter, u8 bWifiOnly)
{
	...
	halbtcoutsrc_GetBtPatchVer(&GLBtCoexist);
...
void hal_btcoex_BtMpInfoNotify(PADAPTER padapter, u8 length, u8 *tmpBuf)
{
	u8 extid, seq, len;
	u16 btRealFwVer;
	u8 btFwVer;
	...
	if (seq == GLBtcQueryBtFwSeq) {
		btRealFwVer = tmpBuf[3] | (tmpBuf[4] << 8);
		btFwVer = tmpBuf[5];

		GLBtCoexist.btInfo.btRealFwVer = btRealFwVer;
		GLBtCoexist.btInfo.btFwVer = btFwVer;
	}
}
