...
void rtw_btcoex_BtMpInfoNotify(PADAPTER padapter, u8 length, u8 *tmpBuf)
{
	PHAL_DATA_TYPE	pHalData;
	...
	hal_btcoex_BtMpInfoNotify(padapter, length, tmpBuf);
}
...
/*
 * cmd[0][3:0] is OP code version
 * cmd[0][7:4] is sequence number
 * cmd[1] is OP code
 * cmd[2-4] are other parameters
 */
void rtw_btcoex_btmpoper_cmd(PADAPTER padapter, u8 *cmd)
{
	rtw_hal_set_hwreg(padapter, HW_VAR_H2C_BT_MP_OPER, cmd);
}
