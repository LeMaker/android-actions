...
void rtw_signal_stat_timer_hdl(RTW_TIMER_HDL_ARGS){
	...
	if(adapter->recvpriv.is_signal_dbg) {
		...
		DBG_871X("[WIFIDBG] avg_signal_strength = %d, num_signal_strength = %d, recvpriv->signal_strength = %d !!!!!!!!!!\n", avg_signal_strength, num_signal_strength, recvpriv->signal_strength);//add by RTK
		...
		if (check_buddy_fwstate(adapter, _FW_UNDER_SURVEY) == _TRUE)
			goto set_timer;
		#endif
#if 0 // modify start by RTK 20150116
		...
		//update value of signal_strength, rssi, signal_qual
		{
			u8 Weight_Now = 7, Weitht_pre = 3, Weight_Total = Weight_Now + Weitht_pre;

                	tmp_s = (Weight_Now * avg_signal_strength + Weitht_pre * recvpriv->signal_strength);
                	if(tmp_s % Weight_Total)
                        	tmp_s = tmp_s / Weight_Total + 1;
                	else
                        	tmp_s = tmp_s / Weight_Total;
                	if(tmp_s > 100)
                        	tmp_s = 100;

                	tmp_q = (Weight_Now * avg_signal_qual + Weitht_pre * recvpriv->signal_qual);
                	if(tmp_q % Weight_Total)
                        	tmp_q = tmp_q / Weight_Total + 1;
                	else
                        	tmp_q = tmp_q / Weight_Total;
                	if(tmp_q > 100)
                        	tmp_q = 100;
		}
#endif //modify end by RTK 20150116
...
//	DBG_871X(FUNC_ADPT_FMT" signal_strength:%3u, rssi:%3d, signal_qual:%3u"
		DBG_871X("[WIFIDBG] "FUNC_ADPT_FMT" signal_strength:%3u, rssi:%3d, signal_qual:%3u"
