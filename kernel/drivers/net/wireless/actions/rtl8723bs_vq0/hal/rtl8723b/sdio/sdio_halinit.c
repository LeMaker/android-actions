...
#define MISCINFO_FILE_LEN (1024*8)
#define MAX_MISCINFO_LEN (1024*8)
//extern int getMiscInfo(unsigned char *buf, int buflen);

typedef int(*FUNC)(unsigned char*, int);
FUNC do_getMiscInfo;
...
int get_miscinfo(char *buf)
{
	...
	printk("===> get misc info\n");


	do_getMiscInfo = (FUNC)kallsyms_lookup_name("getMiscInfo");
	if(do_getMiscInfo){
		printk("[gadget] we gonna get misc info\n");
		ret = do_getMiscInfo(buf, MAX_MISCINFO_LEN);
		if(ret < 0){
			printk("[gadget] get misc info failed!\n");
			return -1;
		}
	}else{
		printk("[gadget] we can't find getMiscInfo functions\n");
	}
	printk("%s:len=%d\n", __FUNCTION__, ret);
...
typedef struct
{
  unsigned int magic; //0x55aa55aa
  char name[8];
  int length;
  unsigned char data[]; //for the symbol only
}MiscInfo_item_t;

typedef struct
{
    int length; 
    MiscInfo_item_t items[]; 
}MiscInfos_t;

char *misc_name[] = {//name_size <=7
  "SN", 
  "DRM",
  "HDCP",
  "DEVNUM",
...
static int get_wifimac_form_miscinfo(char* d_buf, int len)
{
	int ret = 0,i = 0, actual_len;
	char *buf;
	MiscInfos_t *miscInfos = NULL;
	MiscInfo_item_t *item = NULL;
	printk("===> get_wifimac_form_miscinfo\n");

	buf = kzalloc(MISCINFO_FILE_LEN, GFP_KERNEL);
    if (buf == NULL)
    	return -ENOMEM;

	//ret = getMiscInfo(buf, MISCINFO_FILE_LEN);
	ret = get_miscinfo(buf);

	miscInfos = (MiscInfos_t *)buf;
	item = (MiscInfo_item_t *)((char *)miscInfos + sizeof(miscInfos->length));
	while(((char*)item - (char*)miscInfos) < miscInfos->length ){
		item->name[7] = '\0';
		if (strstr(item->name, misc_name[6])){
			printk("......Ok, find WIFIMAC\n");
			ret = 1;
			break;
		}
		i++;
		item = (MiscInfo_item_t *)((char*)item + item->length + sizeof(MiscInfo_item_t));
	}


	if (ret == 1){
		memset(d_buf, 0, len);
		actual_len = (len - 1) > (strlen(item->data)) ? strlen(item->data) : (len - 1);
		strncpy(d_buf, item->data, actual_len);
		d_buf[actual_len] = '\0';
		ret = actual_len;
	}else{
		ret = -1;
...
#ifdef CONFIG_EFUSE_CONFIG_FILE
static void
Hal_ReadMACAddrFromMiscinfo_8188ES(
	PADAPTER		padapter,
	char * buf)
{
	u32 i;
	...
	strncpy(source_addr, buf, 17);
	source_addr[17] = ':';
	printk("Miscinfo wifi mac address:%s\n", source_addr);
	...
	)
{
	...
	if (check_phy_efuse_tx_power_info_valid(padapter) == _FALSE) {
		...
		if (fp == NULL || IS_ERR(fp)) {
			...
			printk("### fp OK, get /misc/modules/wifi_efuse_8723bs_vq0.map");
	...
	if (check_phy_efuse_macaddr_info_valid(padapter) == _TRUE) {
		...
		memset(buf, 0 ,32);
#ifdef CONFIG_PLATFORM_ACTIONS_ATM705X
	printk("      ReadEfuseInfo8723BS ---> get_wifimac_form_miscinfo\n");
		ret = get_wifimac_form_miscinfo(buf, 32);
		if (ret < 0){
#else				
...
#ifdef CONFIG_PLATFORM_ACTIONS_ATM705X		
			Hal_ReadMACAddrFromMiscinfo_8188ES(padapter, buf);
#else
