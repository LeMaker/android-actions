...
#if 0 //(LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 27)) && defined(CONFIG_PM_SLEEP)
...
#if !defined(OOB_INTR_ONLY)
void sdio_claim_host_lock_local(void) // terence 20140926: fix for claim host issue
{
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 25)) && 1
	if (gInstance)
		mutex_lock(&gInstance->claim_host_mutex);
#endif
}

void sdio_claim_host_unlock_local(void) // terence 20140926: fix for claim host issue
{
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 25)) && 1
	if (gInstance)
		mutex_unlock(&gInstance->claim_host_mutex);
#endif
}
...
static void IRQHandler(struct sdio_func *func)
{
	...
	if (mutex_is_locked(&gInstance->claim_host_mutex)) {
		printf("%s: muxtex is locked and return\n", __FUNCTION__);
		return;
	}
#endif /* (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 25)) */

	sdio_claim_host_lock_local();
	sdio_release_host(sd->func[0]);
